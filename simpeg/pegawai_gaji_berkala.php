...
if($reqDepartemen == "")
	$reqDepartemen = $userLogin->idDepartemen;
...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-simpeg/pegawai_gaji_berkala_json.php?reqTahun=<?=$reqTahun?>&reqBulan=<?=$reqBulan?>&reqDepartemen=<?=$reqDepartemen?>&reqJenisPegawai="+ $("#reqJenisPegawai").val(),
			...
			/* Click event handler */
			  ...
			  $('#example tbody').on( 'click', 'tr', function () {
					if ( $(this).hasClass('selected') ) {
					  ...
					  anSelectedPeriodeBaru = element[1];
					  anSelectedMKTBaru = element[2];
					  anSelectedMKBBaru = element[3];
			  });
			  
			  $('#btnEdit').on('click', function () {
				  if(anSelectedData == "")
					  return false;
				  window.parent.OpenDHTML('pegawai_gaji_berkala_add.php?reqId='+anSelectedId+'&reqPeriodeBaru='+anSelectedPeriodeBaru+'&reqMKTBaru='+anSelectedMKTBaru+'&reqMKBBaru='+anSelectedMKBBaru, 'Office Management - Aplikasi Kepegawaian', '980', '515');	
			  });
			  			  
			  $('#btnProses').on('click', function () {
				 oTable.fnReloadAjax("../json-simpeg/pegawai_gaji_berkala_json.php?reqMode=proses&reqTahun="+$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val()+"&reqDepartemen=<?=$reqDepartemen?>&reqJenisPegawai="+ $("#reqJenisPegawai").val());
			  });
			  
			  
			  $("#reqJenisPegawai").change(function() {
				 var depId = $('#cc').combotree('getValue'); 
				 oTable.fnReloadAjax("../json-simpeg/pegawai_gaji_berkala_json.php?reqTahun="+$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val()+"&reqDepartemen="+depId+"&reqJenisPegawai="+ $("#reqJenisPegawai").val());
			  });
			  ...
			  $('#cc').combotree({
					onSelect: function(param){
						 oTable.fnReloadAjax("../json-simpeg/pegawai_gaji_berkala_json.php?reqTahun="+$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val()+"&reqDepartemen="+param.id+"&reqJenisPegawai="+ $("#reqJenisPegawai").val());
					}
			  });
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png">Data Pegawai Gaji Berkala</span>
            </div>            
            </td>
        </tr>
    </table>
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul>
            ...
            <a href="#" id="btnEdit" title="Proses">Update Data</a>
            </li>        
            <li>
    ...
    <div style="position: absolute; margin-top:6px; margin-left:175px; z-index:9999; font-size:12px;">
    	Bulan
    ...
    <thead>
        <tr>
            ...
            <th width="90px">Periode Lama</th> 
            <th width="90px">MKT Lama</th>
            <th width="90px">MKB Lama</th>
            <th width="90px">TMT Lama</th> 
            <th width="90px">Periode Baru</th> 
            <th width="90px">MKT Baru</th>
            <th width="90px">MKB Baru</th> 
        </tr>
    </thead>
    </table>
    </div>    
