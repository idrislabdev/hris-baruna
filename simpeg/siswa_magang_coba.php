        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			...
			/* Click event handler */
			  ...
			  $('#btnEkspor').on('click', function () {
				  ...
				  newWindow = window.open('siswa_magang_data_excel.php?reqDepartemen='+ depId +'&reqPeriodeAwal='+ $('#reqTanggalAwal').val() + "&reqPeriodeAkhir=" + $('#reqTanggalAkhir').val(), 'Cetak');
				  newWindow.focus();
			  });	

			//$('#reqTanggalAwal').datebox();

		  	$('#reqTanggalAwal').live('change', function (){
		  		var depId = $('#cc').combotree('getValue');
			...
			$('#reqTanggalAkhir').live('change', function (){
				var depId = $('#cc').combotree('getValue');
			 	oTable.fnReloadAjax("../json-simpeg/siswa_magang_json.php?reqDepartemen="+ depId +"&reqPeriodeAwal="+ $('#reqTanggalAwal').val() + "&reqPeriodeAkhir=" + $('#reqTanggalAkhir').val());				
			});
			  
			  $('#reqTanggalAwal').bind('datebox', function(e, p) {
					if ( p.method === 'set' ) {
...
<div id="wadah">
    ...
    <div style="position: absolute; margin-top:8px; margin-left:178px; z-index:9999; font-size:12px;">
