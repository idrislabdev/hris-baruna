...
$pegawai_jabatan->selectByParamsJabatanTerakhir(array('A.PEGAWAI_ID'=>$reqId));
$pegawai_jabatan->firstRow();
//echo $pegawai_jabatan->query;exit;
...
*/	
$set_list= new PegawaiPenghasilan();
$set_list->selectByParamsListKelas($tempKelas);
$set_list->firstRow();
//echo $set_list->query;exit;
$tempJumlahPenghasilan= $set_list->getField("PENGHASILAN_MERIT");
$tempJumlahTunjanganJabatan= $set_list->getField("TUNJANGAN_JABATAN");
$tempJumlahTPP= $set_list->getField("TUNJANGAN_KEHADIRAN");
unset($set_list);
...
else
{
	...
	$tempLumpsumMakan = $pegawai_penghasilan->getField('LUMPSUM_MAKAN');
	$tempLumpsumTransport = $pegawai_penghasilan->getField('LUMPSUM_TRANSPORT');
...
$jumlah_entri = $pegawai_penghasilan->getCountByParams(array("PEGAWAI_ID" => $reqId));

if($tempJumlahTunjanganJabatan == "")
	$tempJumlahTunjanganJabatan = 0;
if($tempJumlahInsentif == "")
	$tempJumlahInsentif = 0;
?>
    ...
    <script type="text/javascript">
		...
		function setValue(){
			...
			else
			{
				...
				if($jumlah_entri > 0 && $userLogin->pegawaiId <> 1456
				 && $userLogin->pegawaiId <> 1450 && $userLogin->pegawaiId <> 1451
				 && $userLogin->pegawaiId <> 1662)
				{
				?>
		...
		$(function(){
			...
			<?php /*?>$("#reqJumlahPenghasilan").keyup(function(e) {
    ...
    <table>
        ...
        <tr>
            ...
            <td>
                ...
                &nbsp;&nbsp;&nbsp;&nbsp;
                (%)<input name="reqProsentaseTransportasi" <?=getEnableDisableInput($json_item_gaji, "reqProsentaseTransportasi")?> id="reqProsentaseTransportasi" class="easyui-validatebox" size="3" maxlength="3" type="text" value="<?=$tempProsentaseTransportasi?>" /> <input name="reqLumpTransport" id="reqLumpTransport" <? if ($tempLumpsumTransport == 1) echo "checked";?> type="checkbox" value="1" /> Lumpsum
            </td>
            ...
            <td>
                ...
                &nbsp;&nbsp;&nbsp;&nbsp;
                (%)<input name="reqProsentaseUangMakan" <?=getEnableDisableInput($json_item_gaji, "reqProsentaseUangMakan")?>  id="reqProsentaseUangMakan" class="easyui-validatebox" required size="3" maxlength="3" type="text" value="<?=$tempProsentaseUangMakan?>" /> <input name="reqLumpMakan" id="reqLumpMakan" <? if ($tempLumpsumMakan == 1) echo "checked";?> type="checkbox" value="1" /> Lumpsum
            </td>
        </tr>
        <tr>
            <td>Insentif </td>
