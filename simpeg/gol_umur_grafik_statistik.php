    ...
    <script type="text/javascript">
        $(document).ready(function () {
			...
			var source =
			{
				...
				url: '../json-grafik/usia_json.php?reqDepartemen=<?=$reqDepartemen?>'
			};
			var dataAdapter = new $.jqx.dataAdapter(source, { async: false, autoBind: true, loadError: function (xhr, status, error) { alert('Error loading "' + source.url + '" : ' + error);} });
		
			// prepare jqxChart settings
			var settings = {
				title: "Grafik Pegawai Berdasarkan Golongan Umur",
			...
			$.getJSON('../json-grafik/usia_json.php?reqDepartemen=<?=$reqDepartemen?>', function (data) 
			{
			  ...
			  $('#reqDepartemen').combotree({
					onSelect: function(param){
						document.location.href = 'gol_umur_grafik_statistik.php?reqDepartemen='+param.id;
					}
			  });			
		...
		{
			top.OpenDHTML('gol_umur_grafik_statistik_monitoring.php?reqDepartemen=<?=$reqDepartemen?>&reqId='+id, 'HRIS - Aplikasi Kepegawaian', '600', '300');	
		}
    </script>
