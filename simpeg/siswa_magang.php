...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-simpeg/siswa_magang_json.php?reqDepartemen=<?=$reqDepartemen?>&reqStatusPegawai=1",
			...
			/* Click event handler */
			  ...
			  $('#btnEdit').on('click', function () {
				  if(anSelectedData == "")
					...
					{
						window.parent.OpenDHTML('pegawai_kadet_add.php?reqId='+anSelectedId+'&reqDepartemen=<?=$reqDepartemen?>', 'Office Management - Aplikasi Kepegawaian', '880', '495');
					}
			  });
			...
			var kelas = "";
			  $('#btnEkspor').on('click', function () {
				  newWindow = window.open("siswa_magang_data_excel.php?reqDepartemen="+ departemen +"&reqKelas="+ kelas, 'Cetak');
				  newWindow.focus();
			  });	

			//$('#reqTanggalAwal').datebox();

		  	$('#reqTanggalAwal').on('change', function (){				
		  		var depId = $('#cc').combotree('getValue');
				oTable.fnReloadAjax("../json-simpeg/siswa_magang_json.php?reqDepartemen="+ depId +"&reqPeriodeAwal="+ $('#reqTanggalAwal').val() + "&reqPeriodeAkhir=" + $('#reqTanggalAkhir').val());				
			});
				
			$('#reqTanggalAkhir').on('change', function (){
				var depId = $('#cc').combotree('getValue');
			 	oTable.fnReloadAjax("../json-simpeg/siswa_magang_json.php?reqDepartemen="+ depId +"&reqPeriodeAwal="+ $('#reqTanggalAwal').val() + "&reqPeriodeAkhir=" + $('#reqTanggalAkhir').val());				
			});
			...
			$('#cc').combotree({
					onSelect: function(param){
						departemen = param.departemen_id;
						kelas      = param.kelas_id;
						oTable.fnReloadAjax("../json-simpeg/siswa_magang_json.php?reqDepartemen="+ param.departemen_id +"&reqKelas="+ param.kelas_id +"&reqStatusPegawai="+ $('#reqStatusPegawai').val());
					}
			});
			
			  					  
		  $("#reqStatusPegawai").change(function() { 
			oTable.fnReloadAjax("../json-simpeg/siswa_magang_json.php?reqDepartemen="+ departemen +"&reqKelas="+ kelas +"&reqStatusPegawai="+ $('#reqStatusPegawai').val());
		  });
			  ...
			  $('#btnNonAktif').on('click', function () {
				  ...
				  if(confirm("Nonaktifkan NIS "+anSelectedNissKadet+"?"))
				  {
					  $.getJSON('../json-simpeg/pegawai_set_status.php?reqId='+anSelectedId+'&reqNilai=6', function (data) 
					  { });		
					 alert("Siswa berhasil di non aktifkan");	
					oTable.fnReloadAjax("../json-simpeg/siswa_magang_json.php?reqDepartemen="+ departemen +"&reqKelas="+ kelas +"&reqStatusPegawai="+ $('#reqStatusPegawai').val());
				  }
			  ...
			  $('#btnAktif').on('click', function () {
				  ...
				  if(confirm("Aktifkan NIS "+anSelectedNissKadet+"?"))
				  {
						  $.getJSON('../json-simpeg/pegawai_set_status.php?reqId='+anSelectedId+'&reqNilai=1', function (data) 
						  {});			
					 alert("Siswa berhasil di aktifkan");	
					oTable.fnReloadAjax("../json-simpeg/siswa_magang_json.php?reqDepartemen="+ departemen +"&reqKelas="+ kelas +"&reqStatusPegawai="+ $('#reqStatusPegawai').val());
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png">Data Siswa</span>
            </div>            
            </td>
        </tr>
    </table>
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul>
            ...
            <a href="#" id="btnNonAktif" title="Non Aktifkan">Non Aktifkan</a> 
            <a href="#" id="btnAktif" title="Aktifkan">Aktifkan</a> 
            <a href="#" id="btnEkspor" title="Edit">Export Excel</a> 
            <a href="#" id="btnImport" onClick="window.parent.OpenDHTML('data_siswa_import.php?reqMode=import', 'Office Management - Aplikasi Penghasilan', '600', '300');" title="Tambah">Import</a>
            </li>
          <?php /*?>  <li>
    ...
    <div style="position: absolute; margin-top:4px; margin-left:188px; z-index:9999; font-size:12px;">
        Sekolah : <input id="cc" class="easyui-combotree" name="reqDepartemen" data-options="url:'../json-simpeg/sekolah_combo_json.php'" style="width:300px;" value="<?=$reqDepartemen?>">&nbsp;&nbsp;
