    ...
    <script type="text/javascript">
        $(document).ready(function () {
			...
			var source =
			{
				...
				url: '../json-grafik/agama_json.php?reqDepartemen=<?=$reqDepartemen?>'
			};
			var dataAdapter = new $.jqx.dataAdapter(source, { async: false, autoBind: true, loadError: function (xhr, status, error) { alert('Error loading "' + source.url + '" : ' + error);} });
		
			// prepare jqxChart settings
			var settings = {
				title: "Grafik Pegawai Berdasarkan Agama",
				description: "Tahun <?=date("Y")?>",
				enableAnimations: true,
                showLegend: true,
                legendLayout: { left: 480, top: 140, width: 300, height: 200, flow: 'vertical' },
			...
			$.getJSON('../json-grafik/agama_json.php?reqDepartemen=<?=$reqDepartemen?>', function (data) 
			{
			  ...
			  $('#reqDepartemen').combotree({
					onSelect: function(param){
						document.location.href = 'agama_grafik_statistik.php?reqDepartemen='+param.id;
					}
			  });			
