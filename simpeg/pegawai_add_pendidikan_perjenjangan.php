...
$pegawai_pendidikan_perjenjangan->selectByParams(array('PEGAWAI_PEND_PERJENJANGAN_ID'=>$reqRowId));
$pegawai_pendidikan_perjenjangan->firstRow();
//echo $pegawai_pendidikan_perjenjangan->query;

$tempTanggalAwal = dateToPageCheck($pegawai_pendidikan_perjenjangan->getField('TANGGAL_AWAL'));
$tempTanggalAkhir = dateToPageCheck($pegawai_pendidikan_perjenjangan->getField('TANGGAL_AKHIR'));
$tempNama = $pegawai_pendidikan_perjenjangan->getField('NAMA');
$tempRowId = $reqRowId;
?>
...
<head>
    ...
    <script type="text/javascript">
		...
		$(function(){
			$('#ff').form({
				url:'../json-simpeg/pegawai_add_pendidikan_perjenjangan.php',
				onSubmit:function(){
				...
				success:function(data){
					...
					parent.frames['mainFramePop'].location.href = 'pegawai_add_pendidikan_perjenjangan_monitoring.php?reqId=' + data[0];
					<? if($tempRowId) { ?>
						document.location.href = 'pegawai_add_pendidikan_perjenjangan.php?reqId=' + data[0] + '&reqRowId=' + data[2];
					<? } ?>
				}				
			});
		});
...
<div id="wadah" class="CodeMirror-scroll">
	<div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif); position:fixed; width:100%; margin-top:0px; zIndex: -1'">    
		...
		?>
            <li>
            <a href="pegawai_add_pendidikan_perjenjangan.php?reqId=<?=$reqId?>">Batal</a>
            </li>        
        <?
		}
		?>
