...
$reqJenisPegawaiId = httpFilterRequest("reqJenisPegawaiId");
...
if($pegawai->getField('JENIS_PEGAWAI_ID') == 1 || $pegawai->getField('JENIS_PEGAWAI_ID') == 2)
	$statement = " AND NOT JENIS_PEGAWAI_ID IN (1, 2, 3, 4, 5, 8) ";
else
	$statement = " AND NOT JENIS_PEGAWAI_ID IN (2, 6, 7, 8) ";
	
$jenis_pegawai->selectByParams(array(), -1, -1, $statement);
...
<head>
    ...
    <script type="text/javascript">
		...
		$(function(){
			$('#ff').form({
				url:'../json-simpeg/pegawai_add_perubahan_jenis_pegawai.php',
				onSubmit:function(){
    		...
    		<form id="ff" method="post" novalidate style="margin-top:20px;">
                <table border="0" width="100%">
                    <tr>
                        ...
                        <td>
                            <?=$tempJenisPegawaiNama?>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            Perubahan Jenis Pegawai
                        </td>
                        <td>
                            <select name="reqJenisPegawaiId" id="reqJenisPegawaiId">
                            <?
                            while($jenis_pegawai->nextRow())
                            {
                            ?>
                                <option value="<?=$jenis_pegawai->getField("JENIS_PEGAWAI_ID")?>" <? if($tempJenisPegawaiId == $jenis_pegawai->getField("JENIS_PEGAWAI_ID")) { ?> selected <? } ?>><?=$jenis_pegawai->getField("NAMA")?></option>
                            <?
                            }
                            ?>
                            </select>
