...
<html>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
			...
			var source =
			{
				...
				url: '../json-grafik/kelamin_json.php?reqDepartemen=<?=$reqDepartemen?>'
			};
			var dataAdapter = new $.jqx.dataAdapter(source, { async: false, autoBind: true, loadError: function (xhr, status, error) { alert('Error loading "' + source.url + '" : ' + error);} });
		
			// prepare jqxChart settings
			var settings = {
				title: "Grafik Pegawai Berdasarkan Jenis Kelamin",
				description: "Tahun <?=date("Y")?>",
				enableAnimations: true,
                showLegend: true,
                legendLayout: { left: 530, top: 140, width: 300, height: 200, flow: 'vertical' },
			...
			$.getJSON('../json-grafik/kelamin_json.php?reqDepartemen=<?=$reqDepartemen?>', function (data) 
			{
			  ...
			  $('#reqDepartemen').combotree({
					onSelect: function(param){
						document.location.href = 'jenis_kelamin_grafik_statistik.php?reqDepartemen='+param.id;
					}
			  });			
		...
		{
			top.OpenDHTML('jenis_kelamin_grafik_statistik_monitoring.php?reqDepartemen=<?=$reqDepartemen?>&reqId='+id, 'HRIS - Aplikasi Kepegawaian', '600', '300');	
		}
    </script>
    ...
    <div id="konten">
		...
		<div id='chartGolongan' class="grafik-tampil" style="width:680px; height:500px; position: relative; left: 0px; top: 0px;"></div>    
    </div>
    </body>
</html>
