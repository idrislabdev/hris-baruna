...
$tempAkhirTMT = dateToPageCheck($pegawai_hukuman->getField('AKHIR_TMT'));
...
<head>
	...
	function setValue(sts_pegawai){
		...
		$(function(){
			...
			$('#reqJenisHukumanId').bind('change', function(ev) {		
				var status_pegawai = $('#reqJenisHukumanId').val();
				
				$('#labelAkhirTMT').hide();
				$('#labelAkhirTMT2').hide();
				if(status_pegawai == 14)
				{
					$('#labelAkhirTMT').show();
					$('#labelAkhirTMT2').show();
				}
...
<body onLoad="setValue(<?=$tempJenisHukumanId?>);">
...
<div id="wadah" class="CodeMirror-scroll">
	<div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif); position:fixed; width:100%; margin-top:0px; zIndex: -1'">    
		...
		?>
            <li>
            <a href="pegawai_add_hukuman.php?reqId=<?=$reqId?>">Batal</a>
            </li>        
        <?
		}
		?>
    ...
    <table>
        ...
        <tr>
            ...
            <td><label id="labelAkhirTMT2">Akhir SK</label></td>
            <td>
             <label id="labelAkhirTMT">
                 <input id="reqAkhirTMT" name="reqAkhirTMT" data-options="validType:'date'" class="easyui-datebox" value="<?=$tempAkhirTMT?>"></input>
             </label>
            </td>
        </tr>
        <tr>
