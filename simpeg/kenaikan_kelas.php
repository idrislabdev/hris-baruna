...
$tahun_ajaran->selectByParams(array(),-1,-1,"","ORDER BY TAHUN_AJARAN_ID DESC");
while($tahun_ajaran->nextRow())
{
	$arrPeriode[] = $tahun_ajaran->getField("PERIODE");	
}
...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-simpeg/kenaikan_kelas_json.php?reqPeriode=<?=$reqPeriode?>",						  
			...
			/* Click event handler */
			  ...
			  $("#reqPeriode").change(function() { 
				 ...
				 // else
			  		...
			  		oTable.fnReloadAjax("../json-simpeg/kenaikan_kelas_json.php?reqPeriode="+ $("#reqPeriode").val());
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Data Integrasi Kenaikan Kelas Siswa</span>
            </div>            
            </td>
        </tr>
    </table>
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul>
            <li>
            <a href="#" id="btnImport" onClick="window.parent.OpenDHTML('integrasi_kenaikan_import.php?reqMode=import', 'Office Management - Aplikasi Penghasilan', '600', '300');" title="Tambah">Import</a>
    ...
    <div style="position: absolute; margin-top:6px; margin-left:180px; z-index:9999; font-size:12px;">
 		Periode : <select name="reqPeriode" id="reqPeriode">
				  ...
				  {
				  ?>
                  	<option value="<?=$arrPeriode[$i]?>" <? if($reqPeriode == $arrPeriode[$i]) { ?> selected <? } ?>><?=getNameTahunAjaran($arrPeriode[$i])?></option>
                  <?	  
				  }
				  ?>
        		  </select>
