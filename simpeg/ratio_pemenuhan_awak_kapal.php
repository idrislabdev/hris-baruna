...
$kapal_jenis->selectByParamsPemenuhanAwakKapalJenis(array(), -1, -1, $reqPeriode);
...
$(document).ready( function () {
	$("#reqPeriode").change(function() {
	  document.location.href = "ratio_pemenuhan_awak_kapal.php?reqHeight=<?=$reqHeight?>&reqPeriode=" + $("#reqPeriode").val();
	});	
});
...
?>
<script type="text/javascript">
	window.location.replace('ratio_pemenuhan_awak_kapal.php?reqHeight=' + screen.height);
</script>
<?
}
?>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Pemenuhan Awak Kapal</span>
            </div>            
            </td>
        </tr>
    </table>
        ...
        <!--<div id="tableContainer" class="tableContainer">-->
            <div class="scroll">
              ...
              <table class="example scrollTable" id="sf">
                    ...
                    ?> 
                        <tr id="node-KJ<?=$kapal_jenis->getField("KAPAL_JENIS_ID")?>">
                            <td class="kolom-awal-tbody jarak-tree-table"><?=$kapal_jenis->getField("NAMA")?></td>
                            <td class="kolom-akhir-tbody"><?=$kapal_jenis->getField("RATIO")?> %</td>
                            <?
                            $eis = new EIS();
                            $eis->selectByParamsPemenuhanAwakKapal(array("KAPAL_JENIS_ID" => $kapal_jenis->getField("KAPAL_JENIS_ID")), -1, -1, $reqPeriode);
                            while($eis->nextRow())
                            {
                            ?>
                                <tr id="node-<?=$eis->getField("KAPAL_ID")?>" class="child-of-node-KJ<?=$kapal_jenis->getField("KAPAL_JENIS_ID")?>">
                                    <td><span class='file'><?=$eis->getField("NAMA")?></span></td>
                                    <td align="left"><?=$eis->getField("RATIO")?> %</td>
                                </tr>
                            <?
                            }
                            ?>
