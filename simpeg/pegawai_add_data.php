...
$pegawai = new Pegawai();
$homebase = new Pegawai();
$homebase->selectHomeBase(array());
$agama = new Agama(); 
$bank = new Bank();
...
else
{
	...
	$tempTanggalMasuk = dateToPageCheck($pegawai->getField('TANGGAL_MASUK'));
	...
	$tempKtpNo = $pegawai->getField('KTP_NO');
	$tempTMTNONAKTIF = dateToPageCheck($pegawai->getField('TGL_NON_AKTIF'));

	$tempTMTMPP= dateToPageCheck($pegawai->getField('TANGGAL_MPP'));
	$tempNoSKMPP= $pegawai->getField('NO_MPP');
	
	$tempTinggi= $pegawai->getField('TINGGI');
	...
	<script type="text/javascript">
		...
		function setValue(sts_pegawai){
			...
			else if(sts_pegawai == 5)
			{
				$('#labelTMTMPP').show();
				$('#labelNoSKMPP').show();
			} else if (sts_pegawai == 6)
			{
				$('#labelTMTNONAKTIF').show();
			}
		}
    ...
    <table>
        ...
        <tr>    
            ...
            <td colspan="3">
                ...
                <label id="labelTMTNONAKTIF">
                    &nbsp;&nbsp;&nbsp;Tgl Non Aktif
                    &nbsp;&nbsp;&nbsp;
                    <input id="reqTMTNONAKTIF" name="reqTMTNONAKTIF" class="easyui-datebox" data-options="validType:'date'" value="<?=$tempTMTNONAKTIF?>"></input>
                </label>
            </td>
        </tr>        
        <tr id="labelNoSKMPP">
        ...
        <tr>
		 ...
		 <tr>
            <td>Tgl Masuk</td>
            <td colspan="3">
                <input id="reqTanggalMasuk" name="reqTanggalMasuk" class="easyui-datebox" data-options="validType:'date'" required value="<?=$tempTanggalMasuk?>"></input>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td colspan="3">
                <input name="reqTinggi" id="reqTinggi" class="easyui-validatebox" size="10" type="text" value="<?=$tempTinggi?>" /> cm / 
                <input name="reqBeratBadan" id="reqBeratBadan" class="easyui-validatebox" size="10" type="text" value="<?=$tempBeratBadan?>" /> kg
            </td>
        </tr>
        <tr>
        	<td>Alamat</td>
        ...
        <tr>
			<td>Telepon</td>
			<td colspan="3"><input type="text" class="easyui-validatebox" size="30" maxlength="50" name="reqTelepon" id="reqTelepon" value="<?=$tempTelepon?>" />
				&nbsp;&nbsp;&nbsp;Email
        ...
        <tr>
            ...
            <td>
                 <input id="dd" name="reqTanggalNpwp" class="easyui-datebox" data-options="validType:'date'" value="<?=$tempTanggalNpwp?>"></input>&nbsp;&nbsp;<a href="#" id="clueTipBox" class="clueTipBox" title="Panduan|Tanggal daftar NPWP yang dientri adalah tanggal NPWP yang didaftarkan pegawai ke Perusahaan."><img src="../WEB-INF/images/help.png"></a>
            </td>
        ...
        <tr>
            <td>No KTP</td>
            <td colspan="3">
                 <input name="reqKtpNo" class="easyui-validatebox" size="20" type="text" value="<?=$tempKtpNo?>" /> 
            </td>
        </tr> 
        <tr>
        	<td>Home Base Penerimaan</td>
                <td colspan="3">
                	<select id="reqHomeBase" name="reqHomeBase">
                    <option value="">- Pilih -</option>
					<? while($homebase->nextRow()){?>
                        <option value="<?=$homebase->getField('LOKASI_ID')?>" <? if($tempHomeBase== $homebase->getField('LOKASI_ID')) echo 'selected';?>><?=$homebase->getField('KETERANGAN')?></option>
                    <? }?>
                    </select>  
                </td>
        </tr>
...
<script>
$("#reqBulan,#reqNRP").keypress(function(e) {
