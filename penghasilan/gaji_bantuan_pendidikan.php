...
$lock_proses = $proses_gaji_lock->getProsesGajiLock(array("PERIODE" => $reqPeriode, "JENIS_PROSES" => "GAJI_BANTUAN_PENDIDIKAN"));


$jenis_pegawai->selectByParams();

$akses_app_penghasilan->selectByParamsAksesMenu(array("LINK_FILE" => "gaji_bantuan_pendidikan.php", "AKSES_APP_PENGHASILAN_ID" => $userLogin->userAksesPenghasilan));
...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-gaji/gaji_bantuan_pendidikan_json.php?reqPeriode=<?=$reqPeriode?>",
			  ...
			  $("#reqJenisPegawai").change(function() { 
			  		oTable.fnReloadAjax("../json-gaji/gaji_bantuan_pendidikan_json.php?reqPeriode=" + $("#reqPeriode").val() + "&reqJenisPegawai="+ $("#reqJenisPegawai").val());
			  });
			  ...
			  $('#btnEkspor').on('click', function () {
				  ...
				  newWindow = window.open('gaji_bantuan_pendidikan_excel.php?reqPeriode='+ $("#reqPeriode").val() + '&reqJenisPegawai=2', 'Cetak');
				  newWindow.focus();
				  
			  });		
			  
			  $('#btnDaftarPengantarBankExcel').on('click', function () {
			  	newWindow = window.open('gaji_bantuan_pendidikan_pengantar_bank_excel.php?reqPeriode='+ $("#reqPeriode").val() + '&reqJenisPegawai=2', 'Cetak');
				newWindow.focus();
			  });
			  
			  $('#btnCetakPerbantuan').on('click', function () {
			  	newWindow = window.open('gaji_bantuan_pendidikan_rpt.php?reqJenisPegawaiId=2&reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });			  

			  $('#btnCetakOrganik').on('click', function () {
			  	newWindow = window.open('gaji_bantuan_pendidikan_rpt.php?reqJenisPegawaiId=1&reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });			  

			  $('#btnCetakCapeg').on('click', function () {
			  	newWindow = window.open('gaji_bantuan_pendidikan_rpt.php?reqJenisPegawaiId=5&reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });			  

			  $('#btnCetakPKWT').on('click', function () {
			  	newWindow = window.open('gaji_bantuan_pendidikan_rpt.php?reqJenisPegawaiId=3&reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });			  

			  $('#btnRekapitulasi').on('click', function () {
			  	//newWindow = window.open('gaji_bantuan_pendidikan_rekap_rpt.php?reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				//newWindow.focus();
				window.parent.OpenDHTML('report_penanda_tangan.php?reqJenisReport=REKAPITULASI_BANTUAN_PENDIDIKAN&reqPeriode='+ $("#reqPeriode").val(), 'Office Management - Aplikasi Penghasilan', '600', '300');
			  });			  
			  
			  $('#btnCetakDaftarPengantar').on('click', function () {
			  	newWindow = window.open('gaji_bantuan_pendidikan_pengantar_rpt.php?reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });			  
			  ...
			  $('#btnProses').on('click', function () { 
			  		oTable.fnReloadAjax("../json-gaji/gaji_bantuan_pendidikan_json.php?reqMode=proses&reqPeriode=" + $("#reqPeriode").val() + "&reqJenisPegawai="+ $("#reqJenisPegawai").val());
			  });

			  $('#btnKirimJurnal').on('click', function () {
			  	  if(confirm('Kirim jurnal dan kunci proses bulan terpilih?'))
				  {
					$.getJSON("../json-gaji/proses_gaji_set_lock.php?reqJenisProses=GAJI_BANTUAN_PENDIDIKAN&reqPeriode="+ $("#reqPeriode").val(),
					function(data){
					});	
				  }
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Bantuan Pendidikan</span>
            </div>            
            </td>
        </tr>
    </table>
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
		...
		<ul id="flexmenu2" class="flexdropdownmenu">
          <li><a href="#" id="btnEkspor">Export Data Bantuan Pendidikan</a></li>
    ...
    <thead>
        <tr>
            ...
            <th width="85px">Jumlah Dibayar</th>
        </tr>
    </thead>
    </table>
    </div>    
