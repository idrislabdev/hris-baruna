...
<script type="text/javascript">
   ...
   $.fn.combotree.defaults = {
			...
			if(newValue == '<?=$reqDepartemen?>')
			{
				...
				document.location.href = 'gaji_pkwt.php?reqDepartemen=' + newValue + '&reqPeriode='+ $("#reqPeriode").val(); 
			}
	};
</script>
...
<script type="text/javascript" charset="utf-8">
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-gaji/gaji_tengah_pkwt_json.php?reqPeriode=<?=$reqPeriode?>&reqDepartemen=<?=$reqDepartemen?>",						  
			  "sScrollY": ($(window).height() - <?=$tinggi?>),
			  "sScrollX": "100%",								  
			  "sScrollXInner": "100%",
			  "sPaginationType": "full_numbers"
			  }).makeEditable({
			pReloadTarget: "../json-gaji/gaji_pkwt_json.php?reqMode=proses&reqDepartemen=<?=$reqDepartemen?>",
			...
			/* Click event handler */
			  ...
			  $("#reqPeriode").change(function() { 
				 if($("#reqPeriode").val() == '<?=$arrPeriode[0]?>')
			  		...
			  		oTable.fnReloadAjax("../json-gaji/gaji_pkwt_json.php?reqDepartemen=<?=$reqDepartemen?>&reqPeriode="+ $("#reqPeriode").val());
