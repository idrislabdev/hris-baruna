...
$fname = tempnam("/tmp", "kalkukasi_transport.xls");
...
$potongan_kondisi->selectByParams(array("STATUS_IMPORT" => "1"));
...
for ($i=0; $i < 2; $i++)
{
	...
	if ($i==0)
	{
		...
		$worksheet->write(0, 3, "JUMLAH HADIR", $text_format_line_bold);
		$worksheet->write(0, 4, "TARIF", $text_format_line_bold);


		$row = 1;
		while ($kalkulasi_transport->nextRow()) 
		{
			$worksheet->write($row, 0, $kalkulasi_transport->getField("NRP"), $text_format_line_left);
			$worksheet->write($row, 1, $kalkulasi_transport->getField("NAMA_PEGAWAI"), $text_format_line_left);
			$worksheet->write($row, 2, $kalkulasi_transport->getField("JABATAN"), $text_format_line_left);
			$worksheet->write($row, 3, $kalkulasi_transport->getField("JUMLAH_HADIR"),  $text_format_line_left);
			$worksheet->write($row, 4, $kalkulasi_transport->getField("TARIF"),  $text_format_line_left);
...
header("Content-Type: application/x-msexcel; name=\"kalkulasi_transport_pegawai.xls\"");
header("Content-Disposition: inline; filename=\"kalkulasi_transport_pegawai.xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
?>