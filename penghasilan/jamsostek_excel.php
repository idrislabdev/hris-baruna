...
$worksheet->set_column(12, 12, 23.14);
$worksheet->set_column(13, 13, 18.14);
$worksheet->set_column(14, 14, 12.14);
$worksheet->set_column(15, 15, 12.12);
$worksheet->set_column(16, 16, 12.43);
$worksheet->set_column(17, 17, 25.43);

$worksheet->set_column(20, 20, 3.71);
$worksheet->set_column(21, 21, 35.00);
...
$worksheet->write_blank(1, 17, $text_format_merge);
$worksheet->write_blank(1, 18, $text_format_merge);
...
$worksheet->write_blank(2, 17, $text_format_merge);
$worksheet->write_blank(2, 18, $text_format_merge);
...
$worksheet->write(9, 5, "DEPARTEMEN", $text_format_line_bold);
$worksheet->write(9, 6, "STATUS KAWIN", $text_format_line_bold);
$worksheet->write(9, 7, "TGL LAHIR", $text_format_line_bold);
$worksheet->write(9, 8, "PERIODE KEPESERTAAN", $text_format_line_bold);
$worksheet->write(9, 9, "UPAH TK", $text_format_line_bold);
$worksheet->write(9, 10, "UPAH BERSIH", $text_format_line_bold);
$worksheet->write(9, 11, "Revisi UPAH BERSIH", $text_format_line_bold);
$worksheet->write(9, 12, "JHT PERUSAHAAN (3,70%)", $text_format_line_bold);
$worksheet->write(9, 13, "JHT PEGAWAI (2,00%)", $text_format_line_bold);
$worksheet->write(9, 14, "JKM (0,30%)", $text_format_line_bold);
$worksheet->write(9, 15, "JKK (1,74%)", $text_format_line_bold);
$worksheet->write(9, 16, "IURAN", $text_format_line_bold);
$worksheet->write(9, 17, "KETERANGAN", $text_format_line_bold);
...
$worksheet->write(9, 26, "IURAN", $text_format_line_bold);
...
while($gaji_awal_bulan->nextRow())
{
	...
	if($i == 1)	{
		$arrDepartemen[$index-1]["JTH"] = $temp_jumlah_jht_departemen; //echo ($index-1) . ' ' .  $temp_jumlah_iuran_departemen . '<br>';
		...
		$index++;
		$temp_jumlah_jht_departemen = 0;
		$temp_jumlah_jht_peg_departemen = 0;
		$temp_jumlah_jkm_departemen =0;
		$temp_jumlah_jkk_departemen =0;
		$temp_jumlah_jpk_departemen =0;
		$temp_jumlah_iuran_departemen = 0;
	}
	...
	/*
	$upahBersih = 0;
	$upahBersih = $gaji_awal_bulan->getField("UPAH_TK") * 0.75;
	*/
	...
	$worksheet->write($row, 5, $gaji_awal_bulan->getField("DEPARTEMEN"). $namaDept, $text_format_line_left);
	$worksheet->write($row, 6, $gaji_awal_bulan->getField("KAWIN"), $text_format_line);
	$worksheet->write($row, 7, $gaji_awal_bulan->getField("TGL_LAHIR"), $text_format_line);
	$worksheet->write($row, 8, $gaji_awal_bulan->getField("PERIODE"), $text_format_line);
	$worksheet->write($row, 9, $gaji_awal_bulan->getField("UPAH_TK"), $uang_line);
	$worksheet->write($row, 10, $gaji_awal_bulan->getField("RAPEL_UPAH_TK"), $uang_line);
	$worksheet->write($row, 11, $gaji_awal_bulan->getField("UPAH_REVISI"), $uang_line);
	$worksheet->write($row, 12, $gaji_awal_bulan->getField("JHT"), $uang_line);
	$worksheet->write($row, 13, $gaji_awal_bulan->getField("JHT_PEG"), $uang_line);
	$worksheet->write($row, 14, $gaji_awal_bulan->getField("JKM"), $uang_line);
	$worksheet->write($row, 15, $gaji_awal_bulan->getField("JKK"), $uang_line);
	$worksheet->write($row, 16, $gaji_awal_bulan->getField("IURAN"), $uang_line);
	$worksheet->write($row, 17, $gaji_awal_bulan->getField("KETERANGAN"), $text_format_line_left);
	
	$temp_jumlah_jht += $gaji_awal_bulan->getField('JHT');			$temp_jumlah_jht_departemen += $gaji_awal_bulan->getField('JHT');
	$temp_jumlah_jht_peg += $gaji_awal_bulan->getField('JHT_PEG');	$temp_jumlah_jht_peg_departemen += $gaji_awal_bulan->getField('JHT_PEG');
	$temp_jumlah_jkm += $gaji_awal_bulan->getField('JKM');			$temp_jumlah_jkm_departemen += $gaji_awal_bulan->getField('JKM');	
	$temp_jumlah_jkk += $gaji_awal_bulan->getField('JKK');			$temp_jumlah_jkk_departemen += $gaji_awal_bulan->getField('JKK');	
	$temp_jumlah_iuran += $gaji_awal_bulan->getField('IURAN');		$temp_jumlah_iuran_departemen += $gaji_awal_bulan->getField('IURAN');	
	$row++;
	$no++;
	$i++;
...
$worksheet->write($row, 11, "Jumlah", $text_format_line);
$worksheet->write($row, 12, $temp_jumlah_jht, $uang_line);
$worksheet->write($row, 13, $temp_jumlah_jht_peg, $uang_line);
$worksheet->write($row, 14, $temp_jumlah_jkm, $uang_line);
$worksheet->write($row, 15, $temp_jumlah_jkk, $uang_line);
$worksheet->write($row, 16, $temp_jumlah_iuran, $uang_line);

$row++;
$worksheet->write($row, 11, "Total Iuran Pegawai", $text_format_line);
...
$worksheet->write($row, 16, $temp_jumlah_jht+$temp_jumlah_jkm+$temp_jumlah_jkk+$temp_jumlah_jpk, $uang_line);

$row++;
$worksheet->write($row, 11, "Total Iuran Perusahaan", $text_format_line);
$worksheet->write_blank($row, 12, $text_format_line);
$worksheet->write_blank($row, 13, $text_format_line);
$worksheet->write_blank($row, 14, $text_format_line);
$worksheet->write_blank($row, 15, $text_format_line);
$worksheet->write($row, 16, $temp_jumlah_jht_peg, $uang_line);

$row++;
$worksheet->write($row, 11, "Total", $text_format_line);
$worksheet->write_blank($row, 12, $text_format_line);
$worksheet->write_blank($row, 13, $text_format_line);
$worksheet->write_blank($row, 14, $text_format_line);
$worksheet->write_blank($row, 15, $text_format_line);
$worksheet->write($row, 16, $temp_jumlah_jht+$temp_jumlah_jkm+$temp_jumlah_jkk+$temp_jumlah_jpk+$temp_jumlah_jht_peg, $uang_line);

$row = 10;
$i= $no = 1;

//echo print_r($arrDepartemen);EXIT;
$temp_jumlah_jht = 0;
$temp_jumlah_jht_peg = 0;
$temp_jumlah_jkm = 0;
$temp_jumlah_jkk = 0;
$temp_jumlah_iuran = 0;
if($index == 0){}
else
{
	for($i=0;$i<count($arrDepartemen);$i++)
	{
		...
		else
		{
			...
			$worksheet->write($row, 26, $arrDepartemen[$i]["IURAN"], $uang_line);
...
$worksheet->write($row, 26, $temp_jumlah_iuran, $uang_line);
