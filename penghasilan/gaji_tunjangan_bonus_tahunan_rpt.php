...
//------  Variables ------
$my_report	= $path."penghasilan\\report\\BONUS_TAHUNAN.rpt";
$my_pdf		= $path."penghasilan\\report\\BONUS_TAHUNAN.pdf";
...
if(count($arrJenisPegawaiId) > 1)
{
	...
	for($i=0;$i<count($arrJenisPegawaiId);$i++)
	{
		...
		$statement .= "{THR_REPORT.JENIS_PEGAWAI_ID}=".$arrJenisPegawaiId[$i];
	}
	
	$statement .= " ) ";
}
else
	$statement = " AND {THR_REPORT.JENIS_PEGAWAI_ID}= ".$reqJenisPegawaiId;

$creport->RecordSelectionFormula="{THR_REPORT.PERIODE}='".$reqPeriode."'".$statement;
//else
