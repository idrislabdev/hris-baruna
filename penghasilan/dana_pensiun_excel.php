...
$fname = tempnam("/tmp", "dana_pensiun_excel.xls");
...
$gaji_awal_bulan->selectByParamsDaftarDanaPensiun(array("PERIODE" => $reqPeriode), -1, -1);
...
$worksheet->write(1, 1, "DAFTAR PEMBAYARAN DANA PENSIUN ".$caption, $text_format);
...
while($gaji_awal_bulan->nextRow())
{
	...
	$worksheet->write($row, 4, $gaji_awal_bulan->getField('DANA_PENSIUN'),$uang_line);
	$row++;
	$i++;
}
...
header("Content-Type: application/x-msexcel; name=\"dana_pensiun_excel.xls\"");
header("Content-Disposition: inline; filename=\"dana_pensiun_excel.xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
?>