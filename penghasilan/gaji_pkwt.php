...
$akses_app_penghasilan->selectByParamsAksesMenu(array("LINK_FILE" => "gaji_pkwt.php", "AKSES_APP_PENGHASILAN_ID" => $userLogin->userAksesPenghasilan));
...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-gaji/gaji_pkwt_json.php?reqPeriode=<?=$reqPeriode?>&reqDepartemen=<?=$reqDepartemen?>",						  
			  ...
			  $("#reqPeriode").change(function() { 
				 if($("#reqPeriode").val() == '<?=$arrPeriode[0]?>' && '<?=$lock_proses?>' == '0')
				 	 ...
				 	 $('.toggle').css({"display":"none"}); 
			  		oTable.fnReloadAjax("../json-gaji/gaji_pkwt_json.php?reqDepartemen="+$('#cc').combotree('getValue')+"&reqPeriode="+ $("#reqPeriode").val());
				 	 
			  });
			  
			  $('#cc').combotree({
					onSelect: function(param){
				  		oTable.fnReloadAjax("../json-gaji/gaji_pkwt_json.php?reqDepartemen="+param.id+"&reqPeriode="+ $("#reqPeriode").val());
					}
			  });
			  ...
			  $('#btnCetakBukuBesar').on('click', function () {
				if(<?=$lock_cetak?> == 1)
				{
					window.parent.OpenDHTML('report_penanda_tangan.php?reqJenisReport=BESAR&reqJenisPegawaiId=3&reqPeriode='+ $("#reqPeriode").val(), 'Office Management - Aplikasi Penghasilan', '600', '300');					
				}
				else
				{
			  	...
			  	//newWindow = window.open('gaji_perbantuan_buku_besar.php?reqJenisPegawaiId=3&reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				//newWindow.focus();
			  });

			  $('#btnEkspor').on('click', function () {
			  	newWindow = window.open('gaji_perbantuan_cetak.php?reqJenisPegawaiId=3&reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });
			  
			  $('#btnJamsostekExcel').on('click', function () {
			  	newWindow = window.open('jamsostek_excel.php?reqPeriode='+ $("#reqPeriode").val() + '&reqDepartemen='+$('#cc').combotree('getValue'), 'Cetak');
				newWindow.focus();
			  });
			  
			  $('#btnBPJSExcel').on('click', function () {
			  	newWindow = window.open('bpjs_excel.php?reqJenisPegawaiId=3&reqPeriode='+ $("#reqPeriode").val() + '&reqDepartemen='+$('#cc').combotree('getValue'), 'Cetak');
				newWindow.focus();
			  });
			  
			  $('#btnPotonganExcel').on('click', function () {
			  	newWindow = window.open('potongan_excel.php?reqJenisPegawaiId=3&reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });	
			  
			  $('#btnPotonganLainExcel').on('click', function () {
			  	newWindow = window.open('potongan_lain_excel.php?reqJenisPegawaiId=3&reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });
			  
			  $('#btnTaspenExcel').on('click', function () {
			  	newWindow = window.open('taspen_excel.php?reqPeriode=<?=$reqPeriode?>&reqDepartemen='+$('#cc').combotree('getValue'), 'Cetak');
				newWindow.focus();
			  });		

			  $('#btnCetakPotonganLain').on('click', function () {
			  	newWindow = window.open('potongan_lain_cetak_rpt.php?reqJenisPegawaiId=3&reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });
			  
			  $('#btnCetakPotongan').on('click', function () {
			  	newWindow = window.open('potongan_cetak_rpt.php?reqJenisPegawaiId=3&reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });

			  $('#btnDaftarPengantar').on('click', function () {
			  	newWindow = window.open('daftar_pengantar_cetak_rpt.php?reqJenisPegawaiId=3&reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });
			  ...
			  $('#btnCetakSemua').on('click', function () {
				if(<?=$lock_cetak?> == 1)
				{
					newWindow = window.open('gaji_perbantuan_slip.php?reqJenisPegawaiId=3&reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
					newWindow.focus();
				}
				else
				{
			  ...
			  $('#btnDaftarPengantarBankExcel').on('click', function () {
			  	newWindow = window.open('daftar_pengantar_bank_excel.php?reqJenisPegawaiId=3&reqPeriode=<?=$reqPeriode?>', 'Cetak');
				newWindow.focus();
			  });		
			  ...
			  $('#btnCetak').on('click', function () {
				  if(anSelectedData == "")
					{  
					  newWindow = window.open('gaji_pkwt_cetak.php?reqPeriode=<?=$reqPeriode?>&reqDepartemen='+$('#cc').combotree('getValue')+'&reqJenisPegawaiId=3', 'Cetak');
					  newWindow.focus();
					}
			  });			  

			  $('#btnJamsostekExcel').on('click', function () {
			  	newWindow = window.open('jamsostek_excel.php?reqJenisPegawaiId=3&reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });

			  $('#btnJamsostek').on('click', function () {
			  	//newWindow = window.open('daftar_jamsostek_rpt.php?reqJenisPegawaiId=3&reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				//newWindow.focus();
				window.parent.OpenDHTML('report_penanda_tangan.php?reqJenisReport=DAFTAR_IURAN_JAMSOSTEK&reqJenisPegawaiId=3&reqPeriode='+ $("#reqPeriode").val(), 'Office Management - Aplikasi Penghasilan', '600', '300');				
			  });			
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Data PKWT</span>
            </div>            
            </td>
        </tr>
    </table>
