...
$fname = tempnam("/tmp", "gaji_mobilitas_excel.xls");
...
$mobilitas_bantuan->selectByParams(array(), -1, -1, $statement, $reqPeriode, " ORDER BY E.NAMA ASC");

$worksheet->set_column(0, 0, 8.43);
$worksheet->set_column(1, 1, 10.00);
$worksheet->set_column(2, 2, 27.00);
$worksheet->set_column(3, 3, 27.00);
$worksheet->set_column(4, 4, 7.00);
$worksheet->set_column(5, 5, 9.43);
...
$worksheet->write(1, 1, "DAFTAR MOBILITAS", $text_format);
...
$worksheet->write(4, 3, "NO REKENING", $text_format_line_bold);
$worksheet->write(4, 4, "JUMLAH", $text_format_line_bold);
$worksheet->write(4, 5, "% PPH", $text_format_line_bold);
$worksheet->write(4, 6, "JUMLAH PPH", $text_format_line_bold);
...
{
	if($jenis_bank == $mobilitas_bantuan->getField("NAMA_BANK"))
	{}
	else
	{
		$worksheet->write($row, 1, $mobilitas_bantuan->getField("NAMA_BANK"), $text_format_merge_line_bold);
	...
	$worksheet->write($row, 1, $mobilitas_bantuan->getField("NRP"), $text_format_line);
	$worksheet->write($row, 2, $mobilitas_bantuan->getField("NAMA"), $text_format_line_left);
	$worksheet->write($row, 3,  " ". $mobilitas_bantuan->getField("REKENING_NO"), $text_format_line_left);
	$worksheet->write($row, 4, $mobilitas_bantuan->getField("JUMLAH"), $uang_line);
	$worksheet->write($row, 5, $mobilitas_bantuan->getField("PROSENTASE_POTONGAN"), $text_format_line);
	$worksheet->write($row, 6, $mobilitas_bantuan->getField("JUMLAH_POTONGAN"), $uang_line);
	$worksheet->write($row, 7, $mobilitas_bantuan->getField("DIBAYARKAN"), $uang_line);
	/*for ($k=4; $k<=34; $k++)
	{
			$worksheet->write($row, $k, $absensi_koreksi->getField('HARI_'.($k-3)), $text_format_line);
	}*/
	$jenis_bank = $mobilitas_bantuan->getField("NAMA_BANK");
	$row++;
}

$workbook->close();

header("Content-Type: application/x-msexcel; name=\"gaji_mobilitas_excel.xls\"");
header("Content-Disposition: inline; filename=\"gaji_mobilitas_excel.xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
?>