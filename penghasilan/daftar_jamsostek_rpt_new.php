...
//------  Variables ------
$my_report	= $path."penghasilan\\report\\DAFTAR_IURAN_JAMSOSTEK_NEW.rpt";
$my_pdf 	= $path."penghasilan\\report\\DAFTAR_IURAN_JAMSOSTEK_NEW.pdf";
...
if(count($arrJenisPegawaiId) > 1)
{
	...
	for($i=0;$i<count($arrJenisPegawaiId);$i++)
	{
		...
		$statement .= "{DAFTAR_IURAN_JAMSOSTEK.JENIS_PEGAWAI_ID}=".$arrJenisPegawaiId[$i];
	}
	
	$statement .= " ) ";
}
else
	$statement = " AND {DAFTAR_IURAN_JAMSOSTEK.JENIS_PEGAWAI_ID}= ".$reqJenisPegawaiId;

$creport->RecordSelectionFormula="{DAFTAR_IURAN_JAMSOSTEK.BULANTAHUN}='".$reqPeriode."'".$statement;
//else
