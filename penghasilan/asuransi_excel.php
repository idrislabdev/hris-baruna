...
$fname = tempnam("/tmp", "asuransi_excel.xls");
...
$gaji_awal_bulan->selectByParamsAsuransiReport(array(), -1, -1, $statement, $reqPeriode, " ORDER BY POTONGAN, NAMA ASC");
...
$worksheet->write(1, 1, "DAFTAR POTONGAN UNIT LINK PEGAWAI", $text_format);
...
$worksheet->write(4, 4, "JUMLAH (Rp.)", $text_format_line_bold);
...
while($gaji_awal_bulan->nextRow())
{
	...
	if($bank == $gaji_awal_bulan->getField('POTONGAN'))
	{}
	else		
	{
		if($jumlah == 0)
		...
		else
		{
			...
			$worksheet->write($row, 3, "TOTAL",$text_format_line_bold);
			$worksheet->write($row, 4, $jumlah,$uang_line_bold);
			$worksheet->write($row, 5, "",$text_format_line);
			$row++;	
			$jumlah = 0;
		}
		
		$worksheet->write($row, 1, strtoupper($gaji_awal_bulan->getField('POTONGAN')),$text_format_merge_line_bold);
	...
	$worksheet->write($row, 4, $gaji_awal_bulan->getField('JUMLAH'),$uang_line);
	$worksheet->write($row, 5, $gaji_awal_bulan->getField('KETERANGAN'),$text_format_line_left);
	$row++;
	$i++;
	$jumlah += $gaji_awal_bulan->getField("JUMLAH");
	$bank = $gaji_awal_bulan->getField('POTONGAN');
...
header("Content-Type: application/x-msexcel; name=\"asuransi_excel.xls\"");
header("Content-Disposition: inline; filename=\"asuransi_excel.xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
?>