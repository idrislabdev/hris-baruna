...
$fname = tempnam("/tmp", "gaji_insentif_rekap_excel.xls");
...
$insentif_bantuan->selectByParamsReportRekap(array(), -1, -1, "", $reqPeriode, " ORDER BY URUT");
...
$worksheet->write		(1, 1, "REKAPITULASI INSENTIF PARA MANAGER, KABID, ASMEN, DAN KADIV SERTA TRANSPORTASI PEGAWAI DARAT, AWAK KAPAL, ORGANIK PMS, PTTPK DAN PKWT", $text_format_merge);
...
{
	if($kelompok == $insentif_bantuan->getField("KELOMPOK"))
	{}
	else
	{
		$worksheet->write($row, 0, $insentif_bantuan->getField("URUT"), $text_format_line);
		$worksheet->write($row, 1, $insentif_bantuan->getField("KELOMPOK"), $text_format_left_only);
	...
	$worksheet->write($row, 1, $insentif_bantuan->getField("DEPARTEMEN_ID"), $text_format_line_left);
	$worksheet->write($row, 2, $insentif_bantuan->getField("JUMLAH"), $uang_line);
	$worksheet->write($row, 3, $insentif_bantuan->getField("POTONGAN"), $uang_line);
	$worksheet->write($row, 4, $insentif_bantuan->getField("TOTAL"), $uang_line);
	$worksheet->write($row, 5, $insentif_bantuan->getField("POTONGAN_PPH"), $uang_line);
	$worksheet->write($row, 6, $insentif_bantuan->getField("DIBAYARKAN"), $uang_line);
	
	$jumlah += $insentif_bantuan->getField("JUMLAH");
	$total += $insentif_bantuan->getField("TOTAL");
	$jumlah_potongan += $insentif_bantuan->getField("POTONGAN");
	$jumlah_pph += $insentif_bantuan->getField("POTONGAN_PPH");
	$dibayarkan	+= $insentif_bantuan->getField("DIBAYARKAN");
	$no++;
	$row++;
	$kelompok = $insentif_bantuan->getField("KELOMPOK");
}

$worksheet->write($row, 0, "", $text_format_line);
$worksheet->write($row, 1, "JUMLAH", $text_format_line_left);
$worksheet->write($row, 2, $jumlah, $uang_line);
$worksheet->write($row, 3, $jumlah_potongan, $uang_line);
...
header("Content-Type: application/x-msexcel; name=\"gaji_insentif_rekap_excel.xls\"");
header("Content-Disposition: inline; filename=\"gaji_insentif_rekap_excel.xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
?>