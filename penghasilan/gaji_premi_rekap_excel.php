...
$fname = tempnam("/tmp", "gaji_premi_rekap.xls");
...
$premi->selectByParamsReportRekap(array(), -1, -1, "", $reqPeriode, " ORDER BY NAMA_KAPAL, PERIODE");
...
$worksheet->set_column(4, 4, 21.00);
$worksheet->set_column(5, 5, 21.00);
...
$worksheet->write		(1, 1, "REKAPITULASI PREMI AWAK KAPAL", $text_format_merge);
...
$worksheet->write(2, 1, "TAHUN : ".strtoupper($reqPeriode), $text_format_merge);
else
...
$worksheet->write(6, 3, "JUMLAH (Rp.)", $text_format_line_bold);
$worksheet->write(6, 4, "POTONGAN PPH (Rp.)", $text_format_line_bold);
$worksheet->write(6, 5, "JUMLAH YANG DIBAYAR (Rp.)", $text_format_line_bold);
...
while($premi->nextRow())
{
	...
	$worksheet->write($row, 1, $premi->getField("NAMA_KAPAL"), $text_format_line_left);
	$worksheet->write($row, 2, strtoupper(getNamePeriode($premi->getField("PERIODE"))), $text_format_line_left);
	$worksheet->write($row, 3, $premi->getField("JUMLAH"), $uang_line);
	$worksheet->write($row, 4, $premi->getField("POTONGAN"), $uang_line);
	$worksheet->write($row, 5, $premi->getField("TOTAL"), $uang_line);
	
	$jumlah += $premi->getField("JUMLAH");
	$total += $premi->getField("TOTAL");
	$jumlah_pph += $premi->getField("POTONGAN");
	$no++;
	$row++;
}
...
$worksheet->write($row, 3, $jumlah, $uang_line);
$worksheet->write($row, 4, $jumlah_pph, $uang_line);
$worksheet->write($row, 5, $total, $uang_line);
	

$workbook->close();

header("Content-Type: application/x-msexcel; name=\"gaji_premi_rekap.xls\"");
header("Content-Disposition: inline; filename=\"gaji_premi_rekap.xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
?>