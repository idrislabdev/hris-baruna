...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-gaji/sinkronisasi_pegawai_json.php?reqPeriode=<?=$reqPeriode?>",						  
			...
			/* Click event handler */
			  ...
			  $('#btnEdit').on('click', function () {
				  if(anSelectedData == "")
					  return false;				
				  window.parent.OpenDHTML('merit_pms_add.php?reqMode=update&reqId='+anSelectedId, 'Office Management - Aplikasi Penghasilan', '600', '300');	
			  });
			  ...
			  $('#btnDeleteRow').on('click', function () {
				...
				if ( confirm( "Apakah anda yakin, menghapus data ini ?" ) ) {
					$.getJSON('../json-gaji/delete.php?reqMode=merit_pms&id='+anSelectedId, function (data) 
					{
			  ...
			  $('#btnSinkronisasi').on('click', function () {
					$.messager.confirm('Konfirmasi',"Sinkronisasi data pegawai untuk periode terpilih?",function(r){
						if (r){
							var win = $.messager.progress({
												title:'Sinkronisasi data pegawai',
												msg:'Proses data...'
											});
							var jqxhr = $.get("../json-gaji/sinkronisasi_pegawai_proses_json.php?reqPeriode="+$("#reqPeriode").val(), function() {
								$.messager.progress('close');
							})
							.done(function() {
								$.messager.progress('close');
							oTable.fnReloadAjax("../json-gaji/sinkronisasi_pegawai_json.php?reqPeriode="+$("#reqPeriode").val());
			  ...
			  $('#btnPosting').on('click', function () {
					$.messager.confirm('Konfirmasi',"Posting data pegawai untuk periode terpilih?",function(r){
						if (r){
							var win = $.messager.progress({
												title:'Posting data pegawai',
												msg:'Proses data...'
											});
							var jqxhr = $.get("../json-gaji/sinkronisasi_pegawai_lock_posting_json.php?reqPeriode="+$("#reqPeriode").val(), function() {
								$.messager.progress('close');
							})
							.done(function() {
								$.messager.progress('close');
			  ...
			  $("#reqPeriode, #reqJenisPegawai").change(function() {

					var jqxhr = $.get( "../json-gaji/sinkronisasi_pegawai_tombol_json.php/?reqPeriode="+$('#reqPeriode').val(), function(data){
						//alert(data);return false;
						if(data == "0")
						{
							$("#btnSinkronisasi").show();
							$("#btnPosting").show();									
						}
						else
						{
							$("#btnSinkronisasi").hide();
							$("#btnPosting").hide();
						}						
					});

			  		oTable.fnReloadAjax("../json-gaji/sinkronisasi_pegawai_json.php?reqJenisPegawaiId="+$("#reqJenisPegawai").val()+"&reqDepartemen="+$('#cc').combotree('getValue')+"&reqPeriode="+ $("#reqPeriode").val());
			  });


			  $('#cc').combotree({
					onSelect: function(param){
						 oTable.fnReloadAjax("../json-gaji/sinkronisasi_pegawai_json.php?reqJenisPegawaiId="+$("#reqJenisPegawai").val()+"&reqDepartemen="+param.id+"&reqPeriode="+ $("#reqPeriode").val());
					}
			  });	
		} );
</script>
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Sinkronisasi Pegawai</span>
            </div>            
            </td>
        </tr>
    </table>
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul>
            <li>
            <a href="#" id="btnSinkronisasi" title="Sinkronisasi">Sinkronisasi</a>
        	</li>
        	<li>
            <a href="#" id="btnPosting" title="Posting">Posting</a>
            </li>        
            <li>
            	<a href="#">&nbsp;</a>
            </li>
    ...
    <thead>
        <tr>
            ...
            <th width="30px">Masa Kerja</th>                                            
            <th width="30px">Kelompok</th>                                            
            <th width="30px">Status Calpeg</th>                                            
        </tr>
    </thead>
    </table> 
    </div> 
