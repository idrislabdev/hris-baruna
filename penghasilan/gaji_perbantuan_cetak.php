...
if($reqJenisPegawaiId == '') $reqJenisPegawaiId = 2;
...
$gaji_awal_bulan->selectByParamsGaji(array(), -1, -1, $statement, " ORDER BY C.DEPARTEMEN_ID, C.NO_URUT, C.KELAS ASC", $reqPeriode, $reqJenisPegawaiId);
...
while($gaji_awal_bulan->nextRow())
{
	...
	$lumpsum = $gaji_awal_bulan->getField("LUMPSUM");
	$total_gaji = 0;
	$total_sumbangan = 0;
	$total_potongan = 0;
	$total_tanggungan = 0;
	...
	{	
		...
		if(substr($aColumns[$i],0,4) == "GAJI")
		{
			...
			{
				$hasil = $total_gaji + $lumpsum;
				$worksheet->write($row, $i+1, $hasil, $uang_line);				
			}
			else
			{
				$hasil = $json_gaji->{$column_gaji}{0} + ($i+1==4 ? $lumpsum : 0);
				$worksheet->write($row, $i+1, $hasil, $uang_line);	
