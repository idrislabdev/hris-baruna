...
$csv->addRow(array('P',substr($reqPeriode,-4) . substr($reqPeriode,0,2) . '01','1400015522221',$gaji_awal_bulan2->getField('TOTAL'),$gaji_awal_bulan2->getField('SUM_GAJI'),',,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,'));

}

$monthNum = substr($reqPeriode,0,2);
$monthName = date("M", mktime(0, 0, 0, $monthNum, 10));
$keterangan = strtoupper("GAJI " . $monthName . " " . substr($reqPeriode,-4));
...
{	
	...
	$worksheet->write($row, 4, $gaji_awal_bulan->replaceSpecialCharacter($gaji_awal_bulan->getField('NAMA')),$text_format_line_left);
	$worksheet->write($row, 5, $jumlah_dibayar,$uang_line);
	*/
	$csv->addRow(array($gaji_awal_bulan->getField('REKENING_NO'),$gaji_awal_bulan->replaceSpecialCharacter($gaji_awal_bulan->getField('NAMA')),',,,IDR',$jumlah_dibayar,$keterangan.',,IBU,,,,,,,N,ahmaddal@yahoo.com'));
