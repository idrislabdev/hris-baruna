...
$akses_app_penghasilan->selectByParamsAksesMenu(array("LINK_FILE" => "gaji_cuti_tahunan.php", "AKSES_APP_PENGHASILAN_ID" => $userLogin->userAksesPenghasilan));
...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-gaji/gaji_cuti_tahunan_json.php?reqPeriode=<?=$reqPeriode?>&reqStatusProses=BELUM",
			  ...
			  $("#reqJenisPegawai").change(function() { 
			  		oTable.fnReloadAjax("../json-gaji/gaji_cuti_tahunan_json.php?reqPeriode=" + $("#reqPeriode").val() + "&reqJenisPegawai="+ $("#reqJenisPegawai").val() + "&reqStatusProses="+ $("#reqStatusProses").val());
			  });
			  ...
			  $('#btnEkspor').on('click', function () {
				  ...
				  newWindow = window.open('gaji_cuti_tahunan_excel.php?reqPeriode='+ $("#reqPeriode").val() + '&reqJenisPegawai='+ $("#reqJenisPegawai").val(), 'Cetak');
				  newWindow.focus();
				  
			  });		

			  $('#btnCetakPerbantuan').on('click', function () {
			  	newWindow = window.open('gaji_cuti_tahunan_rpt.php?reqJenisPegawaiId=2&reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });			  

			  $('#btnCetakOrganik').on('click', function () {
			  	newWindow = window.open('gaji_cuti_tahunan_rpt.php?reqJenisPegawaiId=1&reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });			  

			  $('#btnCetakCapeg').on('click', function () {
			  	newWindow = window.open('gaji_cuti_tahunan_rpt.php?reqJenisPegawaiId=5&reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });			  

			  $('#btnCetakPKWT').on('click', function () {
			  	newWindow = window.open('gaji_cuti_tahunan_rpt.php?reqJenisPegawaiId=3&reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });			  

			  $('#btnRekapitulasi').on('click', function () {
			  	/*newWindow = window.open('gaji_cuti_tahunan_rekap_rpt.php?reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();*/
			  ...
			  $('#btnCetakAwakKapal').on('click', function () {
			  	newWindow = window.open('gaji_cuti_tahunan_awak_kapal_rpt.php?reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });
			  
			  $('#btnCetakDaftarPengantar').on('click', function () {
			  	newWindow = window.open('gaji_cuti_tahunan_pengantar_rpt.php?reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });			  
			  ...
			  $("#reqPeriode").change(function() { 
				 if($("#reqPeriode").val() == '<?=$arrPeriode[0]?>' && '<?=$lock_proses?>' == '0')
				 	 ...
				 	 $('.toggle').css({"display":"none"}); 
			  		oTable.fnReloadAjax("../json-gaji/gaji_cuti_tahunan_json.php?reqPeriode=" + $("#reqPeriode").val() + "&reqJenisPegawai="+ $("#reqJenisPegawai").val());
				 	 
			  });

			  $('#btnProses').on('click', function () { 
			  		oTable.fnReloadAjax("../json-gaji/gaji_cuti_tahunan_json.php?reqMode=proses&reqPeriode=" + $("#reqPeriode").val() + "&reqJenisPegawai="+ $("#reqJenisPegawai").val() + "&reqStatusProses="+ $("#reqStatusProses").val());
			  });
			  ...
			  $('#btnApproval').on('click', function () {
				  ...
				  //alert(sData);	
				 if(confirm('Kalkulasi ulang daftar pegawai yang belum diproses?'))
				 { 
			  		oTable.fnReloadAjax("../json-gaji/gaji_cuti_tahunan_json.php?reqMode=proses&reqPeriode=" + $("#reqPeriode").val() + "&reqJenisPegawai="+ $("#reqJenisPegawai").val() + "&reqStatusProses="+ $("#reqStatusProses").val());
				 }
				  window.parent.OpenDHTML('proses_gaji_set_lock_cuti_tahunan.php?reqJenisProses=CUTI_TAHUNAN&reqPeriode='+ $("#reqPeriode").val()+'&reqData=' + sData, 'Office Management - Aplikasi Penghasilan', '600', '300');	
			  ...
			  $('#btnKirimJurnal').on('click', function () {
				  ...
				  {
					$.getJSON("../json-gaji/proses_gaji_set_lock.php?reqJenisProses=GAJI_CUTI_TAHUNAN&reqPeriode="+ $("#reqPeriode").val(),
					function(data){
					});	
				  }
			...
			/* Click event handler */
			  ...
			  $('#btnCetak').on('click', function () {
				...
				var sData = $('input', oTable.fnGetNodes()).serialize();
				  sData = sData.replace(/check=/gi, "");		
				  sData = sData.replace(/&/gi, ",");
					window.parent.OpenDHTML('gaji_cuti_tahunan_cetak.php?reqData=' + sData, 'Office Management - Aplikasi Penghasilan', '600', '300');	
...
<div id="wadah">
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul>
            ...
            <li><a href="#" title="Baca" id="btnApproval"  class="toggle">&nbsp;Approval</a></li>
            <li><a href="#" title="Baca" id="btnCetak"  class="toggle">&nbsp;Cetak Ulang</a></li>
            <?
			}
			?>
    ...
    <thead>
        <tr>
            ...
            <th width="85px">Sumbangan PPH21</th>
            <th width="85px">Tgl Approval</th>
        </tr>
    </thead>
    </table>
    </div>    
