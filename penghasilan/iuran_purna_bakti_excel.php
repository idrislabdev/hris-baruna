...
$fname = tempnam("/tmp", "iuran_purna_bakti_excel.xls");
...
$gaji_awal_bulan->selectByParamsDaftarIuranPurnaBakti(array("PERIODE" => $reqPeriode), -1, -1);
...
$worksheet->write(1, 1, "DAFTAR PEMBAYARAN IURAN PURNA BAKTI ".$caption, $text_format);
...
while($gaji_awal_bulan->nextRow())
{
	...
	$worksheet->write($row, 4, $gaji_awal_bulan->getField('IURAN_PURNA_BAKTI'),$uang_line);
	$row++;
	$i++;
}
...
header("Content-Type: application/x-msexcel; name=\"iuran_purna_bakti_excel.xls\"");
header("Content-Disposition: inline; filename=\"iuran_purna_bakti_excel.xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
?>