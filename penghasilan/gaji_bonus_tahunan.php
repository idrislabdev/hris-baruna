...
$lock_proses = $proses_gaji_lock->getProsesGajiLock(array("PERIODE" => $reqPeriode, "JENIS_PROSES" => "GAJI_BONUS_TAHUNAN"));


$jenis_pegawai->selectByParams();

$akses_app_penghasilan->selectByParamsAksesMenu(array("LINK_FILE" => "gaji_bonus_tahunan.php", "AKSES_APP_PENGHASILAN_ID" => $userLogin->userAksesPenghasilan));
...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-gaji/gaji_bonus_tahunan_json.php?reqPeriode=<?=$reqPeriode?>",
			...
			"aoColumns": [
			  ...
			  $("#reqJenisPegawai").change(function() { 
			  		oTable.fnReloadAjax("../json-gaji/gaji_bonus_tahunan_json.php?reqPeriode=" + $("#reqPeriode").val() + "&reqJenisPegawai="+ $("#reqJenisPegawai").val());
			  });
			  ...
			  $('#btnEkspor').on('click', function () {
				  ...
				  newWindow = window.open('gaji_bonus_tahunan_excel.php?reqPeriode='+ $("#reqPeriode").val() + '&reqJenisPegawai='+ $("#reqJenisPegawai").val(), 'Cetak');
				  newWindow.focus();
				  
			  });		

			  $('#btnCetakPerbantuan').on('click', function () {
			  	newWindow = window.open('gaji_bonus_tahunan_rpt.php?reqJenisPegawaiId=2&reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });			  

			  $('#btnCetakOrganik').on('click', function () {
			  	newWindow = window.open('gaji_bonus_tahunan_rpt.php?reqJenisPegawaiId=1&reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });			  

			  $('#btnCetakCapeg').on('click', function () {
			  	newWindow = window.open('gaji_bonus_tahunan_rpt.php?reqJenisPegawaiId=5&reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });			  

			  $('#btnCetakPKWT').on('click', function () {
			  	newWindow = window.open('gaji_bonus_tahunan_rpt.php?reqJenisPegawaiId=3&reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });			  

			  $('#btnRekapitulasi').on('click', function () {
			  	/*newWindow = window.open('gaji_bonus_tahunan_rekap_rpt.php?reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();*/
			  ...
			  $('#btnCetakAwakKapal').on('click', function () {
			  	newWindow = window.open('gaji_bonus_tahunan_awak_kapal_rpt.php?reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });
			  
			  $('#btnCetakDaftarPengantar').on('click', function () {
			  	newWindow = window.open('gaji_bonus_tahunan_pengantar_rpt.php?reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });			  
			  ...
			  $('#btnProses').on('click', function () { 
			  		oTable.fnReloadAjax("../json-gaji/gaji_bonus_tahunan_json.php?reqMode=proses&reqPeriode=" + $("#reqPeriode").val() + "&reqJenisPegawai="+ $("#reqJenisPegawai").val());
			  });

			  $('#btnKirimJurnal').on('click', function () {
			  	  if(confirm('Kirim jurnal dan kunci proses bulan terpilih?'))
				  {
					$.getJSON("../json-gaji/proses_gaji_set_lock.php?reqJenisProses=GAJI_BONUS_TAHUNAN&reqPeriode="+ $("#reqPeriode").val(),
					function(data){
					});	
				  }
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Bonus Tahunan</span>
            </div>            
            </td>
        </tr>
    </table>
