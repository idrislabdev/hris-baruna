...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-gaji/perhitungan_prestasi_json.php?reqPeriode=<?=$reqPeriode?>",						  
			...
			/* Click event handler */
			  ...
			  $('#btnEdit').on('click', function () {
				  if(anSelectedData == "")
					  return false;				
				  window.parent.OpenDHTML('tunjangan_jabatan_add.php?reqMode=update&reqId='+anSelectedId, 'Office Management - Aplikasi Penghasilan', '600', '300');	
			  });
			  
			  $('#btnImport').on('click', function () {
				  window.parent.OpenDHTML("integrasi_tunjangan_prestasi_import.php?reqMode=import&reqDepartemen="+$('#cc').combotree('getValue')+"&reqJenisPegawai="+ $("#reqJenisPegawai").val()+"&reqPeriode="+ $("#reqPeriode").val(), 'Office Management - Aplikasi Penghasilan', '600', '300');	
			  });
			  ...
			  $("#reqPeriode").change(function() { 
				 ...
				 // else
			  		...
			  		oTable.fnReloadAjax("../json-gaji/perhitungan_prestasi_json.php?reqDepartemen="+$('#cc').combotree('getValue')+"&reqJenisPegawai="+ $("#reqJenisPegawai").val()+"&reqPeriode="+ $("#reqPeriode").val());
				 	 
			  });

			  $('#cc').combotree({
					onSelect: function(param){ 
			  			oTable.fnReloadAjax("../json-gaji/perhitungan_prestasi_json.php?reqDepartemen="+param.id+"&reqJenisPegawai="+ $("#reqJenisPegawai").val()+"&reqPeriode="+ $("#reqPeriode").val());
					}
			  });	
			  
			  $("#reqJenisPegawai").change(function() { 
				oTable.fnReloadAjax("../json-gaji/perhitungan_prestasi_json.php?reqDepartemen="+$('#cc').combotree('getValue')+"&reqJenisPegawaiId="+$("#reqJenisPegawai").val()+"&reqPeriode="+ $("#reqPeriode").val());
			  });
			  ...
			  $('#btnKalkulasi').on('click', function () {
					$.messager.confirm('Konfirmasi',"Proses Kalkulasi Prestasi?",function(r){
						if (r){
							var win = $.messager.progress({
												title:'Proses Kalkulasi Prestasi',
												msg:'Proses data...'
											});
							var jqxhr = $.get("../json-gaji/proses_perhitungan_prestasi_json.php?reqPeriode="+$("#reqPeriode").val(), function() {
								$.messager.progress('close');
							})
							.done(function() {
								$.messager.progress('close');
							oTable.fnReloadAjax("../json-gaji/perhitungan_prestasi_json.php?reqPeriode="+$("#reqPeriode").val()+"&reqDepartemen="+$('#cc').combotree('getValue')+"&reqJenisPegawaiId="+$("#reqJenisPegawai").val());
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Data Integrasi Tunjangan Prestasi</span>
            </div>            
            </td>
        </tr>
    </table>
    ...
    <thead>
        <tr>
            ...
            <th width="50px">Min Jam Mengajar</th> 
            <th width="50px">Jumlah Jam Mengajar</th> 
            <th width="50px">Kelebihan Jam Mengajar</th> 
            <th width="50px">Tarif Mengajar</th> 
            <th width="50px">Tarif Lebih</th> 
            <th width="50px">Total Tunjangan</th>
            <th width="30px">Jumlah Potongan</th>          
            <th width="30px">Pembagi Potongan</th>          
            <th width="50px">Tunjangan Prestasi</th>
            <th width="50px">Total Potongan</th>
        </tr>
    </thead>
    </table> 
    </div> 
