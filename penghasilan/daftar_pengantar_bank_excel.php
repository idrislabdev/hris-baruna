...
$worksheet->set_column(5, 5, 33.00);
...
$worksheet->write(4, 4, "AN. REKENING", $text_format_line_bold);
$worksheet->write(4, 5, "NAMA PEGAWAI", $text_format_line_bold);
$worksheet->write(4, 6, "JUMLAH DITERIMA", $text_format_line_bold);
...
{	
	...
	$worksheet->write($row, 3, " ".$gaji_awal_bulan->getField('REKENING_NO'),$text_format_num);
	$worksheet->write($row, 4, $gaji_awal_bulan->getField('REKENING_NAMA'),$text_format_line_left);
	$worksheet->write($row, 5, $gaji_awal_bulan->replaceSpecialCharacter($gaji_awal_bulan->getField('NAMA')),$text_format_line_left);
	$worksheet->write($row, 6, $jumlah_dibayar,$uang_line);
	$row++;
	$i++;
