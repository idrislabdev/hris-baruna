...
//------  Variables ------
$my_report	= $path."penghasilan\\report\\DAFTAR_SELURUH_POTONGAN.rpt";
$my_pdf		= $path."penghasilan\\report\\DAFTAR_SELURUH_POTONGAN.pdf";
...
if(count($arrJenisPegawaiId) > 1)
{
	...
	for($i=0;$i<count($arrJenisPegawaiId);$i++)
	{
		...
		$statement .= "{POTONGAN_LAIN_REPORT.JENIS_PEGAWAI_ID}=".$arrJenisPegawaiId[$i];
	}
	
	$statement .= " ) ";
}
else
	$statement = " AND {POTONGAN_LAIN_REPORT.JENIS_PEGAWAI_ID}= ".$reqJenisPegawaiId;

$creport->RecordSelectionFormula="{POTONGAN_LAIN_REPORT.PERIODE}='".$reqPeriode."'".$statement;
//else
