...
$lock_proses = $proses_gaji_lock->getProsesGajiLock(array("PERIODE" => $reqPeriode, "JENIS_PROSES" => "GAJI_MOBILITAS"));
...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-gaji/gaji_mobilitas_json.php?reqPeriode=<?=$reqPeriode?>",
			...
			/* Click event handler */
			  ...
			  $("#reqJenisPegawai").change(function() { 
			  		oTable.fnReloadAjax("../json-gaji/gaji_mobilitas_json.php?reqPeriode=" + $("#reqPeriode").val() + "&reqJenisPegawai="+ $("#reqJenisPegawai").val());
			  });

			  $('#btnCetakMobilitas').on('click', function () {
				  /*if(anSelectedData == "")
					  return false;	*/			
				  
				  newWindow = window.open('gaji_mobilitas_rpt.php?reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				  newWindow.focus();
				  
			  });			
			  
			  $('#btnCetakDaftarPengantar').on('click', function () {
			  	newWindow = window.open('gaji_mobilitas_pengantar_rpt.php?reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });				    
			  ...
			  $('#btnEkspor').on('click', function () {
				  ...
				  newWindow = window.open('gaji_mobilitas_excel.php?reqPeriode='+ $("#reqPeriode").val() + '&reqJenisPegawai='+ $("#reqJenisPegawai").val(), 'Cetak');
				  newWindow.focus();
			  ...
			  $('#btnProses').on('click', function () { 
			  		oTable.fnReloadAjax("../json-gaji/gaji_mobilitas_json.php?reqMode=proses&reqPeriode=" + $("#reqPeriode").val());
			  });
			  
			  $('#btnKirimJurnal').on('click', function () {
			  	  if(confirm('Kirim jurnal dan kunci proses bulan terpilih?'))
				  {
					$.getJSON("../json-gaji/proses_gaji_set_lock.php?reqJenisProses=GAJI_MOBILITAS&reqPeriode="+ $("#reqPeriode").val(),
					function(data){
					});	
				  }
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Mobilitas</span>
            </div>            
            </td>
        </tr>
    </table>
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul>
            ...
            <li><a href="#" title="Ekspor" id="btnEkspor">&nbsp;Export Excel</a></li>        
            <li><a href="#" title="Baca" id="btnKirimJurnal"  class="toggle">&nbsp;Kirim Jurnal</a></li>          
        </ul>
        <ul id="flexmenu1" class="flexdropdownmenu">
          <li><a href="#" id="btnCetakMobilitas">Mobilitas</a></li>
    ...
    <thead>
        <tr>
            ...
            <th width="85px">Bantuan PPH21</th>
            <th width="85px">Potongan PPH21</th>
