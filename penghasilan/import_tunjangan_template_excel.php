...
$gaji_kondisi_pegawai->selectByParamsImport();
...
for ($i=0; $i < 2; $i++)
{
	...
	if ($i==0)
	{
		...
		$worksheet->write(0, 3, "GAJI KONDISI ID", $text_format_line_bold);
		...
		{
			$worksheet->write($row, 0, $gaji_kondisi_pegawai->getField("PEGAWAI_ID"), $text_format_line_left);
			$worksheet->write($row, 1, $gaji_kondisi_pegawai->getField("NAMA"), $text_format_line_left);
			$worksheet->write($row, 2, $gaji_kondisi_pegawai->getField("NAMA_JABATAN"), $text_format_line_left);
	...
	elseif($i==1)
	{
			...
			$worksheet->write(1, 0, "INTEGRASI TUNJANGAN", $text_format_merge_line_bold);
			$worksheet->write_blank(1, 1, $text_format_merge_line_bold);
			
			$worksheet->write(2, 0, "GAJI KONDISI ID", $text_format_line_bold_top);
...
header("Content-Type: application/x-msexcel; name=\"import_tunjangan_template.xls\"");
header("Content-Disposition: inline; filename=\"import_tunjangan_template.xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
?>