...
$lock_proses = $proses_gaji_lock->getProsesGajiLock(array("PERIODE" => $reqPeriode, "JENIS_PROSES" => "GAJI_UANG_MAKAN"));
$lock_cetak = $proses_presensi_lock->getProsesPresensiLock(array("JENIS_PROSES" => "KOREKSI_ABSEN_KAPAL", "PERIODE" => $reqPeriode));
...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-gaji/gaji_uang_makan_json.php?reqPeriode=<?=$reqPeriode?>",
			...
			/* Click event handler */
			  ...
			  $("#reqJenisPegawai").change(function() { 
			  		oTable.fnReloadAjax("../json-gaji/gaji_uang_makan_json.php?reqPeriode=" + $("#reqPeriode").val() + "&reqJenisPegawai="+ $("#reqJenisPegawai").val());
			  });
			  
			  $('#btnCetakAwakKapal').on('click', function () {
			  	newWindow = window.open('gaji_uang_makan_rpt.php?reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });			
			  
			  $('#btnCetakRekap').on('click', function () {
			  	//newWindow = window.open('gaji_uang_makan_rekap_rpt.php?reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				//newWindow.focus();
				window.parent.OpenDHTML('report_penanda_tangan.php?reqJenisReport=REKAPITULASI_UANG_MAKAN&reqPeriode='+ $("#reqPeriode").val(), 'Office Management - Aplikasi Penghasilan', '600', '300');
			  });			
			    
			  $('#btnCetakPerbantuan').on('click', function () {
			  	newWindow = window.open('gaji_uang_makan_rpt.php?reqJenisPegawaiId=2&reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });			  

			  $('#btnCetakOrganik').on('click', function () {
			  	newWindow = window.open('gaji_uang_makan_rpt.php?reqJenisPegawaiId=1&reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });			  

			  $('#btnCetakCapeg').on('click', function () {
			  	newWindow = window.open('gaji_uang_makan_rpt.php?reqJenisPegawaiId=5&reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });			  

			  $('#btnCetakPKWT').on('click', function () {
			  	newWindow = window.open('gaji_uang_makan_rpt.php?reqJenisPegawaiId=3&reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				newWindow.focus();
			  });	
			  ...
			  $('#btnEkspor').on('click', function () {
				  ...
				  newWindow = window.open('gaji_uang_makan_excel.php?reqPeriode='+ $("#reqPeriode").val() + '&reqJenisPegawai='+ $("#reqJenisPegawai").val(), 'Cetak');
				  newWindow.focus();
			  ...
			  $('#btnKirimJurnal').on('click', function () {
				  ...
				  {
						$.getJSON("../json-gaji/proses_gaji_set_lock.php?reqJenisProses=GAJI_UANG_MAKAN&reqPeriode="+ $("#reqPeriode").val(),
					function(data){
					});	
					...
					*/
				window.parent.OpenDHTML('proses_gaji_set_lock.php?reqJenisProses=GAJI_UANG_MAKAN&reqPeriode='+ $("#reqPeriode").val(), 'Office Management - Aplikasi Penghasilan', '600', '300');
			  ...
			  $('#btnEksporRekap').on('click', function () {
				  ...
				  newWindow = window.open('gaji_uang_makan_rekap_excel.php?reqPeriode='+ $("#reqPeriode").val(), 'Cetak');
				  newWindow.focus();
				  
			  });	
			  
			  $('#btnEksporRekapTahunan').on('click', function () {
				  /*if(anSelectedData == "")
					  return false;	*/			
				  
				  newWindow = window.open('gaji_uang_makan_rekap_excel.php?reqPeriode='+ $("#reqPeriode").val().substr(2,4), 'Cetak');
				  newWindow.focus();
			  ...
			  $('#btnProses').on('click', function () {
				if(<?=$lock_cetak?> == 1)
				{ 
			  		oTable.fnReloadAjax("../json-gaji/gaji_uang_makan_json.php?reqMode=proses&reqPeriode=" + $("#reqPeriode").val() + "&reqJenisPegawai="+ $("#reqJenisPegawai").val());
				}
				else
				{
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Uang Makan</span>
            </div>            
            </td>
        </tr>
    </table>
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        ...
        <ul id="flexmenu1" class="flexdropdownmenu">
          ...
          <li><a href="#" id="btnCetakRekap">Rekap</a></li>     
        </ul>
        <ul id="flexmenu2" class="flexdropdownmenu">
          <li><a href="#" id="btnEkspor">Penerimaan</a></li>  
          <li><a href="#" id="btnEksporRekap">Rekapitulasi</a></li>
          <li><a href="#" id="btnEksporRekapTahunan">Rekapitulasi Tahunan</a></li>
        </ul>
    </div>
