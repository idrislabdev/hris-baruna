...
$fname = tempnam("/tmp", "gaji_insentif_semua_excel.xls");
...
$insentif_bantuan->selectByParamsReport(array(), -1, -1, "", $reqPeriode, " ORDER BY JENIS_PEGAWAI_ID, BANK_NAMA, NAMA ASC");
...
$worksheet->set_column(8, 8, 9.00);
$worksheet->set_column(9, 9, 7.00);
$worksheet->set_column(10, 10, 14.00);
$worksheet->set_column(11, 11, 14.00);
...
$worksheet->write(4, 0, "NO.", $text_format_line_bold);
$worksheet->write(4, 1, "NRP", $text_format_line_bold);
$worksheet->write(4, 2, "NAMA", $text_format_line_bold);
$worksheet->write(4, 3, "JABATAN", $text_format_line_bold);
$worksheet->write(4, 4, "KELAS", $text_format_line_bold);
$worksheet->write(4, 5, "NPWP", $text_format_line_bold);
$worksheet->write(4, 6, "NO REKENING", $text_format_line_bold);
$worksheet->write(4, 7, "BESAR TUNJANGAN", $text_format_line_bold);
$worksheet->write(4, 8, "POTONGAN", $text_format_line_bold);
$worksheet->write(4, 9, "PPH (Rp.)", $text_format_line_bold);
$worksheet->write(4, 10, "JUMLAH DITERIMA", $text_format_line_bold);
$worksheet->write(4, 11, "TANDA TANGAN", $text_format_line_bold);
...
{
	if($jenis_pegawai == $insentif_bantuan->getField("JENIS_PEGAWAI"))
	{}
	else
	{
		$worksheet->write($row, 0, $insentif_bantuan->getField("JENIS_PEGAWAI"), $text_format_merge_line_bold);
	...
	$worksheet->write($row, 1, $insentif_bantuan->getField("NRP"), $text_format_line);
	$worksheet->write($row, 2, $insentif_bantuan->getField("NAMA"), $text_format_line_left);
	$worksheet->write($row, 3, strtoupper($insentif_bantuan->getField("JABATAN")), $text_format_line_left);
	//$worksheet->write($row, 3, $insentif_bantuan->getField("NPWP"), $text_format_line);
	$worksheet->write($row, 4, $insentif_bantuan->getField("KELAS"), $text_format_line);
	$worksheet->write($row, 5, " " . $insentif_bantuan->getField("NPWP"), $text_format_line);
	$worksheet->write($row, 6, " " . $insentif_bantuan->getField("NO_REKENING"), $text_format_line);
	$worksheet->write($row, 7, $insentif_bantuan->getField("JUMLAH"), $uang_line);
	$worksheet->write($row, 8, $insentif_bantuan->getField("JUMLAH_POTONGAN"), $uang_line);
	$worksheet->write($row, 9, $insentif_bantuan->getField("JUMLAH_PPH"), $uang_line);
	$worksheet->write($row, 10, $insentif_bantuan->getField("DIBAYARKAN"), $uang_line);
	$worksheet->write($row, 11, $no." ............", $text_format_line_left);
	...
	$jenis_pegawai = $insentif_bantuan->getField("JENIS_PEGAWAI");	
...
header("Content-Type: application/x-msexcel; name=\"gaji_insentif_semua_excel.xls\"");
header("Content-Disposition: inline; filename=\"gaji_insentif_semua_excel.xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
?>