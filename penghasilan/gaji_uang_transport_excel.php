...
$worksheet->write(1, 1, "DAFTAR UANG TRANSPORT", $text_format);
...
while($uang_transport_bantuan->nextRow())
{
	...
	if($jenis_pegawai == $uang_transport_bantuan->getField("JENIS_PEGAWAI"))
	{}
	else
	{
	$jenis_bank = "";
		$worksheet->write($row, 1, $uang_transport_bantuan->getField("JENIS_PEGAWAI"), $text_format_merge_line_bold);
	...
	if($jenis_bank == $uang_transport_bantuan->getField("BANK_NAMA"))
	{}
	else
	{
		$worksheet->write($row, 1, $uang_transport_bantuan->getField("BANK_NAMA"), $text_format_merge_line_bold);
		$worksheet->write_blank($row, 2, $text_format_merge_line_bold);
		$worksheet->write_blank($row, 3, $text_format_merge_line_bold);
		$worksheet->write_blank($row, 4, $text_format_merge_line_bold);
		$worksheet->write_blank($row, 5, $text_format_merge_line_bold);
	...
	$jenis_pegawai = $uang_transport_bantuan->getField("JENIS_PEGAWAI");
	$jenis_bank = $uang_transport_bantuan->getField("BANK_NAMA");
	$row++;
}

$workbook->close();

header("Content-Type: application/x-msexcel; name=\"uang_transport_excel.xls\"");
header("Content-Disposition: inline; filename=\"uang_transport_excel.xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
?>