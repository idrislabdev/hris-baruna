...
//echo $departemen->query;
while($departemen->nextRow())
{
	$no = 1;
	$z=1;
	//$departemen_id = $departemen->getField("DEPARTEMEN_ID");
	$departemen_id = $departemen->getField("DEPARTEMEN_DIVISI_ID");
	
	$statement = "AND JENIS_PEGAWAI_ID = ".$reqJenisPegawai." AND DEPARTEMEN_ID = '".$departemen_id."'";
	
	$bantuan_pendidikan->selectByParamsReport(array(), -1, -1, $statement, $reqPeriode, " ORDER BY KELAS ASC, DEPARTEMEN_ID ASC");
	while($bantuan_pendidikan->nextRow())
	{
		if($nama_departemen == $bantuan_pendidikan->getField("DEPARTEMEN"))
		{
		}
		else		
		{
			$worksheet->write($row, 1, strtoupper($bantuan_pendidikan->getField("DEPARTEMEN")), $text_format_left_none);
			$row ++;
		}
		...
		$no++;
		$row++;
		$nama_departemen = $bantuan_pendidikan->getField("DEPARTEMEN");
	}
	
		if($departemen_divisi == $departemen->getField("DEPARTEMEN"))
		{
		}
		else		
		{
		...
		$departemen_divisi = $departemen->getField("DEPARTEMEN");
	
	$z++;
	
	/*$row_judul = $row + 3;
	$row_judul_index = $row + 1;*/
}
$workbook->close();
