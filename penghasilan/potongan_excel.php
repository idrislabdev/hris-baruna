...
$fname = tempnam("/tmp", "potongan_excel.xls");
...
if(count($arrJenisPegawaiId) > 1)
{
	...
	$statement = " AND JENIS_PEGAWAI_ID= ".$reqJenisPegawaiId." AND PERIODE= ".$reqPeriode;	

$gaji_awal_bulan->selectByParamsPotonganReportExcel(array(), -1, -1, $statement, $reqPeriode, " ORDER BY POTONGAN, NAMA ASC");
...
$worksheet->set_column(5, 5, 34.71);
...
$worksheet->write(1, 1, "DAFTAR POTONGAN ".$caption, $text_format);
...
while($gaji_awal_bulan->nextRow())
{
	...
	$worksheet->write($row, 4, $gaji_awal_bulan->getField('ARISAN_PERISPINDO'),$uang_line);
...
header("Content-Type: application/x-msexcel; name=\"potongan_excel.xls\"");
header("Content-Disposition: inline; filename=\"potongan_excel.xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
?>