...
elseif($reqMode == 'DOKUMEN'){
	include_once("../WEB-INF/classes/base-hukum/DokumenAttachment.php");
	$dokumen_attachment = new DokumenAttachment();
	$dokumen_attachment->selectByParams(array("DOKUMEN_ATTACHMENT_ID" => $reqId), -1, -1);
	$dokumen_attachment->firstRow();
	$tipe = $dokumen_attachment->getField('FORMAT');
	$isi = $dokumen_attachment->getField("FILE_UPLOAD");
	$nama = $dokumen_attachment->getField("FILE_NAMA");
}
