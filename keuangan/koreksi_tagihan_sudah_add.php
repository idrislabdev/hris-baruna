...
<head>
	...
	<script type="text/javascript">
		...
		$(function(){
			$('#ff').form({
				url:'../json-keuangansiuk/koreksi_tagihan_sudah_add.php',
				onSubmit:function(){
				...
				success:function(data){
					...
					document.location.href = 'koreksi_tagihan_sudah_add.php?reqId='+data[0];
				...
				{			
					$.getJSON('../json-keuangansiuk/posting_jurnal_aksi.php?reqId=<?=$reqId?>&reqKdSubSis=<?=$tempKdSubSis?>',
					  function(data){					  
						  ...
						  document.location.href = 'koreksi_tagihan_sudah_add.php?reqId=<?=$reqId?>';
						  top.frames['mainFrame'].location.reload();
					});	
				}
			});
		...
		function OptionSetPembayaran(NO_NOTA, KD_SUB_BANTU, MPLG_NAMA, JML_VAL_TRANS, KD_BUKU_BESAR, KLAS_TRANS)
		{
			 $.post( "koreksi_tagihan_sudah_add_template.php", { pNO_NOTA: NO_NOTA, pKD_SUB_BANTU: KD_SUB_BANTU, pMPLG_NAMA: MPLG_NAMA, pJML_VAL_TRANS: JML_VAL_TRANS, pKD_BUKU_BESAR: KD_BUKU_BESAR, pKLAS_TRANS: KLAS_TRANS })
			  .done(function( data ) {
		...
		function openPopup()
		{
			...
			$('#reqNoRef3').keyup(function (e) {
				if(e.which == 120)
				{
					OpenDHTMLPopup('koreksi_tagihan_sudah_pencarian.php', 'Pencarian Kelas', 950, 600);
					return false;
				}
			});
...
<div id="wadah">
    ...
    <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Tambah Data Pembatalan Tagihan Sudah Cetak Nota (SUDAH POSTING)</span>
    </div>
    ...
    <table>
        ...
        <tr>           
            ...
            <td>Tanggal Valuta</td>
			 <td>
                ...
                <input type="hidden" name="reqNoRef2" id="reqNoRef2" class="easyui-validatebox" style="width:150px"  value="<?=$reqNoRef2?>" tabindex="3"  />
                <input name="reqNoRef3" readonly id="reqNoRef3" class="easyui-validatebox" style="width:98%;" type="hidden"  value="<?=$reqNoRef3?>" onKeyDown="openPopup();"  />   <!--onKeyDown="openPopup();"-->
                <input name="reqBadanUsaha" id="reqBadanUsaha" class="easyui-validatebox" style="width:295px" type="hidden" value="SISWA" readonly />
				<input name="reqNoBukuBesarKas" id="reqNoBukuBesarKas" class="easyui-validatebox" style="width:40px; background-color:#f0f0f0" readonly type="hidden" value="<?=$tempNoBukuBesarKas?>"/>
                <input name="reqKodeKasBank" id="reqKodeKasBank" class="easyui-validatebox" readonly style="width:110px; background-color:#f0f0f0" type="hidden" value="<?=$tempKodeKasBank?>" readonly />
			</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>
                ...
                &nbsp;
                <input type="hidden" id="reqJumlahTransaksi" name="reqJumlahTransaksi" class="easyui-validatebox" style="width:140px; background-color:#f0f0f0" OnFocus="FormatAngka('reqJumlahTransaksi')" OnKeyUp="FormatUang('reqJumlahTransaksi')" OnBlur="FormatUang('reqJumlahTransaksi')" readonly value="<?=numberToIna($tempJumlahTransaksi)?>"  />             
            </td>
        </tr>
        <tr>
            <td>Kurs&nbsp;Valuta</td>
    ...
    <!--<table class="example altrowstable" id="alternatecolor" >-->
        <thead class="altrowstable">
          <tr>
              <th>No. JPJ <a onClick="OpenDHTMLPopup('koreksi_tagihan_sudah_pencarian.php?reqId=<?=$reqId?>', 'Rincian Jurnal', 900, 600)"><img src="images/tree-add.png"></a></th>
              <th>No. Kartu</th>
              <th>Siswa</th>
        ...
        <tbody class="example altrowstable" id="alternatecolor"> 
          ...
          ?>
              <tr id="node-<?=$i?>">
                  <td>
                  <input type="text" name="reqNoPpkb[]" id="reqNoPpkb<?=$checkbox_index?>" class="easyui-validatebox" value="<?=$kptt_nota_d->getField("NO_REF2")?>" style="width:98%" readonly />
                  </td>
                  <td>
                  <input type="text" name="reqNIS[]" id="reqNoPpkb<?=$checkbox_index?>" class="easyui-validatebox" value="<?=$kptt_nota_d->getField("KD_SUB_BANTU")?>" style="width:98%" readonly />
                  </td>
                  <td>
                  ...
                  <td>
                    <input type="text" name="reqTagihan[]"  style="text-align:right;"  id="reqSisaPiutang<?=$checkbox_index?>" readonly  OnFocus="FormatAngka('reqSisaPiutang<?=$checkbox_index?>')" OnKeyUp="FormatUang('reqSisaPiutang<?=$checkbox_index?>')" OnBlur="FormatUang('reqSisaPiutang<?=$checkbox_index?>')" value="<?=numberToIna($kptt_nota_d->getField("JML_VAL_TRANS"))?>">
                  </td>
                  <td align="center">
                  <input type="hidden" name="reqBukuBesar[]" value="<?=$kptt_nota_d->getField("KD_BUKU_BESAR")?>">
                  <input type="hidden" name="reqKlasTrans[]" value="<?=$kptt_nota_d->getField("KLAS_TRANS")?>">
                  <label>
