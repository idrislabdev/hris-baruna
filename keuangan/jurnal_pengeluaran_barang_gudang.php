...
<title>Jurnal Penerimaan Kas Bank</title>
...
<style type="text/css" media="screen">
	...
	oTable = $("#example").dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			 ...
			 "aoColumns": [ 
			  ...
			  "bServerSide": true,		
			  "sAjaxSource": "../json-keuangansiuk/jurnal_pengeluaran_barang_gudang_json.php?reqPeriode=<?=$reqPeriode?>",
	...
	$('#btnEdit').on('click', function () {
		...
		if(anSelectedNoPosting == '')
		  window.parent.OpenDHTML('jurnal_pengeluaran_barang_gudang_add.php?reqMode=update&reqId='+anSelectedId, 'Office Management - Aplikasi Keuangan', '600', '300');
		else
		...
		$.getJSON('../json-keuangansiuk/get_posting_json.php?reqTabel=KBBT_JUR_BB&reqId='+anSelectedId,
		  function(data){
			  if(data.NO_POSTING == "")
			  {
				  ...
				  {
					  $.getJSON('../json-keuangansiuk/hapus_data_json.php?reqSource=jurnal_pengeluaran_barang_gudang&reqId='+anSelectedId,
					  function(data){
						  if(data.HASIL == 1)
						  {
	...
	$('#btnCetak').on('click', function () {
	   if(anSelectedData == "")
			return false;	
			  window.parent.OpenDHTML('jurnal_pengeluaran_barang_gudang_penanda_tangan.php?reqNoBukti='+ anSelectedId, 'Office Management - Aplikasi Keuangan', '600', '300');				  
					
	});
	
	$('#reqPeriode').combobox({
	  onSelect: function(param){
		  document.location.href = 'jurnal_pengeluaran_barang_gudang.php?reqPeriode='+ param.id;
	  }
	});
...
<div id="wadah">
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul>
            ...
            <?php /*?><li>
            <a href="#" id="btnAdd" onClick="alert('No.&nbsp;Nota tidak ikut SIUK'); window.parent.OpenDHTML('jurnal_pengeluaran_barang_gudang_add.php?reqMode=insert', 'Office Management - Aplikasi Keuangan', '600', '300');" title="Tambah">Tambah</a>
    ...
    <thead>
        <tr>
            ...
            <th width="170px">Bukti&nbsp;Pendukung</th>
