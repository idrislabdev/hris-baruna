...
include_once("../WEB-INF/classes/base-operasional/KapalKepemilikan.php");
include_once("../WEB-INF/classes/base-operasional/Pelanggan.php");
/* LOGIN CHECK */
...
$kapal_kepemilikan = new KapalKepemilikan();
...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-operasional/kapal_json.php?reqDepartemen=<?=$reqDepartemen?>",
			...
			/* Click event handler */
			  ...
			  $('#btnEdit').on('click', function () {
				  if(anSelectedData == "")
					  return false;				
				  window.parent.OpenDHTML('kapal_add.php?reqId='+anSelectedId, 'Office Management - Aplikasi Operasional Kapal', '980', '575');	
			  });
			  
			  $('#btnCetakDaftarSusunanPerwiraKapal').on('click', function () {
				if(anSelectedData == '')	
					return false;		
			  	newWindow = window.open('daftar_susunan_perwira_kapal_rpt.php?reqId='+ anSelectedId , 'Cetak');
				newWindow.focus();
			  });
			  
			  $('#btnCetakDaftarAwakKapal').on('click', function () {
				if(anSelectedData == '')	
					return false;		
			  	newWindow = window.open('daftar_awak_kapal_rpt.php?reqId='+ anSelectedId, 'Cetak');
				newWindow.focus();
			  });			  			  
			  
			  $('#btnProsesCetakSP').on('click', function () {
				window.parent.OpenDHTML('kapal_surat_perintah.php', 'Office Management - Aplikasi Operasional Kapal', '980', '575');	
			  });			  			  

			  $('#btnProsesPersetujuan').on('click', function () {
				window.parent.OpenDHTML('kapal_persetujuan.php', 'Office Management - Aplikasi Operasional Kapal', '980', '575');	
			  });			  			  
			  ...
			  /* RIGHT CLICK EVENT */
			  $("#reqPelanggan").change(function() { 
				 oTable.fnReloadAjax("../json-operasional/kapal_json.php?reqDepartemen=<?=$reqDepartemen?>&reqPelangganId=" + $("#reqPelanggan").val());
			  });	  			  
		} );
</script>
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Master Kapal</span>
            </div>            
            </td>
        </tr>
    </table>
    <div style="position: relative; z-index:10">
    <div style="position: absolute; margin-top:13px; margin-left:176px; z-index:9999; font-size:12px;">
        Pemilik : 
        		  <select name="reqPelanggan" id="reqPelanggan">
        		  <option value="">Semua</option>
        		  <?
				  $pelanggan->selectByParams();
                  while($pelanggan->nextRow())
				  {
				  ?>
                  	<option value="<?=$pelanggan->getField("PELANGGAN_ID")?>"><?=$pelanggan->getField("NAMA")?></option>
                  <?	  
				  }
				  ?>
        		  </select>                  
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul> 
            <li>
            	<?php /*?><a href="#" id="btnReport" title="Report">Report</a> <?php */?>           
            </li>              
        </ul>           
    </div>
    <?
    ...
    <div style="position: absolute; margin-top:6px; margin-left:318px; z-index:9999; font-size:12px;">
        Departemen : <input id="cc" class="easyui-combotree" name="reqDepartemen" data-options="url:'../json-operasional/departemen_combo_json.php'" style="width:300px;">&nbsp;&nbsp;&nbsp;&nbsp;
    </div>
    </div>-->
    ...
    <thead>
        <tr>
            ...
            <th width="85px">Horse Power</th>
            <th width="85px">IMO Number</th>
            <th width="85px">Jenis Kapal</th>
            <th width="85px">Tahun</th>
            <!--<th width="85px">PERUSAHAAN_BANGUN</th>
            <th width="85px">TEMPAT_BANGUN</th>
            <th width="85px">TAHUN_BANGUN</th>
            <th width="85px">GT</th>
            <th width="85px">NT</th>
            <th width="85px">LOA</th>
            <th width="85px">BREADTH</th>
            <th width="85px">DEPTH</th>
            <th width="85px">DRAFT</th>
            <th width="85px">MESIN_INDUK</th>
            <th width="85px">MESIN_BANTU</th>
            <th width="85px">MESIN_DAYA</th>
            <th width="85px">MESIN_RPM</th>
            <th width="85px">POMPA</th>
            <th width="85px">ISI_TANGKI</th>
            <th width="85px">AIR_BERSIH</th>
            <th width="85px">KECEPATAN</th>
            <th width="85px">FOTO</th>
            <th width="85px">STATUS_KESIAPAN</th>
            <th width="85px">BENDERA</th>-->
        </tr>
    </thead>
    </table>
    </div>    
