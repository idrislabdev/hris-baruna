...
$kptt_nota_d->selectDaftarTagihanSPPKelas(array("KD_SUB_BANTU" => $reqId, "NOT A.DEPARTEMEN_ID" => "16", "TAGIHAN" => $reqNilai));
$kptt_nota_d->firstRow();
if($kptt_nota_d->getField("NO_NOTA") == "")
{
	$info = "";
	$kptt_nota_d_info = new KpttNotaD();
	$kptt_nota_d_info->selectDaftarBayarTagihanSPPKelas(array("KD_SUB_BANTU" => $reqId, "NOT A.DEPARTEMEN_ID" => "16", "TAGIHAN" => $reqNilai), 3, 0);
	$i = 0;
	while($kptt_nota_d_info->nextRow())
	{
		if($i == 0)
			$info = "Daftar tagihan sudah bayar oleh kartu ".$reqId." dengan nominal ".numberToIna($reqNilai)." ".$kptt_nota_d_info->getField("NO_NOTA")."(".$kptt_nota_d_info->getField("NO_REF_PELUNASAN").")";	
		else
			$info .= "; ".$kptt_nota_d_info->getField("NO_NOTA")."(".$kptt_nota_d_info->getField("NO_REF_PELUNASAN").")";
	...
	$infoTagihan = "";
	$kptt_nota_d_info = new KpttNotaD();
	$kptt_nota_d_info->selectDaftarTagihanSPPKelas(array("KD_SUB_BANTU" => $reqId, "NOT A.DEPARTEMEN_ID" => "16"));
	$i = 0;
	while($kptt_nota_d_info->nextRow())
	{
		if($i == 0)
			$infoTagihan = "<br>Tagihan Lain ".numberToIna($kptt_nota_d_info->getField("TAGIHAN"))." ".$kptt_nota_d_info->getField("NO_NOTA");	
		else
			$infoTagihan .= "; ".numberToIna($kptt_nota_d_info->getField("TAGIHAN"))." ".$kptt_nota_d_info->getField("NO_NOTA");
...
?>
  <tr id="node-<?=$i?>">
	  ...
	  <td>
		<input type="text" name="reqSisaBayar[]"  style="text-align:right;"  id="reqSisaBayar<?=$checkbox_index?>"  readonly OnFocus="FormatAngka('reqSisaBayar<?=$checkbox_index?>')" OnKeyUp="FormatUang('reqSisaBayar<?=$checkbox_index?>')" OnBlur="FormatUang('reqSisaBayar<?=$checkbox_index?>')" value="<?=numberToIna($kptt_nota_d->getField("TAGIHAN")-$reqNilai)?>">
	  </td>
	  <td align="center">
	  <label>
