...
<script type="text/javascript" charset="utf-8">
    $(document).ready( function () {
			   $('#btnCetak').live('click', function () {
 				//alert('exec LAPORAN_KEUANGAN.ARUS_KAS DAN LB4NEW, IV, V DAN VI BELUM DIKETAHUI!!!');
				$.getJSON('../json-keuangansiuk/arus_kas_proses_json.php?reqPeriode='+ $("#reqPeriode").val(),
				  function(data){
					newWindow = window.open('arus_kas_rpt.php?reqHalaman='+ $("#reqHalaman").val()+'&reqPeriode='+ $("#reqPeriode").val()+'&reqPejabatKiri='+ $("#reqPejabatKiri").val(), 'Cetak');
					newWindow.focus();
				});					
			  });
			   ...
			   $('#btnExcel').live('click', function () {
				...
				$.getJSON('../json-keuangansiuk/arus_kas_proses_json.php?reqPeriode='+ $("#reqPeriode").val(),
				  function(data){
					newWindow = window.open('arus_kas_excel.php?reqHalaman='+ $("#reqHalaman").val()+'&reqPeriode='+ $("#reqPeriode").val()+'&reqPejabatKiri='+ $("#reqPejabatKiri").val(), 'Cetak');
					newWindow.focus();
				});					
			  });
	});
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Laporan Arus Kas</span>
            </div>            
            </td>
        </tr>
    </table>
        ...
        <tr>           
			 ...
			 <td>
				<select id="reqPejabatKiri">
					...
					{
					?>
                    	<option value="<?=$officer[$i]["NM_KET1"].'-'.$officer[$i]["NM_KET2"]?>" <? if($officer[$i]["KD_TABEL"] == "KACAB") { ?> selected <? } ?>><?=$officer[$i]["NM_KET1"].'-'.$officer[$i]["NM_KET2"]?></option>                        
                    <?	
					}
					?>
                </select>
