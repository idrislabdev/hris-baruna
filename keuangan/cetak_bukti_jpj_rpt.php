...
//------  Variables ------
/*if($reqFont=="CONSOLAS")
$file_name	= "CETAK_BUKTI_JPJ_CONSOLAS";
elseif($reqFont=="LUCIDE_CONSOLE")
$file_name	= "CETAK_BUKTI_JPJ_LUCIDE_CONSOLE";
elseif($reqFont=="CENTURY_GOTHIC")
$file_name	= "CETAK_BUKTI_JPJ_CENTURY_GOTHIC";
else*/
$file_name	= "CETAK_BUKTI_JPJ";

$my_report	= $path.$file_name.".rpt";
$my_pdf		= $path.$file_name.$userLogin->idUser.".rpt";
...
/*$arrReqNoBukti = explode(",", $reqId);
if(count($arrReqNoBukti) > 1)
{
	$statement = " ( ";
	
	for($i=0;$i<count($arrReqNoBukti);$i++)
	{
		if($i > 0)
			$statement .= " OR ";	
			
		$statement .= "{KBBT_JUR_BB_CETAK_1.NO_NOTA}='".$arrReqNoBukti[$i]."'";
	}
	
	$statement .= " ) ";
}
else
	$statement = " {KBBT_JUR_BB_CETAK_1.NO_NOTA}= '".$reqId."'";*/
...
//$creport->ExportOptions->CreateTextFormatOptions->CharactersPerInch = 16;
$creport->ExportOptions->FormatType=1;
...
$arrFinal = array("PESAN" => $file_name.$userLogin->idUser);
