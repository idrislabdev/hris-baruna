...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-keuangansiuk/proses_pelunasan_kas_tunai_json.php?reqPeriode=<?=$reqPeriode?>",			  
			...
			/* Click event handler */
			  ...
			  $('#btnView').on('click', function () {
				  if(anSelectedData == "")
					  return false;				
				  window.parent.OpenDHTML('proses_pelunasan_kas_bank_view.php?reqMode=update&reqId='+anSelectedId, 'Office Management - Aplikasi Keuangan', '600', '300');	
			  });
			  
			   $('#btnCetak').on('click', function () {
				   if(anSelectedData == "")
						return false;	
						  window.parent.OpenDHTML('../keuangan/jurnal_penerimaan_kas_bank_penanda_tangan.php?reqNoBukti='+ anSelectedId, 'Office Management - Aplikasi Keuangan', '600', '300');				  
			  ...
			  $('#btnEdit').on('click', function () {
				  ...
				  if(anSelectedNoPosting == '')
				  	window.parent.OpenDHTML('../keuangan/proses_pelunasan_kas_tunai_add.php?reqMode=update&reqId='+anSelectedId, 'Office Management - Aplikasi Keuangan', '600', '300');	
			  	  else
			  ...
			  $('#btnBatalkan').on('click', function () {
				  ...
				  {}
					...
					{
						$.getJSON('../json-keuangansiuk/hapus_data_json.php?reqSource=PELUNASAN_KAS_TUNAI&reqId='+anSelectedId,
						function(data){
							if(data.HASIL == 1)
							{
				  ...
				  $.getJSON('../json-keuangansiuk/get_posting_json.php?reqTabel=KPTT_NOTA&reqId='+anSelectedId,
					function(data){
						if(data.NO_POSTING == "")
						{
							if(confirm('Apakah anda yakin ingin menghapus nota : '+anSelectedId+'?'))
							{
								$.getJSON('../json-keuangansiuk/hapus_data_json.php?reqSource=PELUNASAN_KAS_BANK&reqId='+anSelectedId,
								function(data){
									if(data.HASIL == 1)
									{
										alert('Data berhasil dihapus.');
			  ...
			  /*$('#btnCetakBukti').on('click', function () {
				if(anSelectedData == "")
				return false;	
			  	window.parent.OpenDHTML('proses_pelunasan_kas_bank_penanda_tangan.php?reqNoBukti='+ anSelectedId, 'Office Management - Aplikasi Keuangan', '600', '300');	
			  });*/
			  ...
			  $('#btnProses').on('click', function () {
				  ...
				  if(anSelectedNoPosting == '')
				  {
					if(confirm('Proses ulang jurnal nomor : '+anSelectedId+'?'))
					{
				  	  $.get( "../json-pembayaran/proses_pelunasan_kas_tunai_json_proses.php/?reqNoBukti="+ anSelectedId, function( data ) {
						  alert( data ); 
  						  oTable.fnReloadAjax("../json-keuangansiuk/proses_pelunasan_kas_tunai_json.php?reqPeriode="+ $("#reqPeriode").val());
					  });
				  	}
				  }
			  	  else
				  	alert('Data yang telah diposting tidak dapat diproses ulang.');
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Monitoring Jurnal Pelunasan Kas Tunai</span>
            </div>            
            </td>
        </tr>
    </table>
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul>
            <li>
            <a href="#" id="btnEdit" title="Edit">Lihat</a>
            <a href="#" id="btnCetak" title="Cetak">Cetak</a>
            <a href="#" id="btnProses" title="Proses Ulang Jurnal">Proses Ulang Jurnal</a>
            <a href="#" id="btnBatalkan" title="Batalkan Jurnal">Batalkan Jurnal</a>
            </li>        
        </ul>
    </div>
    ...
    <thead>
        <tr>
            ...
            <th width="70px">Tgl.Trans</th>
            <th width="300px">Keterangan</th>
            <th width="100px">Jumlah&nbsp;Trans</th>
            <th width="100px">Rek.&nbsp;Kas&nbsp;/&nbsp;Bank</th>
