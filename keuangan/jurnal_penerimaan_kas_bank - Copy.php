...
$kbbt_jur_bb_tmp->selectByParamsPeriode(array("JEN_JURNAL" => "JKM"), -1, -1, " AND BLN_BUKU IS NOT NULL ");
...
<script src="../WEB-INF/lib/media/js/jquery.dataTables.js" type="text/javascript"></script>
...
<script src="../WEB-INF/lib/media/js/TableTools.js" type="text/javascript"></script>
...
<script type="text/javascript" charset="utf-8">
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-keuangansiuk/jurnal_penerimaan_kas_bank_json.php?reqPeriode=<?=$reqPeriode?>",
			...
			sDeleteURL: "../json-keuangansiuk/delete.php?reqMode=jurnal_penerimaan_kas_bank",
			sAddHttpMethod: "GET",  
			"aoColumns": [
				{ 	cssclass: "required" },
				{
			  ...
			  $('#btnRincianJurnal').live('click', function () {
				  if(anSelectedData == "")
					  return false;				
				  ...
				  $.getJSON('../json-keuangansiuk/get_posting_json.php?reqTabel=KBBT_JUR_BB&reqId='+anSelectedId,
					function(data){
						if(data.NO_POSTING == "")
						{
							...
							{
								$.getJSON('../json-keuangansiuk/hapus_data_json.php?reqSource=JURNAL_PENERIMAAN_KAS_BANK&reqId='+anSelectedId,
								function(data){
									if(data.HASIL == 1)
									{
			  ...
			  $('#btnCetak').live('click', function () {
			  	 if(anSelectedData == "")
					  return false;	
						newWindow = window.open('jurnal_penerimaan_kas_bank_rpt.php?reqId='+ anSelectedId, 'Cetak');
						newWindow.focus();					  
							  
			  });
			  
			  $("#reqPeriode").change(function() { 
				 oTable.fnReloadAjax("../json-keuangansiuk/jurnal_penerimaan_kas_bank_json.php?reqPeriode="+ $("#reqPeriode").val());
...
<div id="wadah">
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul>
            ...
            <li>
            <a href="#" id="btnAdd" onClick="window.parent.OpenDHTML('jurnal_penerimaan_kas_bank_add.php?reqMode=insert', 'Office Management - Aplikasi Keuangan', '600', '300');" title="Tambah">Tambah</a>
