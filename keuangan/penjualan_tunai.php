...
$kptt_nota->selectByParamsPeriode(array("TIPE_TRANS" => "JKM-KPT-03"), -1, -1, " AND BLN_BUKU IS NOT NULL ");
...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-keuangansiuk/penjualan_tunai_json.php?reqPeriode=<?=$reqPeriode?>",			  
			...
			/* Click event handler */
			  ...
			  $('#example tbody').on( 'click', 'tr', function () {
					if ( $(this).hasClass('selected') ) {
					  ...
					  anSelectedValuta = element[12];
			  });
			  
			  $('#btnView').on('click', function () {
				  if(anSelectedData == "")
					  return false;				
				  window.parent.OpenDHTML('penjualan_tunai_view.php?reqMode=update&reqId='+anSelectedId, 'Office Management - Aplikasi Keuangan', '600', '300');	
			  });
			  ...
			  $('#btnEdit').on('click', function () {
				  ...
				  if(anSelectedNoPosting == '')
				  	window.parent.OpenDHTML('penjualan_tunai_add.php?reqMode=update&reqId='+anSelectedId, 'Office Management - Aplikasi Keuangan', '600', '300');
			  	  else
				  ...
				  $.getJSON('../json-keuangansiuk/get_posting_json.php?reqTabel=KPTT_NOTA&reqId='+anSelectedId,
					function(data){
						if(data.NO_POSTING == "")
						{
							...
							{
								$.getJSON('../json-keuangansiuk/hapus_data_json.php?reqSource=PENJUALAN_TUNAI&reqId='+anSelectedId,
								function(data){
									if(data.HASIL == 1)
									{
			  ...
			  $('#btnScreenIND').on('click', function () {
			  	 if(anSelectedData == "")
					  return false;	
					window.parent.OpenDHTML('penjualan_tunai_screen_rpt.php?reqId='+ anSelectedPPKB+ '&reqKdValuta=IND', 'Cetak');
			  ...
			  $('#btnScreenENG').on('click', function () {
			  	 if(anSelectedData == "")
					  return false;	
					window.parent.OpenDHTML('penjualan_tunai_screen_rpt.php?reqId='+ anSelectedPPKB+ '&reqKdValuta=ENG', 'Cetak');
			  ...
			  $('#btnPrintIND').on('click', function () {
				  ...
				  $.getJSON('../json-keuangansiuk/penjualan_tunai_cetak.php?reqId='+anSelectedId,
					function(data){
					  window.parent.OpenDHTML('penjualan_tunai_screen_rpt.php?reqId='+ anSelectedPPKB+ '&reqKdValuta=IND', 'Cetak');
			  ...
			  $("#reqPeriode").change(function() { 
				 oTable.fnReloadAjax("../json-keuangansiuk/penjualan_tunai_json.php?reqPeriode="+ $("#reqPeriode").val());
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Transaksi Penjualan Tunai (JKM)</span>
            </div>            
            </td>
        </tr>
    </table>
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul>
            <li>
            <a href="#" id="btnAdd" onClick="window.parent.OpenDHTML('penjualan_tunai_add.php?reqMode=insert', 'Office Management - Aplikasi Keuangan', '600', '300');" title="Tambah">Tambah</a>
