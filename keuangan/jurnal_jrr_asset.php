...
$(document).ready(function() {
	...
	oTable = $("#example").dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			 ...
			 "aoColumns": [ 
			  ...
			  "bServerSide": true,		
			  "sAjaxSource": "../json-keuangansiuk/jurnal_jrr_asset_json.php?reqPeriode=<?=$reqPeriode?>",
		  ...
		  anSelectedNoPosting = element[8];	
			  ...
			  $('#btnEdit').on('click', function () {
				  ...
				  if(anSelectedNoPosting == '')
				  	window.parent.OpenDHTML('jurnal_jrr_asset_add.php?reqMode=update&reqId='+anSelectedId, 'Office Management - Aplikasi Keuangan', '600', '300');
			  	  else
			  ...
			  $('#btnRincianJurnal').on('click', function () {
				  if(anSelectedData == "")
					  return false;				
				  window.parent.OpenDHTML('monitoring_jurnal_transaksi_asset_lihat.php?&reqId='+anSelectedId, 'Office Management - Aplikasi Keuangan', '800', '500');		
			  });
				  ...
				  $.getJSON('../json-keuangansiuk/get_posting_json.php?reqTabel=KBBT_JUR_BB&reqId='+anSelectedId,
					function(data){
						if(data.NO_POSTING == "")
						{
							...
							{
								$.getJSON('../json-keuangansiuk/hapus_data_json.php?reqSource=JURNAL_RUPA_RUPA&reqId='+anSelectedId,
								function(data){
									if(data.HASIL == 1)
									{
			...
			$('#btnCopy').on('click', function () {
				...
				$('#reqPeriode').combobox({
				  onSelect: function(param){
					  if(param.text == 'Semua')
						  document.location.href = 'jurnal_jrr_asset.php?reqMode=all';
					  else						  
					      document.location.href = 'jurnal_jrr_asset.php?reqPeriode='+ param.id;
				  }
				});
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Jurnal Rupa-rupa Asset</span>
            </div>            
            </td>
        </tr>
    </table>
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul>
            ...
            <li>
            <a href="#" id="btnAdd" onClick="window.parent.OpenDHTML('jurnal_jrr_asset_add.php?reqMode=insert', 'Office Management - Aplikasi Keuangan', '600', '300');" title="Tambah">Tambah</a>
