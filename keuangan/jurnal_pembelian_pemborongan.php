...
<title>jurnal pembelian pemborongan</title>
...
$(document).ready(function() {
	...
	oTable = $("#example").dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			 ...
			 "aoColumns": [ 
			  ...
			  "bServerSide": true,		
			  "sAjaxSource": "../json-keuangansiuk/jurnal_pembelian_pemborongan_json.php?reqPeriode=<?=$reqPeriode?>",
	...
	$('#btnEdit').on('click', function () {
		...
		if(anSelectedNoPosting == '')
		  window.parent.OpenDHTML('jurnal_pembelian_pemborongan_add.php?reqMode=update&reqId='+anSelectedId, 'Office Management - Aplikasi Keuangan', '600', '300');
		else
	...
	$('#btnCetak').on('click', function () {
	   if(anSelectedData == "")
			return false;	
			  window.parent.OpenDHTML('jurnal_pembelian_pemborongan_penanda_tangan.php?reqNoBukti='+ anSelectedId, 'Office Management - Aplikasi Keuangan', '600', '300');				  
		...
		$.getJSON('../json-keuangansiuk/get_posting_json.php?reqTabel=KBBT_JUR_BB&reqId='+anSelectedId,
		  function(data){
			  if(data.NO_POSTING == "")
			  {
				  ...
				  {
					  $.getJSON('../json-keuangansiuk/hapus_data_json.php?reqSource=jurnal_pembelian_pemborongan&reqId='+anSelectedId,
					  function(data){
						  if(data.HASIL == 1)
						  {
	...
	$('#reqPeriode').combobox({
	  onSelect: function(param){
		  document.location.href = 'jurnal_pembelian_pemborongan.php?reqPeriode='+ param.id;
	  }
	});
...
<div id="wadah">
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul>
			...
			<?php /*?>
            <li>
            <a href="#" id="btnAdd" onClick="alert('No.&nbsp;Nota tidak ikut SIUK'); window.parent.OpenDHTML('jurnal_pembelian_pemborongan_add.php?reqMode=insert', 'Office Management - Aplikasi Keuangan', '600', '300');" title="Tambah">Tambah</a>
