...
<head>
	...
	<script type="text/javascript">
		...
		{
			//$('#reqStatusVerifikasi').removeAttr('checked');
			
			if(value == 1)
				$("#reqStatusVerifikasi").attr('checked', true);
			else
				$("#reqStatusVerifikasi").attr('checked', false);
		}
		...
		$(function(){
			$('#ff').form({
				url:'../json-anggaran/anggaran_mutasi_validasi_add_data.php',
				onSubmit:function(){
					return $(this).form('validate');
				},
				success:function(data){
					data = data.split("-");
					document.location.href = 'anggaran_mutasi_validasi_add_data.php?reqId=<?=$reqId?>';
		...
		{
			$.messager.confirm('Confirm','Anda yakin akan memvalidasi data ini?',function(r){
			if (r){
				$('#ff').submit();
				}
			})
...
<div id="wadah">
    ...
    <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Data Validasi Ppa</span>
    </div>
    ...
    <table>
        ...
        <tr>           
			 ...
			 <td>
             	<input type="checkbox" <? if($tempStatusVerifikasi == 1) echo "checked";?> name="reqStatusVerifikasi" id="reqStatusVerifikasi" readonly/>
			</td>            
		</tr>
    </table>
        <div>
