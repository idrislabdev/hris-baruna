...
$info_keterangan_uang= "UANG TELAH DITERIMA OLEH";
$info_nama_terang= "NAMA TERANG";

function setPanjangJabatan($panjangJabatan, $panjangPenandatangan)
{
	if($panjangJabatan < $panjangPenandatangan)
	{
		$tempIndexPejabatNama= $panjangPenandatangan - $panjangJabatan;
	}
	elseif($panjangJabatan > $panjangPenandatangan)
	{
		$tempIndexPejabatNama= $panjangJabatan - $panjangPenandatangan;
	}
	else
	{
		$tempIndexPejabatNama= "";
	}
	return $tempIndexPejabatNama;
}

function setPanjangJabatanPlusMin($panjangJabatan, $panjangPenandatangan)
{
	if($panjangJabatan < $panjangPenandatangan)
	{
		$tempIndexPejabatNama= "-";
...
function setPanjangJabatanReview($jabatan, $pejabat, $tempIndexPejabatNama)
{
	$tempVal="";
	if($tempIndexPejabatNama % 2 == 0){}
	else
	{
		$arrJabatan= explode(" ",$jabatan);
		$indexJabatan= count($arrJabatan);
		
		for($x=0; $x<$indexJabatan; $x++)
		{
			if($x==0)
				$jabatan= $arrJabatan[$x];
			else
				$jabatan.="  ".$arrJabatan[$x];
		}
		$panjangJabatan= strlen($jabatan);
		$panjangPenandatangan= strlen($pejabat);
		$tempIndexPejabatNama= setPanjangJabatan($panjangJabatan, $panjangPenandatangan);
	}
	$tempVal= $tempIndexPejabatNama;
...
$tempIndexPejabatNama= setPanjangJabatan($panjangJabatan, $panjangPenandatangan);
$tempIndexPejabatNama= setPanjangJabatanReview($jabatan, $pejabat, $tempIndexPejabatNama);
$tempIndexPejabatNama/=2;

$panjangJabatan= strlen($jabatan);
$panjangKeteranganUang= strlen($info_keterangan_uang);

$tempIndexKeteranganUang= setPanjangJabatan($panjangJabatan, $panjangKeteranganUang);
$tempIndexKeteranganUang= setPanjangJabatanReview($jabatan, $info_keterangan_uang, $tempIndexKeteranganUang);
$tempIndexKeteranganUang/=2;

$panjangJabatan= strlen($jabatan);
$panjangNamaTerang= strlen($info_nama_terang);

$tempIndexNamaTerang= setPanjangJabatan($panjangJabatan, $panjangNamaTerang);
$tempIndexNamaTerang= setPanjangJabatanReview($jabatan, $info_nama_terang, $tempIndexNamaTerang);
$tempIndexNamaTerang/=2;
...
//echo $statement;
$index_set= $index_set_detil= 0;
$set->selectByParamsJKK(array(), -1, -1,$statement);
// echo $set->query;exit();
while($set->nextRow())
{
	...
	$arrData[$index_set]["NO_REF2"]= $set->getField("NO_REF2");
	...
	$arrData[$index_set]["NM_BUKU_BESAR_WITH_PARENT"]= br2nl($set->getField("NM_BUKU_BESAR_WITH_PARENT"));
	$arrData[$index_set]["NM_BUKU_BESAR"]= br2nl($set->getField("NM_BUKU_BESAR"));
	...
	$arrData[$index_set]["NO_POSTING"]= $set->getField("NO_POSTING");
	$arrData[$index_set]["TGL_POSTING"]= $set->getField("TGL_POSTING");
	
	if($tempNoNotaKondisi == $arrData[$index_set]["NO_NOTA"]){}
	else
	{
		$arrDataIndex[$index_set_detil]= $index_set;
		$index_set_detil++;
	}
	$tempNoNotaKondisi= $arrData[$index_set]["NO_NOTA"];
	
	$index_set++;
}
//print_r($arrData);
//print_r($arrDataIndex);
?>
...
<script type="text/javascript" charset="utf-8">
   ...
   function print() {
         ...
         if (qz != null) {
			...
			$tempKondisiNoBukti="";
			for($param_index=0;$param_index<count($arrDataIndex);$param_index++)
			{
				if($param_index == 0){}
				else
				{
			?>
				header=headerHalaman=headerDetil= "";
				rowHeader= rowHeaderDetil= 1;
				isiValue+="\f";
			<?
				}
			?>
			tempIsiValue="";
			// buat header
			header="   YAYASAN BARUNAWATI BIRU SURABAYA                        REPORT ID.    : KBB_BKT_JKK_ENTRY.PR\n";
			header+="   -----------------------------------                            TGL.PROSES    : <?=dateToPageCheck($arrData[$arrDataIndex[$param_index]]["TGL_TRANS"])?>\n";
			header+="   <?php if($arrData[$arrDataIndex[$param_index]]["NO_REF2"] != '1') {echo 'Auto';} else {echo '   '; } ?>                                                   HALAMAN       : ";
			headerHalaman+="\n                             BUKTI PENGELUARAN KAS-BANK\n";
			headerHalaman+="                            ============================\n\n";
			headerHalaman+="   NO SIUK  : <?=$arrData[$arrDataIndex[$param_index]]["NO_NOTA"]?>                                         TANGGAL  : <?=dateToPageCheck($arrData[$arrDataIndex[$param_index]]["TGL_TRANS"])?>\n";
			headerHalaman+=" |=============================================================================================|\n";
			rowHeader=9;
			
			// buat header Detil
			var temp= " | 1. Pemegang Kas Harap membayarkan uang sebesar ";
			headerDetil=fillStringCompletor(temp,"37"," ",":");
			temp+= "Rp. <?=numberToIna($arrData[$arrDataIndex[$param_index]]["JML_RP_TRANS"])?>";
			headerDetil=fillStringCompletor(temp,"95"," ","|")+"\n";rowHeaderDetil++;

			temp= " | 2. Terbilang";
			temp=fillStringCompletor(temp,"24"," ",": ");
			temp+= "<?=terbilang($arrData[$arrDataIndex[$param_index]]["JML_RP_TRANS"])." Rupiah"?>";
			temp=fillStringCompletor(temp,"95"," ","");
			var result = splitIntoLines(temp, 90);
			...
			temp= " | 3. Kepada";
			temp=fillStringCompletor(temp,"24"," ",": ");
			temp+= "<?=$arrData[$arrDataIndex[$param_index]]["NM_AGEN_PERUSH"]?>";
			temp=fillStringCompletor(temp,"95"," ","|");
			var panjangTemp= panjangString(temp);
			//alert(temp+'--'+panjangTemp);
			headerDetil+=temp+"\n";rowHeaderDetil++;
			...
			temp= " | 4. Alamat";
			temp=fillStringCompletor(temp,"24"," ",": ");
			temp+= "<?=$arrData[$arrDataIndex[$param_index]]["ALMT_AGEN_PERUSH"]?>";
			temp=fillStringCompletor(temp,"95"," ","|");
			var panjangTemp= panjangString(temp);
			//alert(temp+'--'+panjangTemp);
			headerDetil+=temp+"\n";rowHeaderDetil++;
			...
			temp= " | 5. Uraian";
			temp=fillStringCompletor(temp,"24"," ",": ");
			temp+= "<?=$arrData[$arrDataIndex[$param_index]]["KETERANGAN_UTAMA"]?>";
			temp=fillStringCompletor(temp,"95"," ","");
			var result = splitIntoLines(temp, 90);
			...
			temp=fillStringCompletor(temp,"24"," ",": ");
			temp+= "<?=$arrData[$arrDataIndex[$param_index]]["BUKTI_PENDUKUNG"]?>";
			temp=fillStringCompletor(temp,"68"," ","");
			tempDetil= temp+"Tanggal, <?=dateToPageCheck($arrData[$arrDataIndex[$param_index]]["TGL_TRANS"])?>";
			...
			//KETERANGAN_DETIL, SALDO_VAL_DEBET, SALDO_VAL_KREDIT, SALDO_RP_DEBET, SALDO_RP_KREDIT, KD_VALUTA
			<?
			$data_awal= $arrDataIndex[$param_index];
			$data_akhir= $arrDataIndex[$param_index+1];
			if($data_akhir == "")
				$data_akhir= count($arrData);
			
			for($checkbox_index=$arrDataIndex[$param_index];$checkbox_index<$data_akhir;$checkbox_index++)
			{
			?>
				if(rowPaper == 63)
				{
			...
			?>
				...
				temp=fillStringCompletor(temp,"52"," ","");
				temp+="RP";
				temp=fillStringCompletor(temp,"58"," ","");
				temp+=" ";
				temp=fillStringCompletor(temp,"65"," ","");
			...
			?>
				...
				<?
				$tempKetTambahanUsd= $tempKetTambahanNilaiUsd="";
				if(strtoupper($tempKdValuta) == "IDR"){}
				else
				{
					if($tempSaldoValKredit == 0)
						$tempKetTambahanNilaiUsd= "Nominal USD $ ".$tempSaldoValDebet;
					else
						$tempKetTambahanNilaiUsd= "Nominal USD $ ".$tempSaldoValKredit;
					
					if($tempKeteranganDetil == ""){}
					else
					$tempKetTambahanUsd= "- Ket: ".$tempKeteranganDetil;
				}
				?>
				
				temp= " |";
				temp=fillStringCompletor(temp,"9"," ","");
				temp+= "<?=$tempNamaBukuBesarParent?> <?=$tempNamaSubBantu?> <?=$tempNamaBukuPusat?>";
				temp=fillStringCompletor(temp,"95"," ","");
				//var result = splitIntoLines(temp, 35);
				//var result = splitIntoLines(temp, 58);
				var result = splitIntoLines(temp, 38);
				for (var i = 0; i < result.length; i++)
				{
					if(rowPaper == 63)
					{
				...
				<?
				if($tempKetTambahanUsd == ""){}
				else
				{
				?>
				temp= " |";
				temp=fillStringCompletor(temp,"9"," ","");
				temp+= "<?=$tempKetTambahanNilaiUsd?>";
				temp=fillStringCompletor(temp,"95"," ","");
				//var result = splitIntoLines(temp, 35);
				//var result = splitIntoLines(temp, 58);
				var result = splitIntoLines(temp, 38);
			...
			if(tempFooter > 63)
			{
				...
				temp= " |";
				temp=fillStringCompletor(temp,"37"," ","");
				temp+="JUMLAH MUTASI";
				temp=fillStringCompletor(temp,"52"," ","");
				temp+="RP";
				temp=fillStringCompletor(temp,"58"," ","");
				temp+="<?=numberToIna($arrData[$data_awal]["JML_RP_TRANS"])?>";
				temp=fillStringCompletor(temp,"65"," ","");
				temp=fillStringCompletor(temp,"80"," ","");
				temp+="<?=numberToIna($arrData[$data_awal]["JML_RP_TRANS"])?>";
				temp=fillStringCompletor(temp,"95"," ","|");
				...
				for(var indexRowFooter= rowPaper; indexRowFooter < 62; indexRowFooter++)
				{
			...
			temp=fillStringCompletor(temp,"68"," ","");
			temp+="SURABAYA, <?=dateToPageCheck($arrData[$data_awal]["TGL_TRANS"])?>";
			temp=fillStringCompletor(temp,"95"," ","|");
			var panjangTemp= panjangString(temp);
			//alert(temp+'--'+panjangTemp);
			isiValue+=temp+"\n";
			...
			temp=fillStringCompletor(temp,"35"," ","|");
			temp=fillStringCompletor(temp,"47"," ","|");
			var indexPejabatNama= <?="68".setPanjangJabatanPlusMin($panjangJabatan, $panjangPenandatangan).$tempIndexPejabatNama?>;
			temp=fillStringCompletor(temp,indexPejabatNama," ","");
			temp+="<?=$pejabat?>";
			...
			temp=fillStringCompletor(temp,"47"," ","|");
			temp=fillStringCompletor(temp,indexPejabatNama," ","");
			panjangPenandatangan= indexPejabatNama + panjangPenandatangan;
			...
			temp=fillStringCompletor(temp,"35"," ","|");
			temp=fillStringCompletor(temp,"47"," ","|");
			var indexKeteranganUang= <?="68".setPanjangJabatanPlusMin($panjangJabatan, $panjangKeteranganUang).$tempIndexKeteranganUang?>;
			temp=fillStringCompletor(temp,indexKeteranganUang," ","");
			temp+="<?=$info_keterangan_uang?>";
			var panjangKeteranganUang= panjangString("<?=$info_keterangan_uang?>");
			temp=fillStringCompletor(temp,"95"," ","|");
			var panjangTemp= panjangString(temp);
			//alert(temp+'--'+panjangTemp);
			isiValue+=temp+"\n";
			...
			temp=fillStringCompletor(temp,"47"," ","|");
			temp=fillStringCompletor(temp,indexKeteranganUang," ","");
			panjangKeteranganUang= indexKeteranganUang + panjangKeteranganUang;
			temp=fillStringCompletor(temp,panjangKeteranganUang,"-","");
			temp=fillStringCompletor(temp,"95"," ","|");
			var panjangTemp= panjangString(temp);
			//alert(temp+'--'+panjangTemp);
			isiValue+=temp+"\n";
			...
			temp=fillStringCompletor(temp,"35"," ","|");
			temp=fillStringCompletor(temp,"47"," ","|");
			var indexNamaTerang= <?="68".setPanjangJabatanPlusMin($panjangJabatan, $panjangNamaTerang).$tempIndexNamaTerang?>;
			temp=fillStringCompletor(temp,indexNamaTerang," ","");
			temp+="<?=$info_nama_terang?>";
			var panjangNamaTerang= panjangString("<?=$info_nama_terang?>");
			temp=fillStringCompletor(temp,"95"," ","|");
			var panjangTemp= panjangString(temp);
			//alert(temp+'--'+panjangTemp);
			isiValue+=temp+"\n";
			...
			temp=fillStringCompletor(temp,"25"," ",": ");
			temp+="<?=$arrData[$arrDataIndex[$param_index]]["NO_POSTING"]?>";
			temp=fillStringCompletor(temp,"58"," ","");
			...
			temp=fillStringCompletor(temp,"25"," ",": ");
			temp+="<?=dateToPageCheck($arrData[$arrDataIndex[$param_index]]["TGL_POSTING"])?>";
			temp=fillStringCompletor(temp,"95"," ","|");
			var panjangTemp= panjangString(temp);
			//alert(temp+'--'+panjangTemp);
			isiValue+=temp+"\n";
    ...
    $(document).ready( function () {
	   $('#btnCetak').live('click', function () {
			print();
			//newWindow = window.open('jurnal_pengeluaran_kas_bank_print.php?reqNoBukti=<?=$reqNoBukti?>&reqPejabat=<?=$reqPejabat?>&reqParam1=<?=$reqParam1?>&reqParam2=<?=$reqParam2?>&reqParam3=<?=$reqParam3?>&reqParam4=<?=$reqParam4?>', 'Cetak');
			//newWindow.focus();	
...
<body>
<textarea id="setValueNilai" style="display:none"></textarea>
    ...
    <div id="header">
        <div id="laporan-header">
            <div id="laporan-header-atas" class="dash-bawah">
                ...
                <div><span class="dash-bawah" style="display:inline;padding:0 0 10px 0;">YAYASAN BARUNAWATI BIRU SURABAYA</span><?php if($arrData[$checkbox_index]["NO_REF2"] != '1') {echo '<span style="margin:10px 0 0; padding:0">Auto</span>';} ?></div>
                    <div><br /><br /><br /><span>NO SIUK : <?=$arrData[$checkbox_index]["NO_NOTA"]?></span></div>
                </div>
                <div id="laporan-header-tengah">
                    <div><br /><br /><br /><span class="dash-bawah" style="display:inline;">BUKTI PENGELUARAN KAS-BANK</span></div>
                </div>
                <div id="laporan-header-kanan">
                    ...
                    <div style="position:absolute; bottom:0; right:0;"><span>TANGGAL : <?=dateToPageCheck($arrData[$checkbox_index]["TGL_TRANS"])?></span></div>
                </div>
            </div>
    ...
    <div id="main" style="position:relative;">
        ...
        <div style="float:left; border:0px solid #C66;">
            ...
            <div id="laporan-uraian">
                <div style="float:left; width:100%;"><span>1. Pemegang Kas Harap membayarkan uang sebesar : Rp. <?=numberToIna($arrData[$checkbox_index]["JML_RP_TRANS"])?></span></div>
        ...
        <div id="laporan-isi-area" class="dash-kiri dash-kanan">
            ...
            <div style="display: table;">
				...
				for($checkbox_index=$checkbox_index;$checkbox_index<count($arrData);$checkbox_index++)
				{
					...
					$tempSaldoRpKredit= $arrData[$checkbox_index]["SALDO_RP_KREDIT"];
					$tempKdValuta= $arrData[$checkbox_index]["KD_VALUTA"];
					$tempNoPosting= $arrData[$checkbox_index]["NO_POSTING"];
					$tempTglPosting= $arrData[$checkbox_index]["TGL_POSTING"];
                ?>
                <div id="tabel" style="display: table-row;">            
                    <div style="display: table-cell;"><span><?=$tempNomor?></span></div>
                    <div style="display: table-cell;"><span><?=$tempKdBukuBesar?>&nbsp;&nbsp;&nbsp;<?=$tempKdSubBantu?>&nbsp;&nbsp;&nbsp;<?=$tempKdBukuPusat?>
                    	<br />
                        <?
						$tempKetTambahanUsd= $tempKetTambahanNilaiUsd="";
						if(strtoupper($tempKdValuta) == "IDR"){}
						...
						{
							if($tempSaldoValKredit == 0)
								$tempKetTambahanNilaiUsd= "<br/>Nominal USD $ ".$tempSaldoValDebet;
							else
								$tempKetTambahanNilaiUsd= "<br/>Nominal USD $ ".$tempSaldoValKredit;
							
							if($tempKeteranganDetil == ""){}
							else
							$tempKetTambahanUsd= "<br />- Ket: ".$tempKeteranganDetil;
						}
						?>
						<?=$tempNamaBukuBesarParent?>&nbsp;&nbsp;&nbsp;<?=$tempNamaSubBantu?>&nbsp;&nbsp;&nbsp;<?=$tempNamaBukuPusat.$tempKetTambahanNilaiUsd.$tempKetTambahanUsd?></span>
                    </div>
            ...
            ?>
            <div style="display: table; border-top:1px dashed #000;">
            	<div id="tabel" style="display: table-row;">            
    ...
    <div id="footer">
        <div id="laporan-footer">
            ...
            <div id="laporan-periksa" class="dash-kanan dash-kiri">
                ...
                <div id="laporan-periksa-kanan">
                    <div id="laporan-periksa-penerima">
                        <span>Uang Telah Diterima Oleh :<br /><br /><br /></span>
                        <div><span class="dash-atas" style="display:inline;">Nama Terang</span></div>
                    </div>
                </div>
            ...
            <div id="laporan-keterangan" class=" dash-kiri dash-kanan dash-bawah" style="">
                ...
                <div id="laporan-keterangan-kiri">
                    <span>
                    a. Nomor Posting : <?=$tempNoPosting?><br /><br />
                    b. Tanggal Posting : <?=dateToPageCheck($tempTglPosting)?><br />
                    </span>
                </div>
