...
<head>
    ...
    $(document).ready(function() {
		...
		$('#btnCetakBukti').on('click', function () {
			 newWindow = window.open('pembatalan_pelunasan_jurnal_rpt.php?reqId=<?=$reqId?>', 'Cetak');
			 newWindow.focus();
		...
		$('#btnPosting').on('click', function () {
			...
			{			
				$.getJSON('../json-keuangansiuk/posting_jurnal_aksi_jurnal_rekap.php?reqId=<?=$reqId?>&reqKdSubSis=<?=$kbbt_jur_bb->getField("KD_SUBSIS")?>',
				  function(data){					  
					  alert("Data berhasil diposting.");		
					  document.location.href = 'pembatalan_pelunasan_add_jurnal.php?reqId=<?=$reqId?>';
