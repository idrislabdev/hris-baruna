...
<title>saldo neraca vs bantu</title>
...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bServerSide": true,		
			  "sAjaxSource": "../json-keuangansiuk/saldo_neraca_vs_bantu_json.php?reqPeriode=<?=$reqPeriode?>&reqKodeValuta=IDR",			  
			...
			/* Click event handler */
			  ...
			  $('#btnRincian').on('click', function () {
				  if(anSelectedData == "")
					  return false;				
				  window.parent.OpenDHTML('saldo_neraca_vs_bantu_rincian.php?reqPeriode='+$('#reqPeriode').combobox('getValue')+'&reqKodeValuta=IDR&reqId='+anSelectedId, 'Office Management - Aplikasi Keuangan', '600', '300');	
			  });
			  
			  $('#reqKodeValuta').combobox({
					onSelect: function(param){
						oTable.fnReloadAjax("../json-keuangansiuk/saldo_neraca_vs_bantu_json.php?reqPeriode="+$('#reqPeriode').combobox('getValue')+"&reqKodeValuta="+param.id);
					}
			  });

			  $('#reqPeriode').combobox({
					onSelect: function(param){
						oTable.fnReloadAjax("../json-keuangansiuk/saldo_neraca_vs_bantu_json.php?reqPeriode="+param.id+"&reqKodeValuta="+$('#reqKodeValuta').combobox('getValue'));
					}
			  });			  
			  ...
			  return false;
			   ...
			   $('.vmenu .first_li').on('click',function() {
				  if( $(this).children().size() == 1 ) {
					  if($(this).children().text() == 'Rincian Nota')
					  {
						  $("#btnRincian").click();
					  } 
					  $('.vmenu').hide();
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Monitoring Saldo Piutang Antara Kartu-Neraca</span>
            </div>            
            </td>
        </tr>
    </table>
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul>
            <li>
            <a href="#" id="btnRincian" title="Rincian">Rincian Nota</a>
            </li>        
        </ul>
    </div>
    ...
    <thead>
        <tr>
            <th rowspan="2" width="50px">Kode</th>
            <th rowspan="2" width="150px">Nama Kartu</th>
            <th colspan="2" style="text-align:center">Saldo</th>
            <th rowspan="2" width="50px">Selisih</th>
         </tr>
         <tr>
    ...
    <!--RIGHT CLICK EVENT -->
    <div class="vmenu">
        <div class="first_li"><span>Rincian Nota</span></div> 
    </div>
    <!--RIGHT CLICK EVENT -->     
</div>
</body>
