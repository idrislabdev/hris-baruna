    ...
    <table style="margin-left:17px;">
        ...
        <tr>           
			 ...
			 <td>
                ...
                Nota Ref <input name="reqNoRefNota" id="reqNoRefNota" class="easyui-validatebox" style="width:160px" type="text" value="<?=$tempNoRefNota?>" tabindex="2" onMouseDown="tabindex=2" />
			</td>
            <td>Kurs&nbsp;Valuta</td>
			 <td>
        ...
        <tr>           
			 ...
			 <td>
             	<input id="reqTanggalTransaksi" name="reqTanggalTransaksi" type="text" class="easyui-datebox" data-options="formatter:myformatter,parser:myparser,validType:'date'" value="<?=$tempTanggalTransaksi?>" tabindex="3" onMouseDown="tabindex=3" />
			</td>
            <td>No Faktur Pajak</td>
			 <td>
				<input name="reqNoFakturPajak" id="reqNoFakturPajak" class="easyui-validatebox" style="width:150px" type="text" value="<?=$tempNoFakturPajak?>" tabindex="7" onMouseDown="tabindex=7" />
			</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td valign="top">Keterangan</td>
			 <td>
				<textarea name="reqKeteranganJurnal" id="reqKeteranganJurnal" style="width:400px; height:80px;"  tabindex="8" onMouseDown="tabindex=8"><?=$tempKeteranganJurnal?></textarea>
   			</td>
        </tr>
        </thead>
    </table>
    ...
    <!--<table class="example altrowstable" id="alternatecolor" >-->
        ...
        <tbody class="example altrowstable" id="alternatecolor"> 
          ...
          ?>
          	<script type="text/javascript">
			$(function() {
				$( "#reqBukuBesar<?=$checkbox_index?>" ).autocomplete({ source:'../json-keuangansiuk/buku_besar_combo_v2_json.php?reqNotId=102', 
              ...
              <tr id="node-<?=$i?>">
                  ...
                  <td>
                    <input type="text" id="reqBukuBesar<?=$checkbox_index?>" name="reqBukuBesar[]" value="<?=$kbbt_jur_bb_d_tmp->getField("KD_BUKU_BESAR")?>" style="width:165px;" tabindex="<? $last_tab++; echo $last_tab ?>" onMouseDown="tabindex=<?=$last_tab?>"  />                   
                  	...
                  	<?
                    if($kbbt_jur_bb_d_tmp->getField('SALDO_VAL_DEBET') > 0)
					{
						$kodeAwal = substr($kbbt_jur_bb_d_tmp->getField("KD_BUKU_BESAR"), 0, 1);	
						if($kodeAwal == "1")
						{}
						else
						{
						?>
                        <strong style="color:#FF0004">Posisi debet harus rekening 1</strong>
                        <?
						}
					}
					?>
                  ...
                  <?
                  // echo $kbbt_jur_bb_d_tmp->getField('SALDO_VAL_DEBET');
      //             	if ($$kbbt_jur_bb_d_tmp->getField('SALDO_VAL_DEBET') >='') 
						// {
						// 	if (substr($kbbt_jur_bb_d_tmp->getField("KD_BUKU_BESAR"),0.3 <> "101"))
						// 	{
						// 		echo "Kode buku besar salah.";exit();
						// 	}
						// }
                  ?>
              </tr>
...
$("#reqBuktiPendukung, #reqNoRefNota, #reqNoFakturPajak, #reqPerusahaan, #reqAlamat, #reqKeteranganJurnal").keyup(function(e) {
	forceupper(this)
