...
<head>
	...
	<script type="text/javascript">
		...
		$(function(){
			$('#ff').form({
				url:'../json-keuangansiuk/proses_pelunasan_pembayaran_add.php',
				onSubmit:function(){
				...
				success:function(data){
					...
					document.location.href = 'proses_pelunasan_pembayaran_add.php?reqId='+data[0];
		...
		function OptionSetPembayaran(anSelectedId, anSelectedInv, anSelectedNama)
		{
			 
			document.getElementById('reqNoRef2').value = anSelectedId;
			document.getElementById('reqNoRef3').value = anSelectedInv;
			document.getElementById('reqKeterangan').value = "PELUNASAN " + anSelectedNama;
			$("#alternatecolor").html("");
			$.get("proses_pelunasan_pembayaran_add_template.php?reqId="+anSelectedId, function (data) {
		...
		function openPopup()
		{
			...
			$('#reqNoRef3').keyup(function (e) {
				if(e.which == 120)
				{
					OpenDHTMLPopup('proses_pelunasan_pembayaran_pencarian.php', 'Pencarian Kelas', 950, 600);
					return false;
				}
			});
...
<div id="wadah">
    ...
    <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Tambah Data Pelunasan SPP Mahasiswa STIAMAK</span>
    </div>
    ...
    <table>
        ...
        <tr>           
			 ...
			 <td>
                ...
                <input name="reqNoRef3" id="reqNoRef3" class="easyui-validatebox" style="width:98%;" type="text"  value="<?=$reqNoRef3?>"  />   <!--onKeyDown="openPopup();"-->
        ...
        <tr>
        	<td colspan="4">
            <div>
                <input type="hidden" name="reqDepartemenId" value="16">
    ...
    <script type="text/javascript">
        ...
        async function BindTable(jsondata, tableid) {/*Function used to convert the JSON array to Html Table*/
		
			var arrNRP = new Array();
            ...
            for (var i = 0; i <= jsondata.length; i++) {
				...
				{
					var total = 0;		
					$("table input[id^=reqJumlahBayar]").each(function() {	
						total = total + Number(FormatAngkaNumber($(this).val()));
					});	
				...
				else
				{
					...
					else
					{
						arrNRP.push(nis);
						//alert(nis + ' ' + nilai);
						$.get("proses_pelunasan_pembayaran_stiamak_add_template.php?reqId="+nis+"&reqNilai="+nilai, function (data) {
							$("#alternatecolor").append(data);
						});	
    ...
    <table>
        <tr>
        	<td>Upload data pelunasan mahasiswa <strong>STIAMAK</strong> (<a href="uploads/template_pelunasan.xlsx">download template</a>)</td>
        	<td><input type="file" id="excelfile" /></td>
            <td><input type="button" id="viewfile" value="Export To Table" onclick="ExportToTable()" /></td>
        </tr>
    </table>        
    ...
    <!--<table class="example altrowstable" id="alternatecolor" >-->
        ...
        <tbody class="example altrowstable" id="alternatecolor"> 
          ...
          $kptt_nota_d->selectByParamsPelunasanPembayaranV2(array("A.NO_NOTA"=>$reqId), -1, -1, "", " ORDER BY LINE_SEQ ASC ");
          ...
          ?>
              <tr id="node-<?=$i?>">
                  ...
                  <td>
                    <input type="text" name="reqSisaBayar[]"  style="text-align:right;"  id="reqSisaBayar<?=$checkbox_index?>"  readonly OnFocus="FormatAngka('reqSisaBayar<?=$checkbox_index?>')" OnKeyUp="FormatUang('reqSisaBayar<?=$checkbox_index?>')" OnBlur="FormatUang('reqSisaBayar<?=$checkbox_index?>')" value="<?=numberToIna($kptt_nota_d->getField("SISA"))?>">
                  </td>
                  <td align="center">
                  <label>
