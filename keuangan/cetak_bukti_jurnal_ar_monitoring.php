...
while($kbbr_general_ref_d->nextRow())
{
	...
	$arrJurnal[$index]["NAMA"] = $kbbr_general_ref_d->getField("ID_REF_DATA");
	$index++;
}
			...
			/* Click event handler */
			  ...
			  $('#example tbody').on( 'click', 'tr', function () {
					if ( $(this).hasClass('selected') ) {
					  ...
					  window.parent.frames['mainFrameDetilPop'].location.href = 'cetak_bukti_jurnal_ar_detail.php?reqId='+anSelectedNota;
			  });
			  ...
			  $('#btnCetak').on('click', function () {
				   if(anSelectedData == "")
					  return false;
					  if ($("#reqJenisJurnal").val()=="JKM")
						window.parent.OpenDHTML('cetak_bukti_jkm_penanda_tangan.php?reqNoBukti='+ anSelectedNota, 'Office Management - Aplikasi Keuangan', '600', '300');	
					  else if ($("#reqJenisJurnal").val()=="JKK")
						window.parent.OpenDHTML('cetak_bukti_jkk_penanda_tangan.php?reqNoBukti='+ anSelectedNota, 'Office Management - Aplikasi Keuangan', '600', '300');	
					  else if ($("#reqJenisJurnal").val()=="JPJ")
						window.parent.OpenDHTML('cetak_bukti_jpj_penanda_tangan.php?reqNoBukti='+ anSelectedNota, 'Office Management - Aplikasi Keuangan', '600', '300');	
					  else if ($("#reqJenisJurnal").val()=="JRR")
						window.parent.OpenDHTML('cetak_bukti_jrr_penanda_tangan.php?reqNoBukti='+ anSelectedNota, 'Office Management - Aplikasi Keuangan', '600', '300');	
					  else
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Cetak Bukti Jurnal A/R</span>
            </div>            
            </td>
        </tr>
    </table>
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul>
            ...
            <a href="#" id="btnCetak" title="Preview">Cetak</a>
            <?php /*?><a href="#" id="btnDeleteRow" title="Hapus">Hapus</a><?php */?>
            </li>        
        </ul>
    </div>
    <div style="position: relative; z-index:10">
   <div style="position: absolute; margin-top:5px; margin-left:180px; z-index:9999; font-size:12px;">
        
        Jurnal : 
        <select name="reqJenisJurnal" id="reqJenisJurnal" style="width:80px">
		<?
		for($i=0;$i<count($arrJurnal);$i++)
        {
        ?>
            <option value="<?=$arrJurnal[$i]["JURNAL_ID"]?>"><?=$arrJurnal[$i]["NAMA"]?></option>	
        <?	
        }
        ?>
        </select>
        ...
        &nbsp;
        Approve
        <input id="reqTanggalAwalAproval" name="reqTanggalAwalAproval" class="easyui-datebox" style="width:77px" value="<?=date("d-m-Y")?>" />
        s/d
        <input id="reqTanggalAkhirAproval" name="reqTanggalAkhirAproval" class="easyui-datebox" style="width:77px" value="<?=date("d-m-Y")?>" />
        &nbsp;
        Cetak
        <input id="reqTanggalAwalNotaCetak" name="reqTanggalAwalNotaCetak" class="easyui-datebox" style="width:77px" />
        s/d
        <input id="reqTanggalAkhirNotaCetak" name="reqTanggalAkhirNotaCetak" class="easyui-datebox" style="width:77px" />
    ...
    <thead>
        <tr>
            ...
            <th width="150px">Nilai&nbsp;Transaksi</th>
