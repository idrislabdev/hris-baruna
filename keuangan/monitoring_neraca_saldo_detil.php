...
<title>monitoring neraca saldo detil</title>
...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bPaginate": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-keuangansiuk/monitoring_neraca_saldo_detil_json.php?reqTahun=<?=$reqTahun?>&reqKodeValuta=<?=$reqKodeValuta?>&reqKodeBukuBesar=<?=$reqKodeBukuBesar?>&reqKodeSubBantu=<?=$reqKodeSubBantu?>",
			...
			/* Click event handler */
			  ...
			  $('#btnDebet').on('click', function () {	
			  		if('<?=$reqKodeBukuBesar?>' == '')
					{
						alert('Pilih Buku Besar terlebih dahulu.');
						return false;
					}
					window.parent.OpenDHTMLFullScreen('monitoring_neraca_saldo_lihat.php?reqTipe=D&reqBulan='+anSelectedId+'&reqTahun=<?=$reqTahun?>&reqKodeValuta=<?=$reqKodeValuta?>&reqKodeBukuBesar=<?=$reqKodeBukuBesar?>&reqKodeSubBantu=<?=$reqKodeSubBantu?>', 'Aplikasi Keuangan', '740', '600');	
			  ...
			  $('#btnKredit').on('click', function () {	
					...
					window.parent.OpenDHTMLFullScreen('monitoring_neraca_saldo_lihat.php?reqTipe=K&reqBulan='+anSelectedId+'&reqTahun=<?=$reqTahun?>&reqKodeValuta=<?=$reqKodeValuta?>&reqKodeBukuBesar=<?=$reqKodeBukuBesar?>&reqKodeSubBantu=<?=$reqKodeSubBantu?>', 'Aplikasi Keuangan', '740', '600');	
			  ...
			  return false;
			   ...
			   $('.vmenu .first_li').on('click',function() {
				  if( $(this).children().size() == 1 ) {
					  if($(this).children().text() == 'Debet')
					  {
						  $("#btnDebet").click();
					  }
					  else if($(this).children().text() == 'Kredit')
					  {
						  $("#btnKredit").click();
					  }
					  $('.vmenu').hide();
...
<body style="" onLoad="setValue();">
<div id="begron"><img src="../WEB-INF/images/bg-kanan.jpg" width="100%" height="100%" alt="Smile"></div>
<div id="wadah">
	<div id="judul-halaman">
        <span><img src="../WEB-INF/images/panah-judul.png"> Rincian</span>
    </div>            
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul style="height:16px;">
			<li>
            <a href="#" id="btnDebet" title="Debet">Debet</a>
            <a href="#" id="btnKredit" title="Kredit">Kredit</a>
            </li>
        </ul>
    </div>
    ...
    <!--RIGHT CLICK EVENT -->
    <div class="vmenu">
        <div class="first_li"><span>Debet</span></div>
        <div class="first_li"><span>Kredit</span></div>
    </div>
    <!--RIGHT CLICK EVENT -->     
</div>
</body>
