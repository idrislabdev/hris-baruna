...
<head>
	...
	function updateData(id, departemenid, tanggalid)
	{
		...
		<?php /*?>$.getJSON('../json-simpeg/transaksi_tipe_tools.php?reqId='+id+'&reqTanggal='+$('#'+tanggalid).datebox('getValue')+'&reqDepartemen='+$('#'+departemenid).combobox('getValue'), function (data) 
		{
	...
	function setSimpan(index)
	{
		if($('#reqButton'+index).val() == "Simpan")
		{
			...
			try 
			{
				...
				var reqKode= $('#reqKode'+index).val();
				var reqKeterangan= $('#reqKeterangan'+index).val();
				var reqNama= $('#reqNama'+index).val();
				var reqStatusPostingJurnal= $('#reqStatusPostingJurnal'+index).val();
				var reqStatusPajak= $('#reqStatusPajak'+index).val();
				var reqStatusMaterai= $('#reqStatusMaterai'+index).val();
				var reqKodePajak1= $('#reqKodePajak1'+index).val();
				var reqKodePajak2= $('#reqKodePajak2'+index).val();
		
				setAddStyle(index);
				$('#reqSubmit'+index).val('');
				$('#reqButton'+index).val('Update');
				
				alert(reqRowId+'--'+reqKode+'--'+reqKeterangan+'--'+reqNama+'--'+reqStatusPostingJurnal+'--'+reqStatusPajak+'--'+reqStatusMaterai+'--'+reqKodePajak1+'--'+reqKodePajak2);
				/*$.getJSON('../json-keuangan/setting_auto_jurnal_add_transaksi_tipe_monitoring_data.php?reqId='+id, function (data)
				{
					$.each(data, function (i, SingleElement) {
					});
				});*/
	...
	function addRowDrawTables(tableID) {
		...
		add_label.innerHTML = '<input type="hidden" id="reqTipeTrans'+id_row+'" name="reqTipeTrans['+id_row+']" /><input type="hidden" id="reqSubmit'+id_row+'" name="reqSubmit['+id_row+']" /><input type="button" onClick="setSimpan('+id_row+')" id="reqButton'+id_row+'" value="Simpan" /> <input type="button" onclick="deleteRowDrawTable(\'dataTableRowDinamis\', this)" value="Hapus" />';
	}
...
<?php /*?>updateData('<?=$transaksi_tipe->getField("TRANSAKSI_TIPE_ID")?>', 'ccDepartemen<?=$i?>', 'dd<?=$i?>')<?php */?>
</div>
</body>
</html>