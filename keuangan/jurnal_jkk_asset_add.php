...
else
{
	...
	$tempNoRef2 = $kbbt_jur_bb_tmp->getField("NO_REF2");
	...
	$tempLokasi= $kbbt_jur_bb_tmp->getField("LOKASI_AKTIVA");
...
<head>
    ...
    <script type="text/javascript" src="js/entri_tabel_jurnal_asset.js"></script>
	...
	<script type="text/javascript">
		...
		$(function(){
			$('#ff').form({
				url:'../json-keuangansiuk/jurnal_jkk_asset_add.php',
				onSubmit:function(){
				...
				success:function(data){
					...
					document.location.href = 'jurnal_jkk_asset_add.php?reqId='+data[0];
		...
		{	
			...
			newWindow.focus();	*/		
			OpenDHTMLPopup('jurnal_pengeluaran_kas_bank_penanda_tangan.php?reqNoBukti=<?=$reqId?>', 'Office Management - Aplikasi Penghasilan', '600', '300');			
		}				
...
<div id="wadah">
    ...
    <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Tambah Data Jurnal Pengeluaran Kas Bank (JKK)</span>
    </div>
    ...
    <table style="margin-left:17px;">
        ...
        <tr>           
			 ...
			 <td>
				...
				<input type="hidden" name="tempNoRef2" value="<?php echo $tempNoRef2; ?>">
			</td>
            <td>Kurs&nbsp;Valuta</td>
			 <td>
        ...
        <tr>           
			 ...
			 <td>
                ...
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                <input  type="hidden" id="reqTahun" name="reqTahun" class="easyui-validatebox" data-options="validType:'minLength[4]'" maxlength="4" style="width:100px; background-color:#f0f0f0" value="<?=$tempTahun?>" />
        ...
        <tr>           
            ...
            <td valign="top">Keterangan</td>
			 <td>
				<textarea name="reqKeteranganJurnal" id="reqKeteranganJurnal" style="width:400px; height:80px;" tabindex="7"  onMouseDown="tabindex=7"><?=$tempKeteranganJurnal?></textarea>
   			</td>
        </tr>
        </thead>
    </table>
    ...
    <!--<table class="example altrowstable" id="alternatecolor" >-->
        <thead class="altrowstable">
          <tr>
          	<th colspan="9" style="text-align:left"><strong>Pastikan Lokasi Aktiva &amp; Aktiva sesuai dengan lookup data!!</strong></th>
          </tr>
          <tr>
              ...
              <th style="width:165px;">Lokasi&nbsp;Aktiva</th>
              <th>Aktiva</th>
              <th>Debet</th>
              <th>Kredit</th>
              <th>Aksi</th>
        ...
        <tbody class="example altrowstable" id="alternatecolor"> 
          ...
          ?>
                ...
                $(function() {
                    ...
                    $( "#reqLokasi<?=$checkbox_index?>" ).autocomplete({ source:'../json-keuangansiuk/lokasi_combo_json.php', 
                                                               autoFocus: true 
                                                            }).autocomplete( "instance" )._renderItem = function( ul, item ) {
                                                                return $( "<li>" )
                    ...
                    $( "#reqKeterangan<?=$checkbox_index?>" ).autocomplete({ source:'../json-keuangansiuk/asset_combo_json.php/?reqJenis='+$("#reqBukuBesar<?=$checkbox_index?>").val(), 
                                                               autoFocus: true 
                                                            }).autocomplete( "instance" )._renderItem = function( ul, item ) {
                                                                return $( "<li>" )
                                                              .append( "<a>" + item.label + "<br>"+ "</a>" )
              ...
              <tr id="node-<?=$i?>">
                  ...
                  <td>
                    <input type="text" id="reqLokasi<?=$checkbox_index?>" name="reqLokasi[]" value="<?=$kbbt_jur_bb_d_tmp->getField("LOKASI_AKTIVA")?>" style="width:165px;" <? if($kbbt_jur_bb_d_tmp->getField("DISABLE_BPUSAT") == "DISABLED") {} else { ?> tabindex="<? $last_tab++; echo $last_tab ?>" required onMouseDown="tabindex=<?=$last_tab?>" <? } ?> <?=$kbbt_jur_bb_d_tmp->getField("DISABLE_BPUSAT")?>  />                   	
                  </td>
                  <td>
                    <input type="text" id="reqKeterangan<?=$checkbox_index?>" name="reqKeterangan[]" class="easyui-validatebox" value="<?=$kbbt_jur_bb_d_tmp->getField("KET_TAMBAH")?>" style="width:95%;" tabindex="<? $last_tab++; echo $last_tab ?>" onMouseDown="tabindex=<?=$last_tab?>"/>
                  </td>
                  <td>
              ...
              <tr id="node-<?=$i?>">
                  ...
                  <td>
                    <input type="text" id="reqBukuBesar<?=$checkbox_index?>" name="reqBukuBesar[]" style="width:165px;" tabindex="8" onMouseDown="tabindex=9"  />
                  <td>
                    <input type="text" id="reqKartu<?=$checkbox_index?>" name="reqKartu[]" style="width:165px;" tabindex="9"  onMouseDown="tabindex=10"  />                   	
                  </td>
                  <td>
                    <input type="text" id="reqBukuPusat<?=$checkbox_index?>" name="reqBukuPusat[]"  style="width:165px;" tabindex="10"  onMouseDown="tabindex=11" />                   	
                  </td>
                  <td>
                    <input type="text" id="reqLokasi<?=$checkbox_index?>" name="reqLokasi[]" class="easyui-validatebox" style="width:165px;" tabindex="10" required onMouseDown="tabindex=11" />                   	
                  </td>
                  <td>
                    <input type="text" id="reqKeterangan<?=$checkbox_index?>" name="reqKeterangan[]" class="easyui-validatebox" style="width:95%;" tabindex="13" required onMouseDown="tabindex=13"/>
                  </td>
                  <td>
                    <input type="text" name="reqDebet[]" id="reqDebet<?=$checkbox_index?>" style="text-align:right; width:95%;" OnFocus="FormatAngka('reqDebet<?=$checkbox_index?>')" OnKeyUp="FormatUang('reqDebet<?=$checkbox_index?>'); hitungDebetTotal('dataTableRowDinamis');setTimeout(setCheckBalance, 1000);" OnBlur="FormatUang('reqDebet<?=$checkbox_index?>')" tabindex="14" value="0" />
                  </td>
                  <td>
                    <input type="text" name="reqKredit[]" id="reqKredit<?=$checkbox_index?>" style="text-align:right; width:95%;" OnFocus="FormatAngka('reqKredit<?=$checkbox_index?>')" OnKeyUp="FormatUang('reqKredit<?=$checkbox_index?>'); hitungKreditTotal('dataTableRowDinamis');setTimeout(setCheckBalance, 1000);" OnBlur="FormatUang('reqKredit<?=$checkbox_index?>')" tabindex="15"  value="0" />
                  </td>
                  <td align="center">
                  <label>
                  <a style="cursor:pointer" onclick="deleteRowDrawTablePhp('dataTableRowDinamis', this)"><img src="../WEB-INF/images/delete-icon.png" width="15" height="15" border="0" /></a>
