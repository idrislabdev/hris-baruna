...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-keuangansiuk/monitoring_saldo_piutang_neraca_json.php?reqPeriode=<?=date("Ym")?>&reqKodeValuta=IDR",			  
			...
			/* Click event handler */
			 ...
			 $('#reqPeriode').combobox({
					onSelect: function(param){
						oTable.fnReloadAjax("../json-keuangansiuk/monitoring_saldo_piutang_neraca_json.php?reqPeriode="+param.id+"&reqKodeValuta="+$('#reqKodeValuta').combobox('getValue'));
					}
			  });

			 $('#reqKodeValuta').combobox({
					onSelect: function(param){
						oTable.fnReloadAjax("../json-keuangansiuk/monitoring_saldo_piutang_neraca_json.php?reqPeriode="+$('#reqPeriode').combobox('getValue')+"&reqKodeValuta="+param.id);
					}
			  });
			  			  			  
			  $('#btnListRekapBadanUsaha').on('click', function () {		
				window.parent.OpenDHTML('monitoring_saldo_piutang_neraca_rekap.php?&reqPeriode='+$('#reqPeriode').combobox('getValue')+'&reqKodeValuta='+$('#reqKodeValuta').combobox('getValue'), 'Office Management - Aplikasi Keuangan', '800', '500');				  				  
			  });
			  ...
			  return false;
			   ...
			   $('.vmenu .first_li').on('click',function() {
				  if( $(this).children().size() == 1 ) {
					  if($(this).children().text() == 'List Rekap / Badan Usaha')
					  {
						  $("#btnListRekapBadanUsaha").click();
					  } 
					  $('.vmenu').hide();
					  $('.overlay').hide();
				  }
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Monitoring Saldo Piutang Antara Neraca Dan Waktu</span>
            </div>            
            </td>
        </tr>
    </table>
    ...
    <div style="position: absolute; margin-top:30px; margin-left:176px; z-index:9999; font-size:12px;">
			...
			data-options="
					...
					textField:'text'
			" value="<?=date("Ym")?>">&nbsp;&nbsp;
            Valuta 	<input id="reqKodeValuta" name="reqKodeValuta" class="easyui-combobox" 
			data-options="
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul>
            <li>
            <a href="#" id="btnListRekapBadanUsaha" title="List Rekap / Badan Usaha">List Rekap / Badan Usaha</a>
            </li>        
        </ul>
    </div>
    ...
    <thead>
        <tr>
            <th rowspan="2" class="th_like" width="120px">Badan&nbsp;Usaha</th>
            <th rowspan="2" class="th_like" width="100px">Kartu</th>
            <th rowspan="2" class="th_like" width="250px">Nama&nbsp;Pelanggan</th>
            <th colspan="2" class="th_like" width="150px">Saldo</th>
            <th rowspan="2" class="th_like" width="100px">Selisih</th>
        </tr>
        <tr>
            <th width="100px">Neraca</th>
    ...
    <!--RIGHT CLICK EVENT -->
    <div class="vmenu">
        <div class="first_li"><span>List Rekap / Badan Usaha</span></div> 
    </div>
    <!--RIGHT CLICK EVENT -->     
</div>
</body>
