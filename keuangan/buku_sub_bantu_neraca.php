...
<title>buku sub bantu neraca</title>
...
<script type="text/javascript" charset="utf-8">
    $(document).ready( function () {
			  $('#btnCetak').live('click', function () {
			  	newWindow = window.open('buku_sub_bantu_neraca_rpt.php?reqHalaman='+ $("#reqHalaman").val()+'&reqPeriode='+ $("#reqPeriode").val()+"&reqA="+$("#reqA").val() +"&reqB="+$("#reqB").val()+"&reqC="+$("#reqC").val() +"&reqD="+$("#reqD").val(), 'Cetak');
				newWindow.focus();
			  });
			  
			  $('#btnExcel').live('click', function () {
			  	newWindow = window.open('buku_sub_bantu_neraca_excel.php?reqHalaman='+ $("#reqHalaman").val()+'&reqPeriode='+ $("#reqPeriode").val()+"&reqA="+$("#reqA").val() +"&reqB="+$("#reqB").val()+"&reqC="+$("#reqC").val() +"&reqD="+$("#reqD").val(), 'Cetak');
				newWindow.focus();
			  });
	});
</script>
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Buku Sub Bantu Neraca</span>
            </div>            
            </td>
        </tr>
    </table>
	...
	<table>
			 ...
			 <td>
             	<select id="reqA">
                  ...
                  while($kbbr_buku_besar->nextRow()){?>
                    <option value="<?=$kbbr_buku_besar->getField('KD_BUKU_BESAR')?>"><?=$kbbr_buku_besar->getField('KD_BUKU_BESAR')?></option>
                  <? }?>
                </select> 
             </td>	
             <td>Dari Kartu</td>
			 <td>
             	<select id="reqC" name="reqC">
        		  <option value=""></option>
                <? 
				$kbbr_kartu_tambah  = new KbbrKartuTambah();

				$kbbr_kartu_tambah->selectByParams(array("KD_AKTIF"=>"A"), -1, -1, "", " ORDER BY KD_SUB_BANTU ASC ");
				
				while($kbbr_kartu_tambah->nextRow()){?>
                    <option value="<?=$kbbr_kartu_tambah->getField('KD_SUB_BANTU')?>"><?=$kbbr_kartu_tambah->getField('KD_SUB_BANTU')?></option>
                <? }?>
                </select>  
             </td>				
        </tr>  
        <tr>           
			 ...
			 <td>
             	<select id="reqB">
        		  <option value=""></option>
				   <?
				  $kbbr_buku_besar  = new KbbrBukuBesar();
				  $kbbr_buku_besar->selectByParams(array("A.KD_AKTIF"=>"A"), -1, -1, "", " ORDER BY KD_BUKU_BESAR DESC ");
             ...
             <td>Sampai Kartu</td>
			 <td>
             	<select id="reqD" name="reqD">
        		  <option value=""></option>
                <? 
				$kbbr_kartu_tambah  = new KbbrKartuTambah();

				$kbbr_kartu_tambah->selectByParams(array("KD_AKTIF"=>"A"), -1, -1, "", " ORDER BY KD_SUB_BANTU DESC ");
