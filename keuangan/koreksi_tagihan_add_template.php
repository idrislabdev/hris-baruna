...
$kptt_nota_d->selectDaftarTagihanJPJPembatalanTagihan(array("A.NO_NOTA" => $reqId));
while($kptt_nota_d->nextRow())
{
	if($kptt_nota_d->getField("PREV_NO_NOTA") == "")
	{
		$checkbox_index = rand();
	...
	?>
	  <tr id="node-<?=$i?>">
		  ...
		  <td>
			...
			$kptt_nota_d_siswa->selectByParamsDaftarSiswaBB(array("KODE_BB" => $kptt_nota_d->getField("KD_BUKU_BESAR")), -1, -1, " AND NOT NIS = '".$kptt_nota_d->getField("KD_SUB_BANTU")."' ");
			while($kptt_nota_d_siswa->nextRow())
			{
			?>
				<option value="<?=$kptt_nota_d_siswa->getField("KD_SUB_BANTU")?>"><?=$kptt_nota_d_siswa->getField("NM_SUB_BANTU")?></option>
			<?
			}
			?>
			</select>
          ...
          <input type="hidden" name="reqKlasTrans[]" class="easyui-validatebox" value="<?=$kptt_nota_d->getField("KLAS_TRANS")?>"  style="width:98%" readonly />
          <input type="hidden" name="reqNoUrut[]" class="easyui-validatebox" value="<?=$kptt_nota_d->getField("PREV_NO_NOTA")?>"  style="width:98%" readonly />
		  <label>
