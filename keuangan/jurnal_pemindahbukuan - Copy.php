...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-keuangansiuk/jurnal_pemindahbukuan_json.php?reqPeriode=<?=$reqPeriode?>",
			...
			sDeleteURL: "../json-keuangansiuk/delete.php?reqMode=jurnal_pemindahbukuan",
			sAddHttpMethod: "GET",  
			"aoColumns": [
				{ 	cssclass: "required" },
				{
			...
			/* Click event handler */
			  ...
			  $('#btnEdit').on('click', function () {
				  ...
				  if(anSelectedNoPosting == '')
					  window.parent.OpenDHTML('jurnal_pemindahbukuan_add.php?reqMode=update&reqId='+anSelectedId+'&reqRowId='+anSelectedRowId, 'Office Management - Aplikasi Keuangan', '600', '300');	
			  	  else
				  ...
				  $.getJSON('../json-keuangansiuk/get_posting_json.php?reqTabel=KBBT_JUR_BB&reqId='+anSelectedId,
					function(data){
						if(data.NO_POSTING == "")
						{
							...
							{
								$.getJSON('../json-keuangansiuk/hapus_data_json.php?reqSource=JURNAL_PEMINDAHBUKUAN&reqId='+anSelectedId,
								function(data){
									if(data.HASIL == 1)
									{
			  ...
			  $('#btnCetak').on('click', function () {
				   if(anSelectedData == "")
					  return false;	
						newWindow = window.open('jurnal_pemindahbukuan_rpt.php?reqId='+ anSelectedId, 'Cetak');
						newWindow.focus();	
			  });
			  
			  $("#reqPeriode").change(function() { 
				 oTable.fnReloadAjax("../json-keuangansiuk/jurnal_pemindahbukuan_json.php?reqPeriode="+ $("#reqPeriode").val());
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Jurnal Pemindahbukuan (JKM)</span>
            </div>            
            </td>
        </tr>
    </table>
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul>
            ...
            <li>
            <a href="#" id="btnAdd" onClick="window.parent.OpenDHTML('jurnal_pemindahbukuan_add.php?reqMode=insert', 'Office Management - Aplikasi Keuangan', '600', '300');" title="Tambah">Tambah</a>
