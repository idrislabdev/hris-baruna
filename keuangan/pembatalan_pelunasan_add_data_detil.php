...
<head>
	...
	function addRowDrawTables(tableID) {
        ...
        element2.type = "text";
		element2.name = "reqNoRef["+ id_row +"]";
		element2.id = "reqNoRef"+id_row;
        ...
        var element3= document.createElement("input");
        element3.type = "text";
		element3.name = "reqJatuhTempo["+ id_row +"]";
		element3.id = "reqJatuhTempo"+id_row;
		element3.style.width = '80px';
		element3.className='easyui-datebox';
        column3.appendChild(element3);
		$('#reqJatuhTempo'+id_row).datebox({  
			validType:'date'
		});  
        ...
        element4.type = "text";
		element4.name = "reqJumlahUpper["+ id_row +"]";
		element4.id = "reqJumlahUpper"+id_row;
		element4.style.width = '100px';
		element4.className='easyui-validatebox';
		element4.onfocus = function() {  
			FormatAngka("reqJumlahUpper"+id_row);
		};
		element4.onkeyup = function() {  
			FormatUang("reqJumlahUpper"+id_row);
		};
        ...
        element6.type = "text";
		element6.name = "reqJumlahPiutang["+ id_row +"]";
		element6.id = "reqJumlahPiutang"+id_row;
		element6.onfocus = function() {  
			FormatAngka("reqJumlahPiutang"+id_row);
		};
		element6.onkeyup = function() {  
			FormatUang("reqJumlahPiutang"+id_row);
		};
    ...
    <tbody> 
      ...
      ?>
          <tr id="node-$kapal_sertifikat_kapal->getField('SERTIFIKAT_KAPAL_ID')">
          	  <td><?=$i+1?></td>
              <td>
              	<input type="text" name="reqNoNota[<?=$checkbox_index?>]" style="width:100px" class="easyui-validatebox" value="" />
              </td>
              <td>
              	<input type="text" name="reqNoRef[<?=$checkbox_index?>]" style="width:80px" class="easyui-validatebox" value="" />
              </td>
			  <td>
                <input type="text" name="reqTanggalTransaksi[<?=$checkbox_index?>]" style="width:80px" class="easyui-datebox" data-options="validType:'date'"  value="" />
              </td>
              <td>
                <input type="text" name="reqJatuhTempo[<?=$checkbox_index?>]" style="width:80px" class="easyui-validatebox" value="" />
              </td>
              <td>
              	<input type="text" style="width:100px;" name="reqJumlahUpper[]"  id="reqJumlahUpper<?=$checkbox_index?>"  OnFocus="FormatAngka('reqJumlahUpper<?=$checkbox_index?>')" OnKeyUp="FormatUang('reqJumlahUpper<?=$checkbox_index?>')" OnBlur="FormatUang('reqJumlahUpper<?=$checkbox_index?>')" value="<?=numberToIna($tempNilaiTransaksi)?>">
              </td>
              <td>
              	<input type="text" style="width:100px;" name="reqJumlahPiutang[]"  id="reqJumlahPiutang<?=$checkbox_index?>"  OnFocus="FormatAngka('reqJumlahPiutang<?=$checkbox_index?>')" OnKeyUp="FormatUang('reqJumlahPiutang<?=$checkbox_index?>')" OnBlur="FormatUang('reqJumlahPiutang<?=$checkbox_index?>')" value="<?=numberToIna($tempNilaiTransaksi)?>">
              </td>
              <td>
              	<input type="text" style="width:100px;" name="reqJumlahDibayar[]"  id="reqJumlahDibayar<?=$checkbox_index?>"  OnFocus="FormatAngka('reqJumlahDibayar<?=$checkbox_index?>')" OnKeyUp="FormatUang('reqJumlahDibayar<?=$checkbox_index?>')" OnBlur="FormatUang('reqJumlahDibayar<?=$checkbox_index?>')" value="<?=numberToIna($tempNilaiTransaksi)?>">
              </td>
              <td align="center">
              <label>
