...
$fname = tempnam("/tmp", "lh2_to_excel_".$reqTahunBuku.".xls");
$workbook = & new writeexcel_workbookbig($fname);
$worksheet = &$workbook->addworksheet();

$keuangan_pusat = new KeuanganPusat();

$keuangan_pusat->selectByParamsLH2ToExcel($reqBulan, $reqTahun, $reqDrKodeRekening, $reqSmpKodeRekening, $reqDrBukuBantu, $reqSmpBukuBantu);

$worksheet->set_column(0, 0, 8.43);
$worksheet->set_column(1, 1, 12.00);
...
$worksheet->write(1, 1, "DAFTAR LH2 PERIODE ".getNamePeriode($reqTahunBuku), $text_format);

$worksheet->write(4, 1, "KD_REKENING", $text_format_line_bold);
$worksheet->write(4, 2, "NM_REKENING", $text_format_line_bold);
$worksheet->write(4, 3, "KD_KARTU", $text_format_line_bold);
$worksheet->write(4, 4, "NM_KARTU", $text_format_line_bold);
$worksheet->write(4, 5, "TGL_ENTRY", $text_format_line_bold);
$worksheet->write(4, 6, "NO_BUKTI", $text_format_line_bold);
$worksheet->write(4, 7, "NO_POSTING", $text_format_line_bold);
...
{
	$worksheet->write($row, 1, $keuangan_pusat->getField("KD_REKENING"), $text_format_line_left);
	$worksheet->write($row, 2, $keuangan_pusat->getField("NM_REKENING"), $text_format_line_left);
	$worksheet->write($row, 3, " ".$keuangan_pusat->getField("KD_KARTU"), $text_format_line_left);
	$worksheet->write($row, 4, $keuangan_pusat->getField("NM_KARTU"), $text_format_line_left);
	$worksheet->write($row, 5, getFormattedDate($keuangan_pusat->getField("TGL_ENTRY")), $text_format_line_left);
	$worksheet->write($row, 6, " ".$keuangan_pusat->getField("NO_BUKTI"), $text_format_line_left);
	$worksheet->write($row, 7, " ".$keuangan_pusat->getField("NO_POSTING"), $text_format_line_left);
...
header("Content-Type: application/x-msexcel; name=\"lh2_to_excel_".$reqTahunBuku.".xls\"");
header("Content-Disposition: inline; filename=\"lh2_to_excel_".$reqTahunBuku.".xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
?>