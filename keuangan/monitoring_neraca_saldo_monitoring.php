...
<title>monitoring saldo piutang neraca</title>
...
<style type="text/css" media="screen">
    ...
    @import "../WEB-INF/lib/media/css/demo_table_jui_resize.css";
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-keuangansiuk/monitoring_neraca_saldo_monitoring_json.php",
			...
			/* Click event handler */
			  ...
			  $('#example tbody').on( 'click', 'tr', function () {
					if ( $(this).hasClass('selected') ) {
					  ...
					  anSelectedSubBantu = element[1];
					  window.parent.frames['mainFrameDetilPop'].location.href = 'monitoring_neraca_saldo_detil.php?reqTahun='+$("#reqTahun").val()+'&reqKodeValuta='+$("#reqKodeValuta").val()+'&reqKodeBukuBesar='+anSelectedId+'&reqKodeSubBantu='+anSelectedSubBantu;
			  });

			  $("#reqTahun").change(function() { 
			  	  oTable.fnReloadAjax("../json-keuangansiuk/monitoring_neraca_saldo_monitoring_json.php?reqTahun="+$("#reqTahun").val());
				  window.parent.frames['mainFrameDetilPop'].location.href = 'monitoring_neraca_saldo_detil.php?reqTahun='+$("#reqTahun").val()+'&reqKodeValuta='+$("#reqKodeValuta").val()+'&reqKodeBukuBesar=&reqKodeSubBantu='+anSelectedSubBantu;
			  });			  
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Monitoring Neraca Saldo</span>
            </div>            
            </td>
        </tr>
    </table>   
    ...
    <div style="position: absolute; margin-top:6px; margin-left:176px; z-index:9999; font-size:12px;">
        Tahun: <select name="reqTahun" id="reqTahun">
        		<?
                while($kbbr_thn_buku->nextRow())
				{
				?>
                	<option value="<?=$kbbr_thn_buku->getField("THN_BUKU")?>" <? if($kbbr_thn_buku->getField("THN_BUKU") == date("Y")) { ?> selected <? } ?>><?=$kbbr_thn_buku->getField("THN_BUKU")?></option>
                <?
				}
				?>  
        		</select>&nbsp;&nbsp;
        
        <div id="tollbar-valuta">
        Valuta : <select name="reqKodeValuta" id="reqKodeValuta">
        		<?
        ...
        <input type="hidden" name="reqKodeSubBantu" id="reqKodeSubBantu" style="width:100px">
    </div>
    </div>      
    ...
    <thead>
        <tr>
            <th style="width:100px;">Kode&nbsp;Buku&nbsp;Besar</th>
            <th style="width:100px;">Kartu</th>   
            <th style="width:300px;">Nama</th>   
            <th style="width:100px;">Kode&nbsp;Buku&nbsp;Pusat</th>                                                
        </tr>
    </thead>
    </table> 
    </div> 
