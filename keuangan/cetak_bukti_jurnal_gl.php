...
<title>cetak bukti jurnal gl</title>
...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-keuangansiuk/cetak_bukti_jurnal_gl_json.php?reqPeriode=<?=$reqPeriode?>&reqJurnal=<?=$reqJurnal?>&reqPosting=0&reqTanggalAwalAproval=<?=date("d-m-Y")?>&reqTanggalAkhirAproval=<?=date("d-m-Y")?>",
			...
			/* Click event handler */
			  ...
			  $('#btnRincianJurnal').on('click', function () {
				  if(anSelectedData == "")
					  return false;				
				  window.parent.OpenDHTML('monitoring_jurnal_transaksi_lihat.php?reqId='+anSelectedNota, 'Office Management - Aplikasi Keuangan', '800', '500');	
			  });
			  ...
			  $("#reqJurnal").change(function() { 
				  ...
				  val_posting= $("#reqPosting").val();
					...
					oTable.fnReloadAjax("../json-keuangansiuk/cetak_bukti_jurnal_gl_json.php?reqPeriode="+ val_periode + "&reqJurnal=" + val_jurnal + "&reqPosting=" + val_posting+"&reqTanggalAwalAproval="+ val_tanggal_awal_approval+"&reqTanggalAkhirAproval="+ val_tanggal_akhir_approval);
				 //oTable.fnReloadAjax("../json-keuangansiuk/cetak_bukti_jurnal_gl_json.php?reqPeriode="+ $("#reqPeriode").val() + "&reqJurnal=" + $("#reqJurnal").val() + "&reqPosting=" + $("#reqPosting").val());
			  });
			  ...
			  $('#btnCetak').on('click', function () {
					...
					else
					{
						...
						for(i=0;i<val.length; i++)
						{
						 ...
						 window.parent.OpenDHTML('cetak_bukti_jurnal_gl_penanda_tangan.php?reqId='+ id + "&reqJurnal=" + $("#reqJurnal").val(), 'Office Management - Aplikasi Keuangan', '600', '300');		
						/*
						$.getJSON('../json-keuangansiuk/cetak_bukti_jurnal_gl_cetak.php?reqId='+id,
						  function(data){
							var pop = window.open('cetak_bukti_jurnal_gl_rpt.php?reqId='+ id, 'Cetak');
							pop.focus();					  
...
/*						newWindow = window.open('cetak_bukti_jurnal_gl_rpt.php?reqId='+ anSelectedId, 'Cetak');
						newWindow.focus();	*/				  
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Cetak Bukti Jurnal G/L</span>
            </div>            
            </td>
        </tr>
    </table>
    ...
    <div style="position: absolute; margin-top:30px; margin-left:176px; z-index:9999; font-size:12px;">
         ...
         Jurnal : <select name="reqJurnal" id="reqJurnal">
            ...
            Posting :
            <select name="reqPosting" id="reqPosting">
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul>
            <li>
            <a href="#" id="btnRincianJurnal" title="Edit">Rincian Jurnal</a>
