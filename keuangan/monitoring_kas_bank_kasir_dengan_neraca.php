...
<title>monitoring kas bank kasir dengan neraca</title>
...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-keuangansiuk/monitoring_kas_bank_kasir_dengan_neraca_json.php?reqValuta=IDR&reqPeriode=<?=$reqPeriode?>",			  
			...
			/* Click event handler */
			  ...
			  $('#btnEdit').on('click', function () {
				  if(anSelectedData == "")
					  return false;				
				  window.parent.OpenDHTML('inquiry_jurnal_kas_keluar_belum_posting_add.php?reqMode=update&reqId='+anSelectedId, 'Office Management - Aplikasi Keuangan', '600', '300');	
			  });

				$('#reqPeriode').combobox({
				  onSelect: function(param){
					  if(param.text == 'Semua')
						  oTable.fnReloadAjax("../json-keuangansiuk/monitoring_kas_bank_kasir_dengan_neraca_json.php?reqValuta="+$("#reqValuta").val()+"&reqPeriode=");
					  else
						  oTable.fnReloadAjax("../json-keuangansiuk/monitoring_kas_bank_kasir_dengan_neraca_json.php?reqValuta="+$("#reqValuta").val()+"&reqPeriode="+param.id);
				  }
				});


			  $("#reqValuta").change(function() { 
				 oTable.fnReloadAjax("../json-keuangansiuk/monitoring_kas_bank_kasir_dengan_neraca_json.php?reqValuta="+$("#reqValuta").val()+"&reqPeriode="+$('#reqPeriode').combobox('getValue'));				 	 
			  });			  
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Monitoring Kas-Bank Kasir Dg Neraca (Akuntansi)</span>
            </div>            
            </td>
        </tr>
    </table>
    <div style="position: relative; z-index:10">
    <div style="position: absolute; margin-top:14px; margin-left:176px; z-index:9999; font-size:12px;">
        Periode : <input id="reqPeriode" class="easyui-combotree" name="reqPeriode" data-options="valueField:'id',textField:'text',url:'../json-keuangansiuk/bulan_tahun_combo_json.php?reqMode=bulan'" style="width:200px;" value="<?=$reqPeriode?>" />
        &nbsp;&nbsp;
        Valuta : <select name="reqValuta" id="reqValuta"><option value="IDR">IDR</option><option value="USD">USD</option></select>
    </div>
    </div>      
    ...
    <thead>
        <tr>
            <th rowspan="2" width="180px">Rekening</th>
            <th rowspan="2" width="120px">Bantu</th>
            <th colspan="3" style="text-align:center">Debet</th>
            <th colspan="3" style="text-align:center">Kredit</th>
        </tr>
        <tr>
			<th width="120px">Kasir</th>
            <th width="120px">Neraca</th>
            <th width="120px">Selisih</th>
            <th width="120px">Kasir</th>
            <th width="120px">Neraca</th>
            <th width="120px">Selisih</th>        
        </tr>
