...
<script type="text/javascript" charset="utf-8">
    $(document).ready( function () {
				$('#btnCetak').live('click', function () {
						newWindow = window.open('monitoring_jurnal_transaksi_jkk_rpt.php?reqNoBukti='+ $("#reqNoBukti").val()+'&reqPejabat='+ $("#reqPejabat").val()+'&reqParam1='+ $("#reqParam1").val()+'&reqParam2='+ $("#reqParam2").val()+'&reqParam3='+ $("#reqParam3").val(), 'Cetak');
				    	newWindow.focus();
				});
			  
			   $('#btnExcel').live('click', function () {
					newWindow = window.open('monitoring_jurnal_transaksi_jkk_excel.php?reqNoBukti='+ $("#reqNoBukti").val()+'&reqPejabat='+ $("#reqPejabat").val()+'&reqParam1='+ $("#reqParam1").val()+'&reqParam2='+ $("#reqParam2").val()+'&reqParam3='+ $("#reqParam3").val(), 'Cetak');
					newWindow.focus();	
...
<div id="wadah">
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
      <td class=""><div id="judul-halaman"> <span><img src="../WEB-INF/images/panah-judul.png"> Bukti Pengeluaran Kas - Bank </span> </div></td>
    </tr>
  </table>
  <table>
    <tr>
      <td>Nomor Bukti</td>
      <td><input name="reqNoBukti" id="reqNoBukti" class="easyui-validatebox" style="width:170px" type="text" readonly value="<?=$reqNoBukti?>" /></td>
    </tr>
    <tr>
      <td>Penandatangan</td>
      <td><select id="reqPejabat">
          <?
                    for($i=0;$i<count($officer);$i++)
					{
					?>
          <option value="<?=$officer[$i]["NM_KET1"]."-".$officer[$i]["NM_KET3"]?>" <? if($officer[$i]["KD_TABEL"] == "KACAB") { ?> selected <? } ?>>
          <?=$officer[$i]["NM_KET1"]?>
          </option>
          <?	
					}
    ...
    <tr>
      <td>Pemeriksa-1</td>
      <td><select name="reqParam1" id="reqParam1">
    ...
    <tr>
      <td>Pemeriksa-2</td>
      <td><select name="reqParam2" id="reqParam2">
    ...
    <tr>
      <td>Pemeriksa-3</td>
      <td><select name="reqParam3" id="reqParam3">
    ...
    <tr>
      <td colspan="3"><input type="button" id="btnCetak" value="Cetak">
        <input type="button" id="btnExcel" value="Excel"></td>
    </tr>
  </table>
</div>
</body>
