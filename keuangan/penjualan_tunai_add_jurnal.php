...
<head>
    ...
    $(document).ready(function() {
		...
		$('#btnCetakBukti').on('click', function () {
			 ...
			 newWindow = window.open('penjualan_tunai_jurnal_penanda_tangan.php?reqNoBukti=<?=$reqId?>', 'Cetak', 'resizable=1,scrollbars=yes,width=' + 500 + 
				',height=' + 500 + 
				',left=' + centerWidth + 
				',top=' + centerHeight);
			 ...
			 /*newWindow = window.open('penjualan_tunai_jurnal_penanda_tangan.php?reqNoBukti=<?=$reqId?>', 'Cetak');
			 newWindow.focus();*/
		...
		$('#btnPosting').on('click', function () {
			...
			{			
				$.getJSON('../json-keuangansiuk/posting_jurnal_aksi.php?reqId=<?=$reqId?>&reqKdSubSis=<?=$kbbt_jur_bb->getField("KD_SUBSIS")?>',
				  function(data){					  
					  alert("Data berhasil diposting.");		
					  document.location.href = 'penjualan_tunai_add_jurnal.php?reqId=<?=$reqId?>';
...
<div id="wadah">
    ...
    <table id="table-gaya" class="example altrowstable" id="alternatecolor" >
    <thead>
      <tr>
    ...
    <?
    if($kbbt_jur_bb->getField("NO_POSTING") == "")
	{
	?>
