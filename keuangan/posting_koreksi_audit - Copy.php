...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-keuangansiuk/posting_koreksi_audit_json.php?reqJurnal=<?=$reqJurnal?>&reqTanggalAwalAproval=<?=date("d-m-Y")?>&reqTanggalAkhirAproval=<?=date("d-m-Y")?>",
			...
			sDeleteURL: "../json-keuangansiuk/delete.php?reqMode=posting_koreksi_audit",
			sAddHttpMethod: "GET",  
			"aoColumns": [
				{ 	cssclass: "required" },
				{
			...
			/* Click event handler */
			  ...
			  $('#btnEdit').on('click', function () {
				  if(anSelectedData == "")
					  return false;				
				  window.parent.OpenDHTML('posting_koreksi_audit_add.php?reqMode=update&reqId='+anSelectedId, 'Office Management - Aplikasi Keuangan', '600', '300');	
			  });
			  ...
			  $("#reqJurnal").change(function() { 
			     val_jurnal= $("#reqJurnal").val();
			  	 ...
			  	 if($('#reqTanggalAkhirAproval').datebox('getValue').length == 10)
					...
					oTable.fnReloadAjax("../json-keuangansiuk/posting_koreksi_audit_json.php?reqJurnal=" + val_jurnal + "&reqTanggalAwalAproval="+ val_tanggal_awal_approval+"&reqTanggalAkhirAproval="+ val_tanggal_akhir_approval);
				 //oTable.fnReloadAjax("../json-keuangansiuk/posting_koreksi_audit_json.php?reqJurnal="+ $("#reqJurnal").val());
			  });
...
<div id="wadah">
    ...
    Tanggal dari&nbsp;&nbsp;
    <input id="reqTanggalAwalAproval" name="reqTanggalAwalAproval" class="easyui-datebox" style="width:80px" />
    &nbsp;sampai&nbsp;&nbsp;
    <input id="reqTanggalAkhirAproval" name="reqTanggalAkhirAproval" class="easyui-datebox" style="width:80px" />   &nbsp;&nbsp;   
     Jenis Jurnal : <select name="reqJurnal" id="reqJurnal">
        		  <?
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul>
            <?php /*?><a href="#" id="btnAdd" onClick="window.parent.OpenDHTML('posting_koreksi_audit_add.php?reqMode=insert', 'Office Management - Aplikasi Keuangan', '600', '300');" title="Tambah">Tambah</a>
