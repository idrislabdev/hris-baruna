...
<title>info transaksi online</title>
...
<style type="text/css" media="screen">
	...
	var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 500,
	  ...
	  "bServerSide": true,		
	  "sAjaxSource": "../json-keuangansiuk/info_transaksi_online_json.php",			  
	...
	$('#reqKode').keydown(function (event) {
		if(event.keyCode == 13){
			...
			$.getJSON("../json-keuangansiuk/get_info_transaksi_online_summary_json.php?reqPeriode="+ $("#reqPeriode").combobox("getValue") +"&reqId="+kode+ "&reqPelunasan=" + $("#reqPelunasan").val(),
			function(data){
				  $("#jumlahRP").text(data.IDR)
				  $("#jumlahUSD").text(data.USD)
			});
			oTable.fnReloadAjax("../json-keuangansiuk/info_transaksi_online_json.php?reqPeriode="+ $("#reqPeriode").combobox("getValue") +"&reqId="+kode+ "&reqPelunasan=" + $("#reqPelunasan").val());
			return false;
		}	
	  ...
	  $('#btnCetak').on('click', function () {
			window.parent.OpenDHTML('info_transaksi_online_penanda_tangan.php?reqPeriode='+ $("#reqPeriode").combobox("getValue") +'&reqKode='+$("#reqKode").val()+ '&reqPelunasan=' + $("#reqPelunasan").val(), 'Office Management - Aplikasi Keuangan', '600', '300');
	  });
	  ...
	  $("#reqPelunasan").change(function() { 
		 oTable.fnReloadAjax("../json-keuangansiuk/info_transaksi_online_json.php?reqPeriode="+ $("#reqPeriode").combobox("getValue") +"&reqId=" + $("#reqKode").val() + "&reqPelunasan=" + $("#reqPelunasan").val());
		 $.getJSON("../json-keuangansiuk/get_info_transaksi_online_summary_json.php?reqPeriode="+ $("#reqPeriode").combobox("getValue") +"&reqId="+ $("#reqKode").val() +"&reqPelunasan=" + $("#reqPelunasan").val(),
	 	 function(data){
			  $("#jumlahRP").text(data.IDR)
			  $("#jumlahUSD").text(data.USD)
		 });	
...
function OptionSet(id, nama,alamat, npwp, badan_usaha){
	...
	$.getJSON("../json-keuangansiuk/get_info_transaksi_online_summary_json.php?reqPeriode="+ $("#reqPeriode").combobox("getValue") +"&reqId="+id+ "&reqPelunasan=" + $("#reqPelunasan").val(),
	function(data){
		  $("#jumlahRP").text(data.IDR)
		  $("#jumlahUSD").text(data.USD)
	});				
	oTable.fnReloadAjax("../json-keuangansiuk/info_transaksi_online_json.php?reqPeriode="+ $("#reqPeriode").combobox("getValue") +"&reqId=" + id + "&reqPelunasan=" + $("#reqPelunasan").val());	
}	
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Daftar Transaksi Pelanggan</span>
            </div>            
            </td>
        </tr>
    </table>
    <div style="position: relative; z-index:10">
    <div style="position: absolute; margin-top:27px; margin-left:186px; z-index:9999; font-size:12px;">
           	Pelanggan <input id="reqKode" name="reqKode" style="width:80px;">&nbsp;&nbsp;
            <input id="reqNama" name="reqNama" style="width:200px;" readonly>
            &nbsp;&nbsp;
            Pelunasan 
            <select name="reqPelunasan" id="reqPelunasan">
            	<option value="2">Semua</option>
            	<option value="1">Lunas</option>
            	<option value="0">Belum</option>
			</select>     
            &nbsp;&nbsp;
            Periode : <input id="reqPeriode" class="easyui-combobox" name="reqPeriode" data-options="valueField:'id',textField:'text',url:'../json-keuangansiuk/tahun_bulan_combo_json.php'" style="width:140px;" value="<?=$reqPeriode?>" />                            
    </div>
    </div>     
    ...
    <tfoot>
    	<tr>
			...
			<td><label id="jumlahRP" style="font-weight:bold">0</label></td>            
        </tr>
    	<tr>
        	<td colspan="5" style="text-align:right; font-weight:bold">Jumlah USD</td>
			<td><label id="jumlahUSD" style="font-weight:bold">0</label></td>            
        </tr>
    </tfoot>
    </table> 
    </div> 
