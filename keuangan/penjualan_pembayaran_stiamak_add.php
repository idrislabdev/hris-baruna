...
if($reqId == "")
{
	$reqPeriodeSPP = date("mY");
	$reqPeriodeSPP = date('mY', strtotime("-1 month", strtotime($reqPeriodeSPP)));
	$reqMode = "insert";	
	$tempKursValuta = 1;
	$tempKursPajak = 1;
	$tempTahun = date("Y");
...
else
{
	...
	$reqPeriodeSPP = $kptt_nota->getField("PERIODE_PEMBAYARAN");
	...
	$reqHapusId = $kptt_nota->getField("NO_EKSPEDISI");
...
<head>
	...
	<script type="text/javascript">
		...
		$(function(){
			$('#ff').form({
				url:'../json-keuangansiuk/penjualan_pembayaran_stiamak_add.php',
				onSubmit:function(){
				...
				success:function(data){
					...
					document.location.href = 'penjualan_pembayaran_stiamak_add.php?reqId='+data[0];
		...
		function openPopup(tipe)
		{
			...
			$('#reqDepartemenKelas').keyup(function (e) {
				if(e.which == 120)
				{
					if(tipe == "PELANGGAN")
					{					
						OpenDHTMLPopup('departemen_kelas_stiamak_pencarian.php', 'Pencarian Kelas', 950, 600);
					}
					return false;
				}
			});
		...
		function OptionSetKelas(anSelectedDepId, anSelectedDepKelasId, anSelectedNama, anSelectedJumlah){
			...
			document.getElementById('reqKeterangan').value = 'PIUTANG SPP ' + anSelectedNama + ' ' + $("#reqPeriodeSPP option:selected").text();
			
			$("#alternatecolor").html("");
			$('#reqHapusId').val("");
			$.get("penjualan_pembayaran_stiamak_template.php?reqNota=<?=$reqId?>&reqId="+anSelectedDepKelasId+"&reqPeriodeSPP="+$("#reqPeriodeSPP").val(), function (data) {
...
<div id="wadah">
    ...
    <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Tambah Data Transaksi Penagihan SPP Mahasiswa STIAMAK</span>
    </div>
    ...
    <table style="margin-left:17px;">
        ...
        <tr>           
			 ...
			 <td>
                ...
                &nbsp;&nbsp;
				Periode SPP
             	<select id="reqPeriodeSPP" name="reqPeriodeSPP">
                	<?
					$occDate=date("mY");
                    for($i=3;$i>=0;$i--)
					{
						$occDate= date('mY', strtotime("-$i month", strtotime($occDate)));
					?>
                    	<option value="<?=$occDate?>" <? if($occDate == $reqPeriodeSPP) { ?> selected <? } ?> ><?=getNamePeriode($occDate)?></option>
                    <?	
					}
					?>
                </select>    
        ...
        <tr>  
			...
			<td>  
                ...
                <input name="reqDepartemenKelas" id="reqDepartemenKelas" class="easyui-validatebox" style="width:70%;" type="text" readonly value="<?=$reqDepartemenKelas?>" onKeyDown="openPopup('PELANGGAN');" />&nbsp;<span style="color:#ff0000;" >Tekan Tombol (F9)</span>
			</td>         
             <!-- <td>Pelanggan</td>
			 <td>
    ...
    <div style="margin-left:50px">
        ...
        <input type="hidden" id="reqHapusId" name="reqHapusId" value="<?=$reqHapusId?>">
    ...
    <!--<table class="example altrowstable" id="alternatecolor" >-->
        ...
        <tbody class="example altrowstable" id="alternatecolor"> 
              ...
              $checkbox_index = 0;
         	  $last_tab = 12;       
              $kptt_nota_d->selectByParamsJpjPembayaran(array("NO_NOTA"=>$reqId), -1, -1, "", " ORDER BY MPLG_KODE ASC ");
			  ...
			  <input type="hidden" name="reqKdBukuPusat[]" value="<?=$kptt_nota_d->getField("KD_BUKU_PUSAT")?>" id="reqKdBukuPusat<?=$checkbox_index?>" />
              <input type="hidden" name="reqDK[]" id="reqDK<?=$checkbox_index?>" value="<?=$kptt_nota_d->getField("KD_D_K")?>">
              <a style="cursor:pointer" onclick="hapusRow('<?=$kptt_nota_d->getField("MPLG_KODE")?>'); $(this).parent().parent().parent().remove(); hitungUlang();"><img src="../WEB-INF/images/delete-icon.png" width="15" height="15" border="0" /></a>
        ...
        <tfoot class="altrowstable">
        	<tr style="background:#f1f4fc">
            	...
            	<td><input type="text" id="reqJumlahTrans" name="reqJumlahTrans" style="text-align:right; background-color:#f0f0f0" readonly value="<?=numberToIna($temp_jml_trans)?>" />
                	<input type="hidden" id="reqJumlahPajak" name="reqJumlahPajak" style="text-align:right; background-color:#f0f0f0" readonly value="<?=numberToIna(dotToComma($temp_jml_pajak))?>" />&nbsp;
                    </td>
            	<td>&nbsp;</td>
            </tr>
        </tfoot>
    ...
    <div>
    	<p style="text-align: right; padding-right: 15px; color:red;" ><img src="../WEB-INF/images/delete-icon.png" width="15" height="15" border="0" /> untuk menghapus data siswa yang tidak ditagihkan SPP</p>
    </div>
    </form>
</div>
<script>
...
{
	$('#reqHapusId').val(id + ';' + $('#reqHapusId').val());
