...
<title>pembatalan sudah cetak</title>
...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-keuangansiuk/pembatalan_sudah_cetak_json.php",			  
			...
			/* Click event handler */
			  ...
			  $('#btnView').on('click', function () {
				  if(anSelectedData == "")
					  return false;				
				  window.parent.OpenDHTML('pembatalan_sudah_cetak_view.php?reqMode=update&reqId='+anSelectedId, 'Office Management - Aplikasi Keuangan', '600', '300');	
			  });
			  ...
			  $('#btnEdit').on('click', function () {
				  ...
				  if(anSelectedNoPosting == '')
				  	window.parent.OpenDHTML('pembatalan_sudah_cetak_add.php?reqMode=update&reqId='+anSelectedId, 'Office Management - Aplikasi Keuangan', '600', '300');	
			  	  else
				  ...
				  $.getJSON('../json-keuangansiuk/get_posting_json.php?reqTabel=KPTT_NOTA&reqId='+anSelectedId,
					function(data){
						if(data.NO_POSTING == "")
						{
							...
							{
								$.getJSON('../json-keuangansiuk/hapus_data_json.php?reqSource=PEMBATALAN_SUDAH_CETAK&reqId='+anSelectedId,
								function(data){
									if(data.HASIL == 1)
									{
			  ...
			  $('#btnScreen').on('click', function () {
				  if(anSelectedData == "")
					  return false;	
					window.parent.OpenDHTML('pembatalan_sudah_cetak_nota_penanda_tangan.php?reqNoBukti='+anSelectedId, 'Office Management - Aplikasi Keuangan', '600', '300');
			  });
			  ...
			  return false;
			   ...
			   $('.vmenu .first_li').on('click',function() {
				  if( $(this).children().size() == 1 ) {
					  ...
					  else if($(this).children().text() == 'Screen')
					  {
						  $("#btnScreen").click();
					  }
					  else if($(this).children().text() == 'Printer')
					  {
						  $("#btnPrinter").click();
					  }
...
<div id="wadah">
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul>
            <li>
            <a href="#" id="btnAdd" onClick="window.parent.OpenDHTML('pembatalan_sudah_cetak_add.php?reqMode=insert', 'Office Management - Aplikasi Keuangan', '600', '300');" title="Tambah">Tambah</a>
            <a href="#" id="btnEdit" title="Edit">Ubah</a>
            <a href="#" id="btnView" title="View" style="display:none">View</a>
            <a href="#" id="btnScreen" title="Screen">Screen</a>
            <a href="#" id="btnPrinter" title="Printer">Printer</a>
    ...
    <div class="vmenu">
        ...
        <div class="first_li"><span>Screen</span></div>
        <div class="first_li"><span>Printer</span></div>
