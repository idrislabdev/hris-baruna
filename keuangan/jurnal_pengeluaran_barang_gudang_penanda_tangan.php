...
<script type="text/javascript" charset="utf-8">
    ...
    $(document).ready( function () {
				$('#btnCetak').live('click', function () {
					/*$.getJSON('jurnal_pengeluaran_barang_gudang_rpt.php?reqNoBukti='+ $("#reqNoBukti").val()+'&reqPejabatKiri='+ $("#reqPejabatKiri").val()+'&reqPejabatTengah='+ $("#reqPejabatTengah").val()+'&reqPejabatKanan='+ $("#reqPejabatKanan").val(),
					function(data){
				  	...
				  	newWindow = window.open('jurnal_pengeluaran_barang_gudang_rpt.php?reqNoBukti='+ $("#reqNoBukti").val()+'&reqPejabatKiri='+ $("#reqPejabatKiri").val()+'&reqPejabatTengah='+ $("#reqPejabatTengah").val()+'&reqPejabatKanan='+ $("#reqPejabatKanan").val(), 'Cetak');
				  	newWindow.focus();				  
				 	window.parent.divwin.close();	
				});
			  
			   $('#btnExcel').live('click', function () {
					newWindow = window.open('jurnal_pengeluaran_barang_gudang_excel.php?reqNoBukti='+ $("#reqNoBukti").val()+'&reqPejabat='+ $("#reqPejabat").val()+'&reqParam1='+ $("#reqParam1").val()+'&reqParam2='+ $("#reqParam2").val()+'&reqParam3='+ $("#reqParam3").val()+'&reqParam4='+ $("#reqParam4").val(), 'Cetak');
					newWindow.focus();	
				 	window.parent.divwin.close();		
			  });
			  
			  $('#btnCetakPreview').live('click', function () {
					OpenDHTMLPopup('jurnal_pengeluaran_barang_gudang_preview.php?reqNoBukti='+ $("#reqNoBukti").val()+'&reqPejabat='+ $("#reqPejabat").val()+'&reqParam1='+ $("#reqParam1").val()+'&reqParam2='+ $("#reqParam2").val()+'&reqParam3='+ $("#reqParam3").val()+'&reqParam4='+ $("#reqParam4").val(), 'Rincian Jurnal', 900, 600)	
			  });
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Jurnal Pengeluaran Barang Gudang (JPP)</span>
            </div>            
            </td>
        </tr>
    </table>
	...
	<table>
        ...
        <tr>           
			 ...
			 <td>
             	<select id="reqPejabat">
					...
					{
					?>
                    	<option value="<?=$officer[$i]["NM_KET1"].'-'.$officer[$i]["NM_KET3"]?>" <? if($officer[$i]["KD_TABEL"] == "ASSMANKEU") { ?> selected <? } ?>><?=$officer[$i]["NM_KET1"]?></option>
                    <?	
					}
					?>
                </select>
        ...
        <tr>
            ...
            <td>
                ...
                <input type="hidden" id="reqParam3" />
                <input type="hidden" id="reqParam4" />
            </td>
        </tr>
        <tr>
        	<td colspan="3"><input type="button" id="btnCetakPreview" onClick="" value="Preview"> 
            <?php /*?><input type="button" id="btnExcel" value="Excel"><?php */?></td>
        </tr>      
    </table>    	   
    </form>
</div>
