...
$jurnal->selectByParams();
while($jurnal->nextRow())
{
	$arrJurnal[$index]["JURNAL_ID"] = $jurnal->getField("JURNAL_ID");
	$arrJurnal[$index]["NAMA"] = $jurnal->getField("NAMA");
	$index++;
}
...
<title>penjualan piutang transaksi cetak bukti jurnal</title>
...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-keuangan/penjualan_piutang_transaksi_cetak_bukti_jurnal_json.php?reqJenisJurnal=<?=$tempJenisJurnal?>",
			...
			/* Click event handler */
			  ...
			  $('#btnEdit').on('click', function () {
				  if(anSelectedData == "")
					  return false;				
				  window.parent.OpenDHTML('penjualan_piutang_transaksi_cetak_bukti_jurnal_detil_monitoring.php?reqMode=update&reqId='+anSelectedId, 'Office Management - Aplikasi Keuangan', '600', '300');	
			  });
			  ...
			  $("#reqJenisJurnal").change(function() { 
				 oTable.fnReloadAjax("../json-keuangan/penjualan_piutang_transaksi_cetak_bukti_jurnal_json.php?reqJenisJurnal="+ $("#reqJenisJurnal").val());
			  });
			  ...
			  $('#reqTanggalAwalAproval').datebox({
				onChange:function(newValue,oldValue){
					if(newValue.length == 10)
						alert('asdsa');
					/*var val = $('#reqRak').combotree('getValue');
					
					oTable.fnReloadAjax("../json/arsip_json.php?reqTipePegawai=" + newValue + "&reqRak=" + val);*/
				}
			  });
...
<div id="wadah">
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul>
            <li>
            <a href="#" id="btnAdd" onClick="window.parent.OpenDHTML('penjualan_piutang_transaksi_cetak_bukti_jurnal_add.php?reqMode=insert', 'Office Management - Aplikasi Keuangan', '600', '300');" title="Tambah">Tambah</a>
            <a href="#" id="btnEdit" title="Edit">Setting</a>
   ...
   <div style="position: absolute; margin-top:8px; margin-left:180px; z-index:9999; font-size:12px;">
    	Transaksi Approval
        <input id="reqTanggalAwalAproval" name="reqTanggalAwalAproval" class="easyui-datebox" />
        s/d
        <input id="reqTanggalAkhirAproval" name="reqTanggalAkhirAproval" class="easyui-datebox" />
        &nbsp;&nbsp;
        Jenis Jurnal : 
        <select name="reqJenisJurnal" id="reqJenisJurnal">
		<?
        ...
        Cetak Nota Tanggal
        <input id="reqTanggalAwalNotaCetak" name="reqTanggalAwalNotaCetak" class="easyui-datebox" />
        s/d
        <input id="reqTanggalAkhirNotaCetak" name="reqTanggalAkhirNotaCetak" class="easyui-datebox" />
    </div>
    </div>
    ...
    <!--RIGHT CLICK EVENT -->
    <div class="vmenu">
        <div class="first_li"><span>Setting</span></div>
