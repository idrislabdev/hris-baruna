...
$kbbr_no_nota->selectByParamsPeriode();

while($kbbr_no_nota->nextRow())
{
	$arrPeriode[] = $kbbr_no_nota->getField("KD_PERIODE");	
}
...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-keuangansiuk/nota_penomoran_json.php?reqPeriode=<?=$reqPeriode?>",			  
			...
			/* Click event handler */
			  ...
			  $("#reqPeriode").change(function() { 
				 oTable.fnReloadAjax("../json-keuangansiuk/nota_penomoran_json.php?reqPeriode="+ $("#reqPeriode").val());
			  ...
			  $('#btnEdit').on('click', function () {
				  if(anSelectedData == "")
					  return false;				
				  window.parent.OpenDHTML('nota_penomoran_add.php?reqMode=update&reqId='+anSelectedId+'&reqPeriode='+ $("#reqPeriode").val(), 'Office Management - Aplikasi Keuangan', '600', '300');	
			  });
			  ...
			  $('#btnHapus').on('click', function () {
				...
				if ( confirm( "Apakah anda yakin, menghapus data ini ?" ) ) {
					$.getJSON('../json-keuangansiuk/delete.php?reqMode=nota_penomoran&id='+anSelectedId+'&reqRowId='+ $("#reqPeriode").val(), function (data) 
					{
			  ...
			  $('#btnCopy').on('click', function () {
				if ( confirm( "Apakah anda yakin ingin menyalin data periode "+ $("#reqPeriode").val() +" ke periode "+(Number($("#reqPeriode").val())+1)+" ?" ) ) {
					$.getJSON('../json-keuangansiuk/nota_penomoran_copy.php?reqTahunAwal='+$("#reqPeriode").val()+'&reqTahunAkhir='+(Number($("#reqPeriode").val())+1), 
					function(data){
						if(data.STATUS == "1")
						{
							alert("Data berhasil disalin.");
							document.location.href = 'nota_penomoran.php';
						}
						else
							alert(data.STATUS);
					});
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Referensi Nomor Transaksi</span>
            </div>            
            </td>
        </tr>
    </table>
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul>
            <li>
            <a href="#" id="btnAdd" onClick="window.parent.OpenDHTML('nota_penomoran_add.php?reqMode=insert&reqPeriode='+$('#reqPeriode').val(), 'Office Management - Aplikasi Keuangan', '600', '300');" title="Tambah">Tambah</a>
            <a href="#" id="btnEdit" title="Edit">Ubah</a>
            <a href="#" id="btnHapus" title="Hapus">Hapus</a>
            </li>    
            <li>
            <a href="#" id="btnCopy" title="Copy">Copy</a>
            </li>    
        </ul>
    </div>
    ...
    <thead>
        <tr>
            ...
            <th width="100px">No Start</th>
            <th width="100px">No Stop</th>
            <th width="100px">Awalan</th>
            <th width="100px">No Terpakai</th>
            <th width="200px">Keterangan</th>
            <th width="50px">Status</th>
            <th width="150px">Last Update Date</th>
            <th width="150px">Last Update By</th>                                          
        </tr>
    </thead>
    </table> 
    </div> 
