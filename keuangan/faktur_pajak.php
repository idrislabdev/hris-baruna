...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-keuangansiuk/faktur_pajak_json.php",			  
			...
			/* Click event handler */
			  ...
			  $('#example tbody').on( 'click', 'tr', function () {
					if ( $(this).hasClass('selected') ) {
					  ...
					  anSelectedJumlah = element[1];
			  });
			  
			  $('#btnEdit').on('click', function () {
				  if(anSelectedData == "")
					  return false;			
					 if(anSelectedJumlah == 0)
					  window.parent.OpenDHTML('faktur_pajak_add.php?reqMode=update&reqId='+anSelectedId, 'Office Management - Aplikasi Keuangan', '600', '300');	
				  else
				  {
				  	 alert("Nomor faktur pajak sudah dipergunakan. anda tidak diperbolehkan untuk mengubah data.");
				  }
			  });
			  ...
			  $('#btnDetil').on('click', function () {
				  if(anSelectedData == "")
					  return false;			
				  window.parent.OpenDHTML('faktur_pajak_lihat.php?reqId='+anSelectedId, 'Office Management - Aplikasi Keuangan', '600', '300');	
			  ...
			  $('#btnFile').on('click', function () {
				  if(anSelectedData == "")
					  return false;			
				  window.parent.OpenDHTML('image_script.php?reqId='+anSelectedId, 'Office Management - Aplikasi Keuangan', '600', '300');	
			  ...
			  $('#btnHapus').on('click', function () {
				...
				if(anSelectedJumlah == 0)
				{
					if ( confirm( "Apakah anda yakin, menghapus data ini ?" ) ) {
						$.getJSON('../json-keuangansiuk/delete.php?reqMode=faktur_pajak&id='+anSelectedId, function (data) 
						{
			  ...
			  return false;
			   ...
			   $('.vmenu .first_li').on('click',function() {
				  if( $(this).children().size() == 1 ) {
					  ...
					  else if($(this).children().text() == 'Detil')
					  {
						  $("#btnDetil").click();
					  }
					  else if($(this).children().text() == 'File')
					  {
						  $("#btnFile").click();
					  }
					  $('.vmenu').hide();
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Pemeliharaan Referensi Faktur Pajak</span>
            </div>            
            </td>
        </tr>
    </table>
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul>        	
            <li>
            <a href="#" id="btnAdd" onClick="window.parent.OpenDHTML('faktur_pajak_add.php?reqMode=insert', 'Office Management - Aplikasi Keuangan', '600', '300');" title="Tambah">Tambah</a>
            <a href="#" id="btnEdit" title="Edit">Ubah</a>
            <a href="#" id="btnHapus" <?php /*?>id="btnDeleteRow"<?php */?> title="Hapus">Hapus</a>
            </li>   
            <li>
	            <a href="#" id="btnDetil" title="Detil">Detil</a>            
            </li>
            <li>
	            <a href="#" id="btnFile" title="File">File</a>            
            </li>     
        </ul>
    </div>
    ...
    <thead>
        <tr>
            ...
            <th width="150px">Nomor Faktur Awal</th>
            <th width="100px">Nomor Faktur Akhir</th>                            
        </tr>
    </thead>
    </table> 
    </div> 
    ...
    <div class="vmenu">
        ...
        <div class="first_li"><span>Detil</span></div>
        <div class="first_li"><span>File</span></div>
    </div>
    <!--RIGHT CLICK EVENT -->     
</div>
</body>
