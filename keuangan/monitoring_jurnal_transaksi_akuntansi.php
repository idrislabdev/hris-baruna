...
$kbbr_general_ref_d->selectByParamsJurnal();
while($kbbr_general_ref_d->nextRow())
{
	$arrJurnal[] = $kbbr_general_ref_d->getField("JURNAL");	
}
if($reqJurnal == "")
	$reqJurnal = $arrJurnal[0];
...
<title>monitoring jurnal tranksaksi akuntansi</title>
...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bServerSide": true,		
			  "sAjaxSource": "../json-keuangansiuk/monitoring_jurnal_transaksi_akuntansi_json.php?reqJurnal=<?=$reqJurnal?>&reqPosting=0&reqTanggalAwalAproval=<?=date("d-m-Y")?>&reqTanggalAkhirAproval=<?=date("d-m-Y")?>",
			...
			/* Click event handler */
			  ...
			  $('#example tbody').on( 'click', 'tr', function () {
					if ( $(this).hasClass('selected') ) {
					  ...
					  anSelectedJurnal = element[2];
			  });
			  
			  $('#btnRincianJurnal').on('click', function () {
				  if(anSelectedData == "")
					  return false;				
				  window.parent.OpenDHTML('monitoring_jurnal_transaksi_lihat.php?reqId='+anSelectedId+'&reqKeterangan='+anSelectedKeterangan, 'Office Management - Aplikasi Keuangan', '800', '500');	
			  });
			  ...
			  $('#btnCetak').on('click', function () {
				  if(anSelectedData == "")
					  return false;	
						/*newWindow = window.open('monitoring_jurnal_transaksi_rpt.php?reqId='+ anSelectedId+'&reqJurnal='+anSelectedJurnal, 'Cetak');
						newWindow.focus();	*/
					  if ($("#reqJurnal").val()=="JKK")
						window.parent.OpenDHTML('monitoring_jurnal_transaksi_jkk_penanda_tangan.php?reqNoBukti='+ anSelectedId, 'Office Management - Aplikasi Keuangan', '600', '300');
					  else if ($("#reqJurnal").val()=="JKM")
						window.parent.OpenDHTML('monitoring_jurnal_transaksi_jkm_penanda_tangan.php?reqNoBukti='+ anSelectedId, 'Office Management - Aplikasi Keuangan', '600', '300');	
					  else if ($("#reqJurnal").val()=="JRR")
						window.parent.OpenDHTML('monitoring_jurnal_transaksi_jrr_penanda_tangan.php?reqNoBukti='+ anSelectedId, 'Office Management - Aplikasi Keuangan', '600', '300');	
					  else if ($("#reqJurnal").val()=="JPJ")
						window.parent.OpenDHTML('monitoring_jurnal_transaksi_jpj_penanda_tangan.php?reqNoBukti='+ anSelectedId, 'Office Management - Aplikasi Keuangan', '600', '300');	
					  else
					  	alert("Jurnal Tidak ada Report...");
			  });
			   ...
			   $("#reqJurnal").change(function() { 
				 ...
				 val_posting= $("#reqPosting").val();
					...
					oTable.fnReloadAjax("../json-keuangansiuk/monitoring_jurnal_transaksi_akuntansi_json.php?reqJurnal="+ val_jurnal + "&reqPosting=" + val_posting +"&reqTanggalAwalAproval="+ val_tanggal_awal_approval+"&reqTanggalAkhirAproval="+ val_tanggal_akhir_approval);
				 //oTable.fnReloadAjax("../json-keuangansiuk/monitoring_jurnal_transaksi_akuntansi_json.php?reqJurnal="+ $("#reqJurnal").val() + "&reqPosting=" + $("#reqPosting").val());
			  });
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Monitoring Jurnal Transaksi Akuntansi</span>
            </div>            
            </td>
        </tr>
    </table>
    ...
    <div style="position: absolute; margin-top:30px; margin-left:176px; z-index:9999; font-size:12px;">
         Periode
          <input id="reqTanggalAwalAproval" name="reqTanggalAwalAproval" class="easyui-datebox" style="width:80px" value="<?=date("d-m-Y")?>" />
          s/d
          <input id="reqTanggalAkhirAproval" name="reqTanggalAkhirAproval" class="easyui-datebox" style="width:80px" value="<?=date("d-m-Y")?>" />   &nbsp;&nbsp;  
         
         
         Posting : <select name="reqPosting" id="reqPosting">
        		    <option value="0">Belum</option>
                    <option value="1">Sudah</option>
        		  </select> &nbsp;&nbsp;
                  
         Jurnal : <select name="reqJurnal" id="reqJurnal">
         
        		  <?
                  for($i=0;$i<count($arrJurnal);$i++)
				  {
				  ?>
                  	<option value="<?=$arrJurnal[$i]?>"><?=$arrJurnal[$i]?></option>
                  <?	  
				  }
				  ?>
        		  </select>  
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul>
            <li>
            <?php /*?><a href="#" id="btnAdd" onClick="window.parent.OpenDHTML('monitoring_jurnal_transaksi_akuntansi_add.php?reqMode=insert', 'Office Management - Aplikasi Keuangan', '600', '300');" title="Tambah">Tambah</a>
            <a href="#" id="btnEdit" title="Edit">Ubah</a><?php */?>
            <a href="#" id="btnRincianJurnal" title="Rincian Jurnal">Rincian Jurnal</a>
            <a href="#" id="btnCetak" title="Cetak">Cetak</a>
            <?php /*?><a href="#" id="btnDeleteRow" title="Hapus">Hapus</a><?php */?>
            </li>        
        </ul>
    </div>
    ...
    <thead>
        <tr>
            ...
            <th width="120px">No.&nbsp;Bukti&nbsp;Jurnal</th>
            <th width="120px">No.&nbsp;SIUK</th>
            <th width="250px">Pelanggan</th>
            <th width="100px">Tgl&nbsp;Jurnal</th>
            <th width="50px">Valuta</th>
            <th width="100px">Jumlah&nbsp;(Dollar)</th>
            <th width="100px">Jumlah&nbsp;(Rupiah)</th>
            <th width="100px">Posting</th>                                                      
        </tr>
    </thead>
    </table> 
    </div> 
