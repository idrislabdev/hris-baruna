...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-pembayaran/pembayaran_spp_report_json.php?reqTanggalAwal=<?=$reqTanggalAwal?>&reqTanggalAkhir=<?=$reqTanggalAkhir?>",			  
			...
			/* Click event handler */
			  ...
			  $('#btnEdit').on('click', function () {
				  ...
				  if(anSelectedNoPosting == '')
				  	window.parent.OpenDHTML('pembayaran_spp_add.php?reqMode=update&reqId='+anSelectedId, 'Office Management - Aplikasi Pembayaran', '600', '300');
			  	  else
			  ...
			  $('#btnHapus').on('click', function () {
				  if(anSelectedData == "")
					...
					{
						alert('Data yang telah diposting tidak dapat dihapus.');
						return;
					}

					if(confirm('Apakah anda yakin ingin menghapus nota : '+anSelectedId+'?'))
					{
						$.getJSON('../json-pembayaran/hapus_data_json.php?reqId='+anSelectedId,
						function(data){
							if(data.HASIL == 1)
							{
								alert('Data berhasil dihapus.');
									...
									var selesai = $('#reqTanggalAkhir').datebox('getValue');
							 		...
							 		oTable.fnReloadAjax("../json-pembayaran/pembayaran_spp_report_json.php?reqTanggalAwal=" + mulai + "&reqTanggalAkhir=" + selesai);
			  ...
			  /*$('#btnCetakBukti').on('click', function () {
				if(anSelectedData == "")
				return false;	
			  	window.parent.OpenDHTML('proses_pelunasan_transaksi_kas_bank_penanda_tangan.php?reqNoBukti='+ anSelectedId, 'Office Management - Aplikasi Pembayaran', '600', '300');	
			  });*/
			  ...
			  return false;
			   ...
			   $('.vmenu .first_li').on('click',function() {
				  if( $(this).children().size() == 1 ) {
					  ...
					  else if($(this).children().text() == 'Cetak Bukti')
					  {
						  $("#btnCetakBukti").click();
					  }	 
					  $('.vmenu').hide();
			 ...
			 var departemen = "";
			  $('#reqTanggalAwal, #reqTanggalAkhir').datebox({
					onSelect: function(date){
						...
						var selesai = $('#reqTanggalAkhir').datebox('getValue');
				 		...
				 		oTable.fnReloadAjax("../json-pembayaran/pembayaran_spp_report_json.php?reqTanggalAwal=" + mulai + "&reqTanggalAkhir=" + selesai+"&reqDepartemen="+departemen);
			  ...
			  $('#btnEksporXls').on('click', function () {
				...
				var selesai = $('#reqTanggalAkhir').datebox('getValue');
				  ...
				  document.location.href = "pembayaran_spp_report_excel.php?reqMode=update&reqTanggalAwal="+mulai+"&reqTanggalAkhir="+selesai+"&reqDepartemen="+departemen;	
			  });
			  $('#btnEksporPdf').on('click', function () {
				...
				var selesai = $('#reqTanggalAkhir').datebox('getValue');
				  ...
				  window.parent.OpenDHTML("pembayaran_spp_report_pdf.php?reqMode=update&reqTanggalAwal="+mulai+"&reqTanggalAkhir="+selesai+"&reqDepartemen="+departemen, 'Aplikasi Pembayaran');	
			  });
		});
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Rekap Transaksi Pembayaran Tunai  </span>
            </div>            
            </td>
        </tr>
    </table>
    ...
    <div style="position: absolute; margin-top:28px; margin-left:186px; z-index:9999; font-size:12px;">
        Periode :  <input id="reqTanggalAwal" name="reqTanggalAwal" class="easyui-datebox" data-options="validType:'date'" value="<?=$reqTanggalAwal?>" /> s.d 
        		   <input id="reqTanggalAkhir" name="reqTanggalAkhir" class="easyui-datebox" data-options="validType:'date'" value="<?=$reqTanggalAkhir?>" />
        Sekolah : <input id="reqDepartemen" class="easyui-combotree" name="reqDepartemen" data-options="url:'../json-pembayaran/departemen_combo_json.php'" style="width:480px;" value="<?=$reqDepartemen?>">
    ...
    <thead>
        <tr>
            ...
            <th width="140px">Sekolah</th>
            <th width="120px">Tagihan</th>
            <th width="100px">Periode Tagihan</th>
        </tr>
    </thead>
    </table> 
    </div> 
