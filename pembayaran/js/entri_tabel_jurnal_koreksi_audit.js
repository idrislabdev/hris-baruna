function addRow()
{
	...
	var element = document.createElement("input");
	element.type = "text";
	element.name = "reqDebetRp[]";
	element.id = "reqDebetRp"+rownum;
	element.style.textAlign = "right";
	element.className='easyui-validatebox';
	element.style.width = "111px";
	element.value = 0;
	element.onfocus = function() {  
		FormatAngka("reqDebetRp"+rownum);
	};
	element.onkeyup = function() {  
		FormatUang("reqDebetRp"+rownum);
	};
	element.onblur = function() {  
		FormatUang("reqDebetRp"+rownum);
	};		
	...
	var element = document.createElement("input");
	element.type = "text";
	element.name = "reqKreditRp[]";
	element.id = "reqKreditRp"+rownum;
	element.style.textAlign = "right";
	element.className='easyui-validatebox';
	element.style.width = "111px";
	element.value = 0;
	element.onfocus = function() {  
		FormatAngka("reqKreditRp"+rownum);
	};
	element.onkeyup = function() {  
		FormatUang("reqKreditRp"+rownum);
	};
	element.onblur = function() {  
		FormatUang("reqKreditRp"+rownum);
	};		
	...
	button.innerHTML = '<center><a style="cursor:pointer;" onclick="deleteRowDrawTable(\'dataTableRowDinamis\', this)"><span class="merah">&otimes;</span></a></center>';
...
function hitungDebetTotal()
{
	...
	for(var i=0; i<=rownum; i++) {
		...
		{
			rp = Math.floor(FormatAngkaNumber($("#reqDebet"+i).val()) * FormatAngkaNumber($("#reqKursValuta").val()));
			$("#reqDebetRp"+i).val(FormatCurrency(rp));
...
function hitungKreditTotal()
{
	...
	for(var i=0; i<=rownum; i++) {
		...
		{
			rp = Math.floor(FormatAngkaNumber($("#reqKredit"+i).val()) * FormatAngkaNumber($("#reqKursValuta").val()));
			$("#reqKreditRp"+i).val(FormatCurrency(rp))
...
function hitungDebetTotalRp()
{
	...
	var total = 0;		
	for(var i=0; i<=rownum; i++) {
		if(typeof $("#reqDebetRp"+i).val() == "undefined")
		{}
		else
		{
			jumlah = FormatAngkaNumber($("#reqDebetRp"+i).val());
			total = total + Number(jumlah);	
		}
	}
	$("#reqJumlahDebetRp").val(FormatCurrency(total));
}

function hitungKreditTotalRp()
{
	if (!document.getElementsByTagName) return;
	tabBody=document.getElementsByTagName("TBODY").item(0);
	...
	var total = 0;		
	for(var i=0; i<=rownum; i++) {
		if(typeof $("#reqKreditRp"+i).val() == "undefined")
		{}
		else
		{
			jumlah = FormatAngkaNumber($("#reqKreditRp"+i).val());
			total = total + Number(jumlah);	
		}
	}
	$("#reqJumlahKreditRp").val(FormatCurrency(total));	
}
