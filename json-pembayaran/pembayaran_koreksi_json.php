...
$allRecord = $kptt_nota_spp->getCountByParamsBak(array("JEN_JURNAL" => "JKM", "A.KD_SUBSIS"=>"KPT", "JEN_TRANS"=>2, "STATUS_PROSES"=>1), $statement);
if($_GET['sSearch'] == "")
	$allRecordFilter = $allRecord;
else	
	$allRecordFilter = $kptt_nota_spp->getCountByParamsBak(array("JEN_JURNAL" => "JKM", "A.KD_SUBSIS"=>"KPT", "JEN_TRANS"=>2, "STATUS_PROSES"=>1), $statement." AND (UPPER(A.NO_NOTA) LIKE '%".strtoupper($_GET['sSearch'])."%' OR UPPER(KD_KUSTO) LIKE '%".strtoupper($_GET['sSearch'])."%' OR UPPER(MPLG_NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')");

$kptt_nota_spp->selectByParamsBak(array("JEN_JURNAL" => "JKM", "A.KD_SUBSIS"=>"KPT", "JEN_TRANS"=>2, "STATUS_PROSES"=>1), $dsplyRange, $dsplyStart, $statement." AND (UPPER(A.NO_NOTA) LIKE '%".strtoupper($_GET['sSearch'])."%' OR UPPER(KD_KUSTO) LIKE '%".strtoupper($_GET['sSearch'])."%' OR UPPER(MPLG_NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
