...
$kptt_nota_spp->selectByParamsRekapPosting(array("TO_CHAR(TGL_TRANS, 'DD-MM-YYYY')" => $reqTanggal));
...
while($kptt_nota_spp->nextRow())
{
	...
	if($kptt_nota->insert())
	{
		...
		$kptt_nota_spp_d->selectByParamsJurnal(array("TO_CHAR(B.TGL_TRANS, 'DD-MM-YYYY')" => $reqTanggal));
...
if($adaData == 0)
	echo "Tanggal ".$reqTanggal." tidak terdapat transaksi / telah diposting semua.";
else
	echo "Transaksi tanggal ".$reqTanggal." berhasil dijurnal.";
