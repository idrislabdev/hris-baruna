...
class kalkulasi_json extends CI_Controller {
	...
	function kalkulasi_absensi_edit_json()
	{
	...
	function kalkulasi_absensi_json()
	{
		include_once("../WEB-INF/classes/utils/UserLogin.php");
		include_once("../WEB-INF/classes/base-absensi/AbsensiRekap.php");
		include_once("../WEB-INF/functions/string.func.php");
		...
		$aColumns = array('PEGAWAI_ID', 'NAMA', 'KELOMPOK', 'UNIT_KERJA', 'JABATAN', 'TOTAL_PERAK', 'TOTAL_DAERAH');
		$aColumnsAlias = array('PEGAWAI_ID', 'NAMA', 'KELOMPOK', 'UNIT_KERJA', 'JABATAN', 'TOTAL_PERAK', 'TOTAL_DAERAH');
		...
		if ( $reqJenis == "Proses" ) {
			$absensi_rekap->callGenerateAbsensi($reqPeriode);
		}


		$allRecord = $absensi_rekap->getCountByParamsMK(array("a.periode" => $reqPeriode));
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $absensi_rekap->getCountByParamsMK(array("a.periode" => $reqPeriode), " AND (UPPER(b.nama) LIKE '%".strtoupper($_GET['sSearch'])."%' OR UPPER(b.nama) LIKE '%".strtoupper($_GET['sSearch'])."%')");

		$absensi_rekap->selectByParamsRekapMK(array("a.periode" => $reqPeriode), $dsplyRange, $dsplyStart, " AND (UPPER(b.nama) LIKE '%".strtoupper($_GET['sSearch'])."%' OR UPPER(b.nama) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
