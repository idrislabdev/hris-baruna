...
class fingerscan_json extends CI_Controller {
	...
	function json()
	{
		...
		$reqMesin = httpFilterGet("reqMesin");

		ini_set("memory_limit","500M");
		ini_set('max_execution_time', 520);

		$arrMesin = explode(",", $reqMesin);

		$aColumns[0] = "NRP";
		$aColumns[1] = "NAMA";
		$aColumns[2] = "DEPARTEMEN";
		$aColumnsAlias[0] = "NRP";
		$aColumnsAlias[1] = "NAMA";
		$aColumnsAlias[2] = "A.DEPARTEMEN_ID";
		$j = 3;
		for($i=0;$i<count($arrMesin);$i++)
		{
			
			$aColumns[$j] = "JAM_MASUK_".$arrMesin[$i];
			$aColumnsAlias[$j] = "PEL_ABSENSI.AMBIL_JAM_ABSENSI(PEGAWAI_ID, TO_DATE('".$tanggal."', 'DDMMYYYY'), 'I', '".$arrMesin[$i]."')";	
			$j++;
			$aColumns[$j] = "JAM_PULANG_".$arrMesin[$i];
			$aColumnsAlias[$j] = "PEL_ABSENSI.AMBIL_JAM_ABSENSI(PEGAWAI_ID, TO_DATE('".$tanggal."', 'DDMMYYYY'), 'O', '".$arrMesin[$i]."')";	
			$j++;
		}
		array('NRP', 'NAMA', 'DEPARTEMEN', 'JAM_MASUK', 'JAM_PULANG');
		/*
		 * Ordering
		 */
		...
		$allRecord = $absensi->getCountByParamsFingerScan(array(), "", $reqTanggal);
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $absensi->getCountByParamsFingerScan(array(), " AND (UPPER(A.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%' OR UPPER(A.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')", $reqTanggal);
		//echo $absensi->query;exit;
		$absensi->selectByParamsFingerScan(array(), $dsplyRange, $dsplyStart, " AND (UPPER(A.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%' OR UPPER(A.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder, $reqTanggal, $reqMesin);
		//echo $absensi->errorMsg;exit;
		//echo $absensi->query;exit;
			...
			while($absensi->nextRow())
			{
				...
				for ( $i=0 ; $i<count($aColumns) ; $i++ )
				{
					if($aColumns[$i] == "STATUS")
						$row[] = getNameValueAktif($absensi->getField(trim($aColumns[$i])));					
					else
