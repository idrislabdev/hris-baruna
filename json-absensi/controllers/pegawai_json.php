...
class pegawai_json extends CI_Controller {
	...
	function pegawai_jenis_jam_kerja_json()
	{
		include_once("../WEB-INF/classes/utils/UserLogin.php");
		include_once("../WEB-INF/functions/string.func.php");
		include_once("../WEB-INF/functions/default.func.php");
		...
		$pegawai_jam_kerja = new PegawaiJamKerjaJenis();
		...
		$aColumns = array("PEGAWAI_ID", "NRP", "NIPP", "NAMA", "JABATAN_NAMA", "DEPARTEMEN", "JENIS_JAM_KERJA");
		$aColumnsAlias = array("A.PEGAWAI_ID", "NRP", "NIPP", "A.NAMA", "D.NAMA", "B.NAMA", "F.NAMA");
		...
		$allRecord = $pegawai_jam_kerja->getCountByParams(array(),$statement);
		//echo $pegawai_jam_kerja->query;
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $pegawai_jam_kerja->getCountByParams(array(), $statement." AND ((UPPER(A.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%') OR (UPPER(A.NIPP) LIKE '%".strtoupper($_GET['sSearch'])."%')) ");

		//echo $pegawai_jam_kerja->query;
		$pegawai_jam_kerja->selectByParams(array(), $dsplyRange, $dsplyStart, $statement." AND ((UPPER(A.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%') OR (UPPER(A.NIPP) LIKE '%".strtoupper($_GET['sSearch'])."%')) " , $sOrder);
		...
		while($pegawai_jam_kerja->nextRow())
		{
			$row = array();
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				if($aColumns[$i] == "TANGGAL_LAHIR")
					$row[] = getFormattedDate($pegawai_jam_kerja->getField($aColumns[$i]));
				else if($aColumns[$i] == "KETERANGAN")
					$row[] = truncate($pegawai_jam_kerja->getField($aColumns[$i]), 5)."...";
				else
					$row[] = $pegawai_jam_kerja->getField($aColumns[$i]);
			}
	...
	function pegawai_jenis_jam_kerja_json_set()
	{
		/* INCLUDE FILE */
		include_once("../WEB-INF/classes/base-absensi/PegawaiJamKerjaJenis.php");
		include_once("../WEB-INF/functions/string.func.php");
