...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-inventaris/rincian_aktiva_tetap_json.php?reqPeriode=<?=$reqPeriode?>&reqPeriodeLalu=<?=$reqPeriodeLalu?>",						  
			...
			/* Click event handler */
			  ...
			  $('#btnEksporXls').on('click', function () {
				  document.location.href = "rincian_rekap_aktiva_tetap_excel.php?reqMode=update&reqPeriode="+$("#reqPeriode").val()+"&reqPeriodeLalu="+$("#reqPeriodeLalu").val();	
			  });
			  $('#btnEksporPdf').on('click', function () {
				  window.parent.OpenDHTML("rincian_rekap_aktiva_tetap_pdf.php?reqMode=update&reqPeriode="+$("#reqPeriode").val()+"&reqPeriodeLalu="+$("#reqPeriodeLalu").val(), 'Sistem Informasi Inventaris');	
			  });

			  $('#btnLihat').on('click', function () {
					 window.parent.OpenDHTML('jurnal_flexy_lihat.php?reqPeriode='+ $("#reqPeriode").val()+"&reqPeriodeLalu="+$("#reqPeriodeLalu").val(), 'Office Management - Fixed Asset', '900', '600');	
			  });
			  ...
			  $('#btnKalkulasi').on('click', function () {
				...
				{  
					var jqxhr = $.get( "../json-inventaris/kalkulasi_penyusutan.php?reqPeriode="+$("#reqPeriode").val(), function(data) {
					  alert(data);
			  		  oTable.fnReloadAjax("../json-inventaris/rincian_aktiva_tetap_json.php?reqPeriode="+ $("#reqPeriode").val());
					});	
				}				 
			  });
			  ...
			  $("#reqPeriode").change(function() { 
			  		oTable.fnReloadAjax("../json-inventaris/rincian_aktiva_tetap_json.php?reqPeriode="+ $("#reqPeriode").val()+"&reqPeriodeLalu="+ $("#reqPeriodeLalu").val());
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> RINCIAN Rekapitulasi Aktiva Tetap</span>
            </div>            
            </td>
        </tr>
    </table>
