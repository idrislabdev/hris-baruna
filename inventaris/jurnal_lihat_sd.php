...
<head>
    ...
    $(document).ready(function() {
	  ...
	  $('#btnBuatJurnal').on('click', function () {
		...
		if(confirm('Buat jurnal penyusutan periode <?=getNamePeriode($reqPeriodeLalu)?> s.d <?=getNamePeriode($reqPeriode)?> ?'))
		{  
			var jqxhr = $.get( "../json-inventaris/kalkulasi_penyusutan_jurnal_sd.php?reqPeriodeLalu=<?=$reqPeriodeLalu?>&reqPeriode=<?=$reqPeriode?>&reqTanggalTransaksi="+$("#reqTanggalTransaksi").datebox("getValue"), function(data) {
			  alert(data);
			  $("#btnBuatJurnal").hide();
			});	
		}		
...
<div id="wadah">
    ...
    <div id="rincian-header">	
    	<table>
        	<tr>
            	<td>Periode</td>
                <td>:</td>
                <td><?=getNamePeriode($reqPeriodeLalu)?> s.d <?=getNamePeriode($reqPeriode)?></td>    
    ...
    <tbody> 
	  ...
	  $i = 1;
	  $checkbox_index = 0;
	  $kalkulasi_penyusutan->selectByParamsJurnalV2($reqPeriodeLalu, $reqPeriode);
	  $saldo_val_debet = 0;
	  $saldo_val_kredit = 0;
	  $saldo_rp_debet = 0;
	  $saldo_rp_kredit = 0;
