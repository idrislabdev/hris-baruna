...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-inventaris/penghapusan_aktiva_tetap_json.php?reqPeriode=<?=$reqPeriode?>&reqPeriodeAwal=<?=$reqPeriodeAwal?>&reqPeriodeAkhir=<?=$reqPeriodeAkhir?>",					  
			...
			/* Click event handler */
			  ...
			  $('#btnEksporXls').on('click', function () {
				  document.location.href = 'penghapusan_rekap_aktiva_tetap_excel.php?reqMode=update&reqId='+anSelectedId;	
			  });

			  $('#btnEksporPdf').on('click', function () {
				  window.parent.OpenDHTML('penghapusan_rekap_aktiva_tetap.php?reqMode=update&reqId='+anSelectedId, 'Sistem Informasi Inventaris');	
			  });
			  ...
			  $('#btnKalkulasi').on('click', function () {
				...
				{  
					var jqxhr = $.get( "../json-inventaris/kalkulasi_penyusutan.php?reqPeriode="+$("#reqPeriode").val(), function(data) {
					  alert(data);
			  		  oTable.fnReloadAjax("../json-inventaris/penghapusan_aktiva_tetap_json.php?reqPeriode="+ $("#reqPeriode").val());
					});	
				}				 
			  });
			  ...
			  $("#reqPeriode, #reqBulanAwal, #reqBulanAkhir, #reqTahunAwal, #reqTahunAkhir").change(function() { 
			  		oTable.fnReloadAjax("../json-inventaris/penghapusan_aktiva_tetap_json.php?reqPeriode="+ $("#reqPeriode").val() + "&reqPeriodeAwal="+ $("#reqBulanAwal").val() +  $("#reqTahunAwal").val() + "&reqPeriodeAkhir="+ $("#reqBulanAkhir").val() +  $("#reqTahunAkhir").val());
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png">Penghapusan Aktiva Tetap</span>
            </div>            
            </td>
        </tr>
    </table>
