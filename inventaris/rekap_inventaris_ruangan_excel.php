...
$worksheet->write(1, 1, "DAFTAR REKAP INVENTARIS RUANGAN", $text_format);
	
$worksheet->write(4, 1, "INVENTARIS", $text_format_line_bold);
$worksheet->write(4, 2, "SPESIFIKASI", $text_format_line_bold);
$worksheet->write(4, 3, "NOMOR", $text_format_line_bold);
$worksheet->write(4, 4, "BARCODE", $text_format_line_bold);
$worksheet->write(4, 5, "PEROLEHAN", $text_format_line_bold);
$worksheet->write(4, 6, "HARGA", $text_format_line_bold);
...
while($inventaris_ruangan->nextRow())
{
	...
	{
		$worksheet->write($row, 1, $inventaris_ruangan->getField("LOKASI"), $text_format_merge_line_bold);
	...
	$worksheet->write($row, 1, $inventaris_ruangan->getField("INVENTARIS"), $text_format_line_left);
	$worksheet->write($row, 2, $inventaris_ruangan->getField("SPESIFIKASI"), $text_format_line_left);
	$worksheet->write($row, 3, $inventaris_ruangan->getField("NOMOR"), $text_format_line_left);
	$worksheet->write($row, 4, " ".$inventaris_ruangan->getField("BARCODE"), $text_format_line_left);
	$worksheet->write($row, 5, getNamePeriode($inventaris_ruangan->getField("PEROLEHAN_PERIODE")), $text_format_line_left);
	$worksheet->write($row, 6, $inventaris_ruangan->getField("PEROLEHAN_HARGA"), $uang_line);
...
header("Content-Type: application/x-msexcel; name=\"rekap_inventaris_ruangan_excel.xls\"");
header("Content-Disposition: inline; filename=\"rekap_inventaris_ruangan_excel.xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
?>