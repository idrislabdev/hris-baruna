...
for ($i=0; $i < 2; $i++)
{
	...
	if ($i==0)
	{
		...
		$worksheet->set_column(12, 12, 18.00);
		$worksheet->set_column(13, 13, 18.00);
		...
		$worksheet->write(0, 0, "INVENTARIS RUANGAN ID", $text_format_line_bold);
		$worksheet->write(0, 1, "INVENTARIS ID", $text_format_line_bold);
		$worksheet->write(0, 2, "JENIS INVENTARIS ID", $text_format_line_bold);
		$worksheet->write(0, 3, "LOKASI ID", $text_format_line_bold);
		$worksheet->write(0, 4, "KODE", $text_format_line_bold);
		$worksheet->write(0, 5, "NOMOR", $text_format_line_bold);
		$worksheet->write(0, 6, "NAMA", $text_format_line_bold);
		$worksheet->write(0, 7, "LOKASI", $text_format_line_bold);
		$worksheet->write(0, 8, "TANGGAL PEROLEHAN", $text_format_line_bold);
		$worksheet->write(0, 9, "UMUR EKONOMIS", $text_format_line_bold);
		$worksheet->write(0, 10, "UMUR EKONOMIS REAL", $text_format_line_bold);
		$worksheet->write(0, 11, "NILAI PEROLEHAN", $text_format_line_bold);
		$worksheet->write(0, 12, "NILAI PENYUSUTAN", $text_format_line_bold);
		$worksheet->write(0, 13, "NILAI AKHIR", $text_format_line_bold);
		...
		while ($integrasi_potongan->nextRow()) 
		{
			...
			$worksheet->write($row, 7, "", $text_format_line_left);
			$worksheet->write($row, 8, "", $text_format_line_left);
			$worksheet->write($row, 9, "", $text_format_line_left);
			$worksheet->write($row, 10, "", $text_format_line_left);
			$worksheet->write($row, 11, "", $text_format_line_left);
			$worksheet->write($row, 12, "", $text_format_line_left);
			$worksheet->write($row, 13, "", $text_format_line_left);
	...
	elseif($i==1)
	{
			...
			while ($potongan_kondisi->nextRow()) {
				$worksheet->write_string($row, 0, $potongan_kondisi->getField("POTONGAN_KONDISI_ID"),$text_format_line_left);	
...
header("Content-Type: application/x-msexcel; name=\"import_kalkulasi_penyusutan_template_excel.xls\"");
header("Content-Disposition: inline; filename=\"import_kalkulasi_penyusutan_template_excel.xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
?>