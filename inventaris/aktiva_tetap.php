...
$kalkulasi_penyusutan->selectByParamsPeriodeAda();
...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-inventaris/aktiva_tetap_json.php?reqPeriode=<?=$reqPeriode?>",						  
			...
			/* Click event handler */
			  ...
			  $('#btnEksporXls').on('click', function () {
				  document.location.href = "rekap_aktiva_tetap_excel.php?reqMode=update&reqPeriode="+$("#reqPeriode").val();	
			  });
			  $('#btnEksporPdf').on('click', function () {
				  window.parent.OpenDHTML("rekap_aktiva_tetap_pdf.php?reqMode=update&reqPeriode="+$("#reqPeriode").val(), 'Sistem Informasi Inventaris');	
			  });
			  ...
			  $('#btnKalkulasi').on('click', function () {
				...
				{  
					var jqxhr = $.get( "../json-inventaris/kalkulasi_penyusutan.php?reqPeriode="+$("#reqPeriode").val(), function(data) {
					  alert(data);
			  		  oTable.fnReloadAjax("../json-inventaris/aktiva_tetap_json.php?reqPeriode="+ $("#reqPeriode").val());
					});	
				}				 
			  });
			  ...
			  $("#reqPeriode").change(function() { 
			  		document.location.href ="aktiva_tetap.php?reqPeriode="+ $("#reqPeriode").val();
			  });
