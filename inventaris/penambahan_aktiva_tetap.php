...
if($reqPeriode == "")
{
	...
	$reqPeriodeAwal = $arrPeriode[0];
	$reqPeriodeAkhir = $arrPeriode[0];
}
...
<style type="text/css" media="screen">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-inventaris/penambahan_aktiva_tetap_json.php?reqPeriode=<?=$reqPeriode?>&reqPeriodeAwal=<?=$reqPeriodeAwal?>&reqPeriodeAkhir=<?=$reqPeriodeAkhir?>",						  
			...
			/* Click event handler */
			  ...
			  $('#btnEksporXls').on('click', function () {
				  document.location.href = "penambahan_rekap_aktiva_tetap_excel.php?reqMode=update&reqPeriode="+$("#reqPeriode").val();		
			  });
			  $('#btnEksporPdf').on('click', function () {
				  window.parent.OpenDHTML("penambahan_rekap_aktiva_tetap_pdf.php?reqMode=update&reqPeriode="+$("#reqPeriode").val(), 'Sistem Informasi Inventaris');	
			  });
			  ...
			  $('#btnKalkulasi').on('click', function () {
				...
				{  
					var jqxhr = $.get( "../json-inventaris/kalkulasi_penyusutan.php?reqPeriode="+$("#reqPeriode").val(), function(data) {
					  alert(data);
			  		  oTable.fnReloadAjax("../json-inventaris/penambahan_aktiva_tetap_json.php?reqPeriode="+ $("#reqPeriode").val());
					});	
				}				 
			  });
			  ...
			  $("#reqPeriode, #reqBulanAwal, #reqBulanAkhir, #reqTahunAwal, #reqTahunAkhir").change(function() { 
			  		oTable.fnReloadAjax("../json-inventaris/penambahan_aktiva_tetap_json.php?reqPeriode="+ $("#reqPeriode").val() + "&reqPeriodeAwal="+ $("#reqBulanAwal").val() +  $("#reqTahunAwal").val() + "&reqPeriodeAkhir="+ $("#reqBulanAkhir").val() +  $("#reqTahunAkhir").val());
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png">Penambahan Aktiva Tetap</span>
            </div>            
            </td>
        </tr>
    </table>
    ...
    <div style="position: absolute; margin-top:6px; margin-left:180px; z-index:9999; font-size:12px;">
 		Periode : <select name="reqPeriode" id="reqPeriode">
                  ...
                  <select name="reqBulanAwal" id="reqBulanAwal">
					<?
                    for($i=1; $i<=12; $i++)
                    {
                    ...
                    <select name="reqTahunAwal" id="reqTahunAwal">
                        <? 
                   ...
                   s.d 
                  <select name="reqBulanAkhir" id="reqBulanAkhir">
					<?
                    for($i=1; $i<=12; $i++)
                    {
                        $tempNama=getNameMonth($i);
                    ...
                    <select name="reqTahunAkhir" id="reqTahunAkhir">
                        <? 
                        for($i=date("Y")-10; $i <= date("Y"); $i++)
                        {
                        ?>
