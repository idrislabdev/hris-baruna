    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			 ...
			 "aoColumns": [ 
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-inventaris/pendataan_add_monitoring_json.php?reqId=<?=$reqId?>",			  
			  ...
			  $("#example tbody").click(function(event) {
					  ...
					  window.parent.frames['mainFrameDetilPop'].location.href = 'pendataan_add_data.php?reqId=<?=$reqId?>&reqInventarisId='+anSelectedId+'&reqInventarisNama='+anSelectedNama;
			  });
			  
			  $('#btnEdit').on('click', function () {
				  if(anSelectedData == "")
					  return false;				
				  window.parent.OpenDHTML('inventaris_add_data.php?reqMode=update&reqLokasiId=<?=$reqId?>&reqId='+anSelectedId, 'Sistem Informasi Inventaris', '900', '500');	
			  });
			  
			  $('#btnPindah').on('click', function () {
				  if(anSelectedData == "")
					  return false;				
				  window.parent.OpenDHTML('inventaris_add_lokasi.php?reqMode=update&reqId=<?=$reqId?>&reqInventarisId='+anSelectedId, 'Sistem Informasi Inventaris', '900', '500');	
			  });
			 ...
			 $('#btnCetakBarcodeSemua').on('click', function () {
				newWindow = window.open('barcode.php?reqId=<?=$reqId?>', 'Cetak');
				newWindow.focus();
			  });
			  ...
			  $('#btnHapus').on('click', function () {
					...
					if ( confirm( "Apakah anda yakin, menghapus data ini ?" ) ) {
						...
						var jqxhr = $.get('../json-inventaris/delete.php?reqMode=inventaris_ruangan&id='+anSelectedId+'&reqRowId=<?=$reqId?>', function() {
							document.location.reload();
							window.parent.frames['mainFrameDetilPop'].location.href = 'pendataan_add_data.php';
						});
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png">Pendataan <?=$reqLokasi?></span>
            </div>            
            </td>
        </tr>
    </table>
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif)">    
        <ul>
            <li>
                <a href="#" id="btnAdd" onClick="window.parent.OpenDHTML('pendataan_add_monitoring_add.php?reqMode=insert&reqId=<?=$reqId?>', 'Sistem Informasi Asset', '900', '500');" title="Tambah">Tambah Asset</a>
                <a href="#" id="btnEdit" title="Ubah"> Ubah Asset</a>            
                <a href="#" id="btnPindah" title="Pindah"> Pindah Asset</a>            
                <a href="#" id="btnHapus" title="Hapus"> Hapus Asset</a>            
            </li>        
	        <?php /*?><li>
            	<a href="#" id="btnCetakBarcodeSemua">Cetak Barcode Semua</a>
            </li> <?php */?>  
        </ul>
    </div>
    ...
    <thead>
        <tr>
            ...
            <th width="160px">Spesifikasi</th>                                             
            <th width="50px">Jenis</th>    
            <th width="50px">Jumlah</th>                                                                                       
            <th width="50px">Umur Ekonomis / Sisa Umur</th>                                                                                       
        </tr>
    </thead>
    </table> 
    </div> 
