...
$aColumns = array("ANGGARAN_MUTASI_ID", "STATUS_KIRIM_LEBIH_KURANG", "JENIS_ANGGARAN_ID", "NO_NOTA", "JENIS_ANGGARAN", "PUSPEL", "PEGAWAI", "TGL_TRANS", "NO_REF3", "NO_REF2", "JML_VAL_TRANS", "JML_VAL_REALISASI", "KET_TAMBAH");
$aColumnsAlias = array("ANGGARAN_MUTASI_ID", "STATUS_KIRIM_LEBIH_KURANG", "A.JENIS_ANGGARAN_ID", "A.NO_NOTA", "JENIS_ANGGARAN", "PUSPEL", "B.NAMA", "TGL_TRANS", "NO_REF3", "NO_REF2", "JML_VAL_TRANS", "JML_VAL_REALISASI", "KET_TAMBAH");
...
if ( isset( $_GET['iSortCol_0'] ) )
{
	...
	if ( trim($sOrder) == "ORDER BY ANGGARAN_MUTASI_ID asc" )
	{
		...
		*/
		$sOrder = " ORDER BY NVL(TGL_PERTANGGUNGJAWABAN, TGL_TRANS) ASC ";
...
if($reqStatus == 0)
	$statement .= " AND NVL(JML_VAL_REALISASI, 0) = 0 ";
elseif($reqStatus == 1)
	$statement .= " AND NVL(JML_VAL_REALISASI, 0) > 0 ";
else
	$statement .= "";

$statement .= " AND PUSPEL = '".$reqId."' AND NVL(STATUS_PERMOHONAN, 0) = 1 AND A.JENIS_ANGGARAN_ID IN (1, 3, 5, 6) AND VERIFIKASI = 'S' ";

$statement_json= " AND (UPPER(B.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%' OR UPPER(C.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')";
...
while($anggaran_mutasi->nextRow())
{
	...
	for ( $i=0 ; $i<count($aColumns) ; $i++ )
	{
		...
		else if($aColumns[$i] == "JML_VAL_TRANS" || $aColumns[$i] == "JUMLAH" || $aColumns[$i] == "JML_VAL_REALISASI")
