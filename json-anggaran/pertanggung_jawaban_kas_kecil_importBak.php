	...
	if($anggaran_mutasi->insert())
	{
		...
		for($i=1;$i<=$baris;$i++)
		{
			...
			else
			{
				...
				$anggaran_mutasi_d->setField("NAMA", $data->val($i, 2));
				$anggaran_mutasi_d->setField("UNIT", 1);
				$anggaran_mutasi_d->setField("HARGA_SATUAN", dotToNo($data->val($i, 3)));
				$anggaran_mutasi_d->setField("JUMLAH", dotToNo($data->val($i, 3)));
