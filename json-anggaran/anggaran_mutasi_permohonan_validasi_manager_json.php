...
$aColumns = array("ANGGARAN_MUTASI_ID", "VERIFIKASI", "CHECK", "NO_NOTA", "NO_REF3", "JENIS_ANGGARAN", "PUSPEL_NAMA", "PEGAWAI", "TGL_TRANS", "JML_VAL_TRANS", "KET_TAMBAH");
$aColumnsAlias = array("ANGGARAN_MUTASI_ID", "VERIFIKASI", "CHECK", "NO_NOTA", "NO_REF3", "JENIS_ANGGARAN", "PUSPEL_NAMA", "B.NAMA", "TGL_TRANS", "JML_VAL_TRANS", "KET_TAMBAH");
...
if($reqStatus == "")
	$statement .= " AND VERIFIKASI IS NULL ";
else
	$statement .= " AND VERIFIKASI = '".$reqStatus."' ";

$statement .= " AND NVL(STATUS_PERMOHONAN, 0) = 1 AND APPROVED_DATE IS NOT NULL ";
...
while($anggaran_mutasi->nextRow())
{
	...
	for ( $i=0 ; $i<count($aColumns) ; $i++ )
	{
		if($aColumns[$i] == "TGL_TRANS")
			...
			if ($reqStatus == "")
			{
				$row[] = '<input type="checkbox" name="reqCheck[]" class="selectedId" id="reqCheck'.$index.'" value="'.$anggaran_mutasi->getField("ANGGARAN_MUTASI_ID").'">';
			}
			else
			{
