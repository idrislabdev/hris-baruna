...
$aColumns = array("THN_BUKU", "KD_BUKU_BESAR", "KD_BUKU_PUSAT", "NM_BUKU_BESAR", "KD_SUB_BANTU", "NM_BUKU_PUSAT", "ANGG_TAHUNAN", 
                 "JUMLAH_MUTASI", "D_K", "REALISASI");
$aColumnsAlias = array("THN_BUKU", "KD_BUKU_BESAR", "KD_BUKU_PUSAT", "KD_BUKU_BESAR", "KD_SUB_BANTU", "KD_BUKU_PUSAT", "ANGG_TAHUNAN", 
                 "JUMLAH_MUTASI", "D_K", "REALISASI");
...
while($kbbt_neraca_angg->nextRow())
{
	...
	for ( $i=0 ; $i<count($aColumns) ; $i++ )
	{
		if($aColumns[$i] == "TANGGAL")
			$row[] = getFormattedDate($kbbt_neraca_angg->getField($aColumns[$i]));
		else if($aColumns[$i] == "ANGG_TAHUNAN" || $aColumns[$i] == "JUMLAH_MUTASI")
			$row[] = currencyToPage($kbbt_neraca_angg->getField($aColumns[$i]));
		else
			$row[] = $kbbt_neraca_angg->getField($aColumns[$i]);
	}
