    ...
    **/ 
	...
	{//A.HP, A.DAYA, 
		$str = "
                ...
                A.KAPAL_PEKERJAAN_KAPAL_ID, A.KAPAL_PEKERJAAN_KAPAL_ID_GANTI, A.KAPAL_PEKERJAAN_ID, A.KAPAL_ID, B.NAMA KAPAL_NAMA, 
                   ...
                   TO_CHAR(A.TEMP_TANGGAL_AWAL, 'YYYY-MM-DD HH24:MI:SS') TEMP_TANGGAL_AWAL, TO_CHAR(A.TEMP_TANGGAL_AKHIR, 'YYYY-MM-DD HH24:MI:SS') TEMP_TANGGAL_AKHIR
    ...
    function selectByParamsKapalGanti($paramsArray=array(),$limit=-1,$from=-1, $statement="", $order="ORDER BY KAPAL_PEKERJAAN_KAPAL_ID ASC")
	{
		$str = "
				...
				A.KAPAL_PEKERJAAN_KAPAL_ID, A.KAPAL_PEKERJAAN_ID, A.KAPAL_ID, B.NAMA KAPAL_NAMA, 
				   A.STATUS, C.KAPAL_HISTORI_ID, C.KAPAL_PEKERJAAN_ID KAPAL_PEKERJAAN_ID_SEBELUM, 
				   TO_CHAR(C.TANGGAL_MASUK, 'YYYY-MM-DD HH24:MI:SS') TANGGAL_MASUK_SEBELUM, TO_CHAR(C.TANGGAL_KELUAR, 'YYYY-MM-DD HH24:MI:SS') TANGGAL_KELUAR_SEBELUM,
				   D.KAPAL_ID KAPAL_ID_PENGGANTI, (SELECT NAMA FROM PPI_OPERASIONAL.KAPAL X WHERE D.KAPAL_ID = X.KAPAL_ID) KAPAL_PENGGANTI, D.KAPAL_PEKERJAAN_KAPAL_ID_GANTI, KAPAL_PEKERJAAN_KAPAL_ID_CAD,
				...
				FROM PPI_OPERASIONAL.KAPAL_PEKERJAAN_KAPAL A
				LEFT JOIN PPI_OPERASIONAL.KAPAL B ON B.KAPAL_ID = A.KAPAL_ID 
				LEFT JOIN PPI_OPERASIONAL.KAPAL_HISTORI C ON A.KAPAL_PEKERJAAN_ID = C.KAPAL_PEKERJAAN_ID AND B.KAPAL_ID = C.KAPAL_ID AND TANGGAL_KELUAR IS NULL
				LEFT JOIN (SELECT SUBSTR(Y.HISTORI_ID,0,2) HISTORI_ID_PARENT, KAPAL_PEKERJAAN_KAPAL_ID KAPAL_PEKERJAAN_KAPAL_ID_CAD, KAPAL_PEKERJAAN_KAPAL_ID_GANTI, TANGGAL_KELUAR, KAPAL_HISTORI_ID, TANGGAL_MASUK, X.KAPAL_ID FROM PPI_OPERASIONAL.KAPAL_PEKERJAAN_KAPAL X
				INNER JOIN PPI_OPERASIONAL.KAPAL_HISTORI Y ON X.KAPAL_PEKERJAAN_ID = Y.KAPAL_PEKERJAAN_ID AND X.KAPAL_ID = Y.KAPAL_ID  AND Y.TANGGAL_KELUAR IS NULL WHERE X.STATUS = 'C' AND X.TEMP_TANGGAL_AKHIR IS NULL) D ON A.KAPAL_PEKERJAAN_KAPAL_ID = D.KAPAL_PEKERJAAN_KAPAL_ID_GANTI
				WHERE A.KAPAL_PEKERJAAN_KAPAL_ID IS NOT NULL AND A.STATUS = 'A'
				"; 
	...
	function selectByParamsKontrakHistori($paramsArray=array(),$limit=-1,$from=-1, $statement="", $order="ORDER BY KAPAL_PEKERJAAN_KAPAL_ID ASC")
	{
		$str = "
				...
				A.KAPAL_HISTORI_ID, A.KAPAL_ID, A.KAPAL_PEKERJAAN_ID, 
				...
				C.NAMA KAPAL_NAMA, DECODE(HISTORI_PARENT_ID, 0, 'Kapal Utama', 'Kapal Cadangan') INFO_STATUS, A.KETERANGAN
				FROM PPI_OPERASIONAL.KAPAL_HISTORI A
				LEFT JOIN PPI_OPERASIONAL.KAPAL C ON C.KAPAL_ID = A.KAPAL_ID 
				LEFT JOIN PPI_OPERASIONAL.KAPAL_PEKERJAAN_KAPAL B ON A.KAPAL_ID_GANTI = B.KAPAL_PEKERJAAN_KAPAL_ID AND B.STATUS = 'A'
				WHERE 1=1
				"; 
    ...
    function selectByParamsKontrakPengawakan($paramsArray=array(),$limit=-1,$from=-1, $statement="", $order="ORDER BY KAPAL_PEKERJAAN_KAPAL_ID ASC")
	{
		$str = "
				SELECT 
                A.KAPAL_PEKERJAAN_KAPAL_ID, A.KAPAL_PEKERJAAN_KAPAL_ID_GANTI, A.KAPAL_PEKERJAAN_ID, A.KAPAL_ID, B.NAMA KAPAL_NAMA, 
