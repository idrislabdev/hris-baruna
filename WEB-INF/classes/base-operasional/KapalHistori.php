  ...
  class KapalHistori extends Entity{ 
    ...
    **/
	...
	function insertHistori()
	{
		...
		$str = "
				INSERT INTO PPI_OPERASIONAL.KAPAL_HISTORI (
				   KAPAL_HISTORI_ID, KAPAL_ID, ".$this->getField("KONTRAK_NAMA").", 
				   TANGGAL_MASUK, TANGGAL_KELUAR, KONTRAK_ID, HISTORI_ID, HISTORI_PARENT_ID, KAPAL_ID_GANTI) 
 			  	VALUES (
				  ".$this->getField("KAPAL_HISTORI_ID").",
				  '".$this->getField("KAPAL_ID")."',
				  '".$this->getField("KONTRAK_NAMA_ID")."',
				  ".$this->getField("TANGGAL_MASUK").",
				  ".$this->getField("TANGGAL_KELUAR").",
				  '".$this->getField("KONTRAK_ID")."',
				  PPI_OPERASIONAL.HISTORI_ID_GENERATE_KONTRAK('".$this->getField("HISTORI_PARENT_ID")."', '".$this->getField("KONTRAK_NAMA_ID")."', '".$this->getField("KONTRAK_ID")."', '".$this->getField("KONTRAK_NAMA")."'),
    ...
    function updateTanggalMasukKapalPekerjaan()
	{
		$str = "
	...
	function updateTanggalKeluarKapalPekerjaan()
	{
		$str = "
				UPDATE PPI_OPERASIONAL.KAPAL_PEKERJAAN_KAPAL
				SET    
	...
	function deleteKontrakSbpp()
	{
		...
		function deleteHistoriKapal()
	{
		$str = "DELETE FROM PPI_OPERASIONAL.KAPAL_HISTORI
                WHERE 
                  KAPAL_HISTORI_ID = '".$this->getField("KAPAL_HISTORI_ID")."' 
				"; 
		...
		function updateOffHireKontrak()
	{
		$str = "UPDATE PPI_OPERASIONAL.KONTRAK_SBPP_KAPAL SET TANGGAL_KELUAR_KONTRAK = ".$this->getField("TANGGAL_KELUAR_KONTRAK")."
                WHERE 
    ...
    **/ 
	...
	function selectByParamsHistori($paramsArray=array(),$limit=-1,$from=-1, $statement="", $order="ORDER BY KAPAL_HISTORI_ID ASC")
	{
		$str = "
				  SELECT 
				  KAPAL_HISTORI_ID, A.KAPAL_ID, A.KONTRAK_SBPP_ID, COALESCE(B.NAMA, E.NAMA, H.NAMA) KONTRAK_SBPP_NAMA, COALESCE(C.NAMA, F.NAMA, I.NAMA) LOKASI_NAMA, D.KODE, D.NAMA KAPAL_NAMA,
					 A.TANGGAL_MASUK, TANGGAL_KELUAR,

					 A.KAPAL_PEKERJAAN_ID, A.KAPAL_PEKERJAAN_SEWA_ID, A.PENUGASAN_ID
				...
				LEFT JOIN PPI_OPERASIONAL.KAPAL_PEKERJAAN E ON A.KAPAL_PEKERJAAN_ID = E.KAPAL_PEKERJAAN_ID
				LEFT JOIN PPI_OPERASIONAL.LOKASI F ON F.LOKASI_ID = E.LOKASI_ID
				LEFT JOIN PPI_OPERASIONAL.KAPAL G ON A.KAPAL_ID=G.KAPAL_ID
				LEFT JOIN PPI_OPERASIONAL.PENUGASAN H ON A.PENUGASAN_ID = H.PENUGASAN_ID
				LEFT JOIN PPI_OPERASIONAL.LOKASI I ON I.LOKASI_ID = H.LOKASI_ID
				LEFT JOIN PPI_OPERASIONAL.KAPAL J ON A.KAPAL_ID=J.KAPAL_ID
				WHERE 1=1
				"; 
