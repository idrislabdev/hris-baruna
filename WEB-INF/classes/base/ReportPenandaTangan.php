  ...
  class ReportPenandaTangan extends Entity{ 
    ...
    **/
	...
	function insert()
	{
		...
		$this->setField("REPORT_PENANDA_TANGAN_ID", $this->getNextId("REPORT_PENANDA_TANGAN_ID","REPORT_PENANDA_TANGAN")); 

		$str = "
				INSERT INTO REPORT_PENANDA_TANGAN (
	...
	{
		/*Auto-generate primary key(s) by next max value (integer) */
		$str = "UPDATE REPORT_PENANDA_TANGAN A SET
	...
	function delete()
	{
        $str = "DELETE FROM REPORT_PENANDA_TANGAN_USER_LOGIN
                WHERE 
    ...
    **/ 
    function selectByParams($paramsArray=array(),$limit=-1,$from=-1, $statement='', $sOrder='ORDER BY REPORT_PENANDA_TANGAN_ID ASC')
	{
		...
		FROM PPI.REPORT_PENANDA_TANGAN A
		WHERE 1=1 "; 
		//
    ...
    **/ 
	...
	{
		$str = "SELECT COUNT(REPORT_PENANDA_TANGAN_ID) AS ROWCOUNT FROM REPORT_PENANDA_TANGAN A WHERE REPORT_PENANDA_TANGAN_ID IS NOT NULL ".$stat; 
		while(list($key,$val)=each($paramsArray))
		{
			$str .= " AND $key = '$val' ";
		}
	...
	{
		$str = "SELECT COUNT(REPORT_PENANDA_TANGAN_ID) AS ROWCOUNT FROM REPORT_PENANDA_TANGAN A WHERE REPORT_PENANDA_TANGAN_ID IS NOT NULL "; 
		while(list($key,$val)=each($paramsArray))
		{
			$str .= " AND $key LIKE '%$val%' ";
		}
