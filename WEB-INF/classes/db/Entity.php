...
include_once("../WEB-INF/setup/config.php");	
...
class Entity{
	...
	**/
    function execQuery($sql,$inputArray=false){
		$rs = $this->dbEntity->conn->Execute($sql,$inputArray);
		if(!$rs){
			$this->errorMsg = "PESAN ERROR IMAIS : ".$this->dbEntity->getErrorMsg()." (".$this->dbEntity->getErrorNo().")";
		}
		return $rs;
    }
    ...
    function StartTrans(){
		$rs = $this->dbEntity->conn->StartTrans(); 
		if(!$rs){
			$this->errorMsg = "PESAN ERROR : ".$this->dbEntity->getErrorMsg()." (".$this->dbEntity->getErrorNo().")";
		}
		return $rs;
    ...
    function CompleteTrans(){
		$rs = $this->dbEntity->conn->CompleteTrans();
		if(!$rs){
			$this->errorMsg = "PESAN ERROR : ".$this->dbEntity->getErrorMsg()." (".$this->dbEntity->getErrorNo().")";
		}
		return $rs;
    ...
    function FailTrans(){
		$rs = $this->dbEntity->conn->FailTrans();
		if(!$rs){
			$this->errorMsg = "PESAN ERROR : ".$this->dbEntity->getErrorMsg()." (".$this->dbEntity->getErrorNo().")";
		}
		return $rs;
