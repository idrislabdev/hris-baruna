  ...
  class ParametersBase extends Entity{ 
    ...
    **/
    function insert(){
      if(!$this->canInsert())
        ...
        $str = "INSERT INTO PPI_ASSET.parameters 
    ...
    **/
    function update(){
      if(!$this->canUpdate())
        ...
        $str = "UPDATE PPI_ASSET.parameters 
                SET 
    ...
    **/
    function delete(){
      if(!$this->canDelete())
        ...
        $str = "DELETE FROM PPI_ASSET.parameters 
                WHERE 
                  ID_PARAMETERS = '".$this->getField("ID_PARAMETERS")."'"; 
        return $this->execQuery($str);
      }
    ...
    **/
    function selectById($ID_PARAMETERS){
      $str = "SELECT * FROM PPI_ASSET.parameters
              WHERE 
    ...
    **/ 
    function selectByParams($paramsArray=array(),$limit=-1,$from=-1,$orderKey="ID_PARAMETERS",$orderMethod="ASC"){
      $str = "SELECT * FROM PPI_ASSET.parameters WHERE ID_PARAMETERS IS NOT NULL "; 
    ...
    **/ 
    function getCountByParams($paramsArray=array()){
      $str = "SELECT COUNT(ID_PARAMETERS) AS ROWCOUNT FROM PPI_ASSET.parameters WHERE ID_PARAMETERS IS NOT NULL "; 
      while(list($key,$val)=each($paramsArray)){
        $str .= " AND $key = '$val' ";
      }
      $this->select($str); 
