...
function drilldownmenu(setting){
	this.sublevelarrow={src:'../WEB-INF/lib/DrillDownMenu/right2.png', width:'8px', top:'3px', left:'0px'} //full URL to image used to indicate there's a sub level
	this.breadcrumbarrow='../WEB-INF/lib/DrillDownMenu/breadcrumbarrow.png' //full URL to image separating breadcrumb links (if it's defined)
	this.loadingimage='../WEB-INF/lib/DrillDownMenu/loader.gif' //full URL to 'loading' image, if menu is loaded via Ajax
	...
	this.backarrow='../WEB-INF/lib/DrillDownMenu/leftarrow.gif' //full URL to image added in front of back LI 
...
drilldownmenu.prototype.buildmenu=function($){
	...
	this.$uls.each(function(i){ //loop through each UL
		...
		if (i==0){ //if topmost UL
			$('<li class="backcontroltitle">Menu Utama</li>').prependTo($thisul).click(function(e){e.preventDefault()})
			...
			$('<li class="backcontrol"><img src="'+thisdrill.backarrow+'" style="border-width:0" /> Kembali</li>') //add back LI item
