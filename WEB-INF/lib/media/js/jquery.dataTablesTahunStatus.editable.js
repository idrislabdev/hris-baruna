		...
		*/
        ...
        function _fnOnRowMutasi(event) {//tak huapus
			...
			if(id){
				opWidth = 800;
				opHeight = 400;
				opUrl    = 'mutasi_pegawai.php?reqPegawaiId='+id+'&reqId='+properties.pDepartemenId;
        ...
        function _fnOnRowBaca(event) {
			if($("#batas_awal").val() == "" || $("#batas_akhir").val() == ""){
				alert('Isi terlebih dahulu periode awal dan akhir.')
				return false;
			}
			else
				fnReloadAjax('../json/absensi_json.php?reqTanggalAwal=' + $("#batas_awal").val() + '&reqTanggalAkhir=' + $("#batas_akhir").val() );
        }
        ...
        */
        ...
        return this.each(function () {
            ...
            if (oAddDeleteToolbar.length != 0) {
				...
				//oAddDeleteToolbar.append("<br>Periode awal <input type='text' id='batas_awal' style='width:80px' maxlength='10' onkeydown=\"return format_date(event,'batas_awal');\" > akhir <input type='text' id='batas_akhir' style='width:80px' maxlength='10' onkeydown=\"return format_date(event,'batas_akhir');\" >");
				//oAddDeleteToolbar.append("&nbsp;&nbsp;&nbsp;Periode awal <input type='text' id='batas_awal' style='width:80px' maxlength='10' > akhir <input type='text' id='batas_akhir' style='width:80px' maxlength='10' >");
				//oAddDeleteToolbar.append("&nbsp;&nbsp;&nbsp; Tahun <select id='tahun'><option value=''>Semua</option></select> &nbsp; <input type='text' id='reqTanggalLahir' style='width:80px' maxlength='10' >");
				
				oAddDeleteToolbar.append("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<label>Status Pegawai </label> <select id='status_pegawai'></select>"); 
				oStatusPegawaiRowSelect = $("#status_pegawai");
				$.getJSON("../json-simpeg/status_pegawai_lookup_json",
				function(data){
					var z=0;
					for(i=0;i<data.STATUS_PEGAWAI_ID.length; i++)
					{
						$("<option value='" + data.STATUS_PEGAWAI_ID[i] + "' >" + data.NAMA[i] + "</option>").appendTo("#status_pegawai"); 
						
						if( properties.pStatusPegawai == data.STATUS_PEGAWAI_ID[i])
						{
							$("#status_pegawai")[0].options[z].selected = true;
						}
						z++;
					} 
				});
				...
				oAddDeleteToolbar.append("&nbsp;&nbsp;<span style='font-size:14px'>Periode <select id='bulan'><option value='01'>Januari</option><option value='02'>Februari</option><option value='03'>Maret</option><option value='04'>April</option><option value='05'>Mei</option><option value='06'>Juni</option><option value='07'>Juli</option><option value='08'>Agustus</option><option value='09'>September</option><option value='10'>Oktober</option><option value='11'>November</option><option value='12'>Desember</option></select> <select id='tahun'></select></span>");	
				oBulanRowSelect = $("#bulan");
				var bulan_sekarang = "";
				if(properties.pTahun){
					var bulan_select = properties.pBulan;
					var tahun_select = properties.pTahun;
				}
				else{
					var bulan_select = parseInt(currentTime.getMonth()) + 1;
					var tahun_select = currentTime.getFullYear()
				}
				
				if( bulan_select < 10)
					bulan_sekarang = bulan_select;
				else
					bulan_sekarang = parseInt(bulan_select);
				
				
				$("#bulan").val(bulan_sekarang);
				
				oTahunRowSelect = $("#tahun");
				for(i=currentTime.getFullYear()-1;i<=currentTime.getFullYear(); i++)
				{
					$("<option value='" + i + "'>" + i + "</option>").appendTo("#tahun");			
				}
				//$("#tahun").val(currentTime.getFullYear());
				$("#tahun").val(tahun_select);
							
				oBulanRowSelect.change(function () { 
					window.document.location.href = properties.pLocation + '?reqTahun=' + $("#tahun").val() + '&reqBulan=' + $("#bulan").val() + '&reqStatusPegawai=' + $("#status_pegawai").val() + '&reqDepartemen=' + properties.pDepartemenId;
				});
				...
				oStatusPegawaiRowSelect.change(function () { 
					window.document.location.href = properties.pLocation + '?reqTahun=' + $("#tahun").val() + '&reqBulan=' + $("#bulan").val() + '&reqStatusPegawai=' + $("#status_pegawai").val() + '&reqDepartemen=' + properties.pDepartemenId;
				});
            }
