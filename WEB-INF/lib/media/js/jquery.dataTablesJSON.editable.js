...
(function ($) {
    ...
    $.fn.makeEditable = function (options) {
		...
		oKartuKendaliKatalogRowButton, oTunjukSilangKatalogRowButton;
...
//			alert(mySplitResult[6]+'--');
        ...
        function _fnOnRowBaca(event) {
			fnReloadAjax(properties.pReloadTarget + "&reqPeriode="+ $("#reqPeriode").val());
        }
		...
		function _fnOnRowKartuKendaliKatalog(event) {
			var id = fnGetCellID($('tr.' + properties.sSelectedRowClass + ' td', oTable)[0]);
			//alert(id+'--'+'--'+$("#bulan").val()+'--'+$("#tahun").val()+'--'+$("#batas").val())
			opWidth = 350;
			opHeight = 250;
			opUrl    = 'cetak_kartu_kendali_katalog.php?reqId='+id;
			//opUrl    = 'cetak_kartu_kendali.php?reqId='+properties.pSatkerId+'&reqPegawaiId='+id+'&reqBulan='+$("#bulan").val()+'&reqTahun='+$("#tahun").val()+'&reqBatas='+$("#batas").val();
			var left = (screen.width/2)-(opWidth/2);
			var top = (screen.height/2)-(opHeight/2);
			newWindow = window.open(opUrl, "", "width = " + opWidth + "px, height = " + opHeight + "px, resizable = 1, scrollbars, top="+top+", left="+left);
			newWindow.focus();
		...
		function _fnOnRowTunjukSilangKatalog(event) {
			var id = fnGetCellID($('tr.' + properties.sSelectedRowClass + ' td', oTable)[0]);
			//alert(id+'--'+'--'+$("#bulan").val()+'--'+$("#tahun").val()+'--'+$("#batas").val())
			opWidth = 350;
			opHeight = 250;
			opUrl    = 'cetak_tunjuk_silang_katalog.php?reqId='+id;
			//opUrl    = 'cetak_kartu_kendali.php?reqId='+properties.pSatkerId+'&reqPegawaiId='+id+'&reqBulan='+$("#bulan").val()+'&reqTahun='+$("#tahun").val()+'&reqBatas='+$("#batas").val();
			var left = (screen.width/2)-(opWidth/2);
			var top = (screen.height/2)-(opHeight/2);
			newWindow = window.open(opUrl, "", "width = " + opWidth + "px, height = " + opHeight + "px, resizable = 1, scrollbars, top="+top+", left="+left);
        ...
        */
        ...
        return this.each(function () {
			...
			oKartuKendaliKatalogRowButton = $('#btnKartuKendaliKatalogRow');
            if (oKartuKendaliKatalogRowButton.length != 0)
            {
                oKartuKendaliKatalogRowButton.click(_fnOnRowKartuKendaliKatalog);
            }
            else {
                oKartuKendaliKatalogRowButton = null;
            }
			
			oTunjukSilangKatalogRowButton = $('#btnTunjukSilangKatalogRow');
            if (oTunjukSilangKatalogRowButton.length != 0)
            {
                oTunjukSilangKatalogRowButton.click(_fnOnRowTunjukSilangKatalog);
            }
            else {
                oTunjukSilangKatalogRowButton = null;
            }	
