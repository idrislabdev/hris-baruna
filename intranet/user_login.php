...
<link rel="stylesheet" type="text/css" href="../WEB-INF/lib/DataTables-1.10.6/media/css/jquery.dataTables.css">
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,		
			  "sAjaxSource": "../json-intranet/user_login_json.php?reqStatus=1-1,5",						  
			...
			/* Click event handler */
			  ...
			  $('#btnEdit').on('click', function () {
				  if(anSelectedData == "")
					  return false;				
				  window.parent.OpenDHTML('user_login_add.php?reqId='+anSelectedId, 'Office Management - Administrasi Intranet', '900', '580');	
			  });
			  ...
			  $('#btnDeleteRow').on('click', function () {
				...
				if ( confirm( "Apakah anda yakin, menghapus data ini ?" ) ) {
					$.getJSON('../json-intranet/delete.php?reqMode=user_login&id='+anSelectedId, function (data) 
					{
			  ...
			  $('#btnReset').on('click', function () {
				  if(anSelectedData == "")
				  	...
				  	//alert (anSelectedPass);
				  if(confirm('Reset password user terpilih?'))
				  {
					  $.getJSON('../json-intranet/user_login_reset.php?reqId='+anSelectedId+'', function (data) 
					  {
						  $.each(data, function (i, SingleElement) {
						  });
					  });									
			  ...
			  return false;
			   ...
			   $('.vmenu .inner_li span').on('click',function() {												
					  if($(this).text() == 'Aktif')
					  {
						  $.getJSON('../json-intranet/user_login_set_status.php?reqId='+anSelectedId+'&reqNilai=1', function (data) 
						  {
							  $.each(data, function (i, SingleElement) {
							  });
						  });									
						  oTable.fnUpdate('Aktif', anSelectedPosition, 7, false);
					  }
					  else if($(this).text() == 'Non-Aktif')
					  {
						  $.getJSON('../json-intranet/user_login_set_status.php?reqId='+anSelectedId+'&reqNilai=0', function (data) 
						  {
							  $.each(data, function (i, SingleElement) {
								  
							  });
						  });									
						  oTable.fnUpdate('Non-Aktif', anSelectedPosition, 7, false);
					  }
					  $('.vmenu').hide();
					  $('.overlay').hide();
			  });			   
			  ...
			  $("#reqGroupUser").change(function() { 
				 fnReloadAjax("../json-intranet/user_login_json.php?reqGroupUser="+$("#reqGroupUser").val()+"&reqStatus="+ $("#reqStatus").val());				 	 
			  });
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png">Data User Login</span>
            </div>            
            </td>
        </tr>
    </table>
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul>
            <li>
            <a href="#" id="btnAdd" onClick="window.parent.OpenDHTML('user_login_add.php', 'Office Management - Administrasi Intranet', '900', '580');" title="Tambah">Tambah</a>
            <a href="#" id="btnEdit" title="Edit">Ubah</a>
            <a href="#" id="btnDeleteRow" title="Hapus">Hapus</a>
            <a href="#" id="btnReset" title="Reset">Reset Password</a>
            </li>        
        </ul>
    </div>
    <div style="position: relative; z-index:10">
    <div style="position: absolute; margin-top:6px; margin-left:182px; z-index:9999; font-size:12px;">
    	Group User : <select name="reqGroupUser" id="reqGroupUser">
        					<option value="">Semua</option>
        				<?
                        while($user_group->nextRow())
						{
						?>
                        	<option value="<?=$user_group->getField("USER_GROUP_ID")?>"><?=$user_group->getField("NAMA")?></option>	
						<?	
						}
						?>
                        </select>&nbsp;&nbsp;
       Status (Login/Pegawai) : <select name="reqStatus" id="reqStatus">
       				<option value="1-1,5">Aktif/Aktif</option>
       				<option value="1-2,3,4,6">Aktif/Non-Aktif</option>
       				<option value="0-1,5">Non-Aktif/Aktif</option>
       				<option value="0-2,3,4,6">Non-Aktif/Non-Aktif</option>
                </select>
    </div>
    </div>    
    ...
    <thead>
        <tr>
            ...
            <th width="150px">Group User</th>
            <th width="150px">Nama</th> 
            <th width="150px">User Login</th>                                            
