...
<style type="text/css" media="screen">    
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "sAjaxSource": "../json-absensi/absensi_json.php",
			  "sScrollX": "100%",								  
			  "sScrollXInner": "100%",
			  "sPaginationType": "full_numbers",
			  "fnRowCallback": function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
				if ( aData[7] == 'Disetujui') {
					var i=0;
					for (i=0;i<=8;i++)
					{
					jQuery('td:eq('+i+')', nRow).addClass('setuju');
					}
				} 
				else if( aData[7] == 'Ditolak') {
					var i=0;
					for (i=0;i<=8;i++)
					{
					jQuery('td:eq('+i+')', nRow).addClass('tolak');
					}
				}
				else {
					var i=0;
					for (i=0;i<=8;i++)
					{
					jQuery('td:eq('+i+')', nRow).addClass('konfirmasi');
					}
				}
				return nRow;
			   }
			...
			sDeleteURL: "../json-absensi/delete.php?reqMode=absensi",
			sAddHttpMethod: "GET",
			"aoColumns": [
				{ 	cssclass: "required" },
				{
			  ...
			  $('#btnEdit').on('click', function () {
				  if(anSelectedData == "")
					  return false;				
				  window.parent.OpenDHTML('absensi_add.php?reqMode=update&reqId='+anSelectedId, 'Office Management - Aplikasi Presensi', '600', '350');	
			  });
			  ...
			  return false;
			   ...
			   $('.vmenu .inner_li span').on('click',function() {												
					  if($(this).text() == 'Disetujui')
					  {
						 ...
						 $.getJSON('../json-absensi/absensi_json_set.php?reqId='+anSelectedId+'&reqNilai=1', function (data) 
						  {
					  ...
					  else if($(this).text() == 'Ditolak')
					  {
						 ...
						 {	
						  ...
						  $.getJSON('../json-absensi/absensi_json_set.php?reqId='+anSelectedId+'&reqNilai=2', function (data) 
						  {
							  $.each(data, function (i, SingleElement) {
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Data Absensi</span>
            </div>            
            </td>
        </tr>
    </table>
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul>
            <li>
            <a href="#" id="btnAdd" onClick="window.parent.OpenDHTML('absensi_add.php?reqMode=insert', 'Office Management - Aplikasi Presensi', '600', '500');" title="Tambah">Tambah</a>
