...
$lock_proses = $proses_presensi_lock->getProsesPresensiLock(array("JENIS_PROSES" => "KOREKSI_ABSEN_DARAT", "PERIODE" => $reqBulan.$reqTahun));
...
<style type="text/css" media="screen">    
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "bProcessing": true,
			  "bServerSide": true,								
			  "sAjaxSource": "../json-absensi/absensi_koreksi_json.php?reqTahun=<?=$reqTahun?>&reqBulan=<?=$reqBulan?>&reqDepartemen=<?=$reqDepartemen?>&reqStatusPegawai=<?=$reqStatusPegawai?>",
			  ...
			  $('#example tbody').on( 'click', 'tr', function () {
					...
					row.addClass("selected");
					  ...
					  anSelectedKelompok = element[2];
			  });
			  
			  $('#btnKoreksi').on('click', function () {
				  if(anSelectedData == "")
					  return false;				
				  window.parent.OpenDHTML('absensi_koreksi_add.php?reqId='+anSelectedId+'&reqKelompok='+anSelectedKelompok+'&reqBulan='+$("#reqBulan").val()+'&reqTahun='+$("#reqTahun").val()+'', 'Office Management - Aplikasi Presensi', '880', '550');	
			  });
			  
			  $('#btnApproval').on('click', function () {
			  	  if(confirm('Approval koreksi presensi pegawai darat?'))
				  {
					  $.getJSON("../json-absensi/proses_presensi_set_lock.php?reqJenisProses=KOREKSI_ABSEN_DARAT&reqPeriode="+ $("#reqBulan").val() + $("#reqTahun").val(),
					  function(data){
					  });	
			  ...
			  $('#btnProses').on('click', function () {
					$.getJSON("../json-absensi/absensi_koreksi_validasi_json.php?reqPeriode="+$("#reqBulan").val()+""+$("#reqTahun").val(),
					function(data){
						jumlah = data.DATA;
					});		
					if(jumlah == 0)		  
			      		oTable.fnReloadAjax("../json-absensi/absensi_koreksi_json.php?reqMode=proses&reqTahun="+$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val()+"&reqDepartemen="+$('#cc').combotree('getValue')+"&reqJenisPegawai="+ $("#reqJenisPegawai").val());
					else
			      	{
						if(confirm("Proses ulang rekap akan mengakibatkan data presensi peg. darat sebelumnya terhapus. Lanjutkan?"))
				      		oTable.fnReloadAjax("../json-absensi/absensi_koreksi_json.php?reqMode=proses&reqTahun="+$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val()+"&reqDepartemen="+$('#cc').combotree('getValue')+"&reqJenisPegawai="+ $("#reqJenisPegawai").val());
			  ...
			  $("#reqJenisPegawai").change(function() { 
				 ...
				 oTable.fnReloadAjax("../json-absensi/absensi_koreksi_json.php?reqTahun="+$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val()+"&reqDepartemen="+$('#cc').combotree('getValue')+"&reqJenisPegawai="+ $("#reqJenisPegawai").val()+"&reqJamKerja="+ $("#reqJamKerja").val());				 	 
			  });
			  ...
			  $("#reqBulan").change(function() { 
				$.getJSON("../json-absensi/proses_presensi_validasi_lock.php?reqPeriode="+$("#reqBulan").val()+""+$("#reqTahun").val(),
					function(data){
						jumlah = data.DATA;
						if(jumlah == 1)
							$('.toggle').css({"display":"none"});
			  ...
			  $('#cc').combotree({
					onSelect: function(param){
 						 refreshfnHide();
						 oTable.fnReloadAjax("../json-absensi/absensi_koreksi_json.php?reqTahun="+$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val()+"&reqDepartemen="+param.id+"&reqJenisPegawai="+ $("#reqJenisPegawai").val()+"&reqJamKerja="+ $("#reqJamKerja").val());				 	 
					}
			  });	  
			...
			function daysInMonth(month, year) {
			  ...
			  $('#btnCetak').on('click', function () {
				  ...
				  newWindow = window.open('absensi_koreksi_cetak.php?reqBulan='+$("#reqBulan").val()+'&reqTahun='+ $("#reqTahun").val()+'&reqDepartemen='+$('#cc').combotree('getValue')+'&reqStatusPegawai=<?=$reqStatusPegawai?>&reqJenisPegawai='+ $("#reqJenisPegawai").val(), 'Cetak');
				  newWindow.focus();
			  });				  
...
<div id="wadah">
  ...
  <div style="position: relative; z-index:10">
    <div style="position: absolute; margin-top:6px; margin-left:180px; z-index:9999; font-size:12px;">
    	...
    	Jam Kerja : <select name="reqJamKerja" id="reqJamKerja">
        					<option value="">Semua Darat</option>
                            <option value="1">Normal</option>
                            <option value="3">Piket</option>
                            <option value="4">Awak Kapal</option>
                        </select>                           
    </div>
