...
$worksheet->write		(1, 1, "REKAP JAM KERJA BULAN ".getNamePeriode($periode), $text_format);

$worksheet->write(3, 1, "NAMA", $text_format_line_bold);

for ($i=2; $i<=32; $i++)
{
	$worksheet->write(3, $i, $i-1, $text_format_merge_line_bold);
}
...
while($absensi_rekap->nextRow())
{
	...
	for ($k=2; $k<=32; $k++)
	{
			$worksheet->write($row, $k, $absensi_rekap->getField('HARI_'.($k-1)), $text_format_line);
	}
	$row++;
}
...
header("Content-Type: application/x-msexcel; name=\"cetak_jam_kerja_rekap.xls\"");
header("Content-Disposition: inline; filename=\"cetak_jam_kerja_rekap.xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
?>