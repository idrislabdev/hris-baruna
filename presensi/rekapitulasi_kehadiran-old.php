        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			...
			"aoColumns": [
			  ...
			  $('#btnCetak').on('click', function () {
				  ...
				  newWindow = window.open('rekapitulasi_kehadiran_cetak.php?reqBulan='+$("#reqBulan").val()+'&reqTahun='+ $("#reqTahun").val()+'&reqDepartemen=<?=$reqDepartemen?>&reqJenisPegawai='+ $("#reqJenisPegawai").val(), 'Cetak');
				  newWindow.focus();
			  });

			  $("#reqJenisPegawai").change(function() { 
			  
				 oTable.fnReloadAjax("../json-absensi/rekapitulasi_kehadiran_json.php?reqTahun="+$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val()+"&reqDepartemen=<?=$reqDepartemen?>&reqJenisPegawai="+ $("#reqJenisPegawai").val());				 	 
			  });
