...
<style type="text/css" media="screen">    
    ...
    $(document).ready( function () {
        ...
        var oTable = $('#example').dataTable({ bJQueryUI: true,"iDisplayLength": 50,
			  ...
			  "sAjaxSource": "../json-absensi/absensi_ijin_cuti_json.php?reqTahun=<?=$reqTahun?>&reqBulan=<?=$reqBulan?>",
			  "sScrollX": "100%",								  
			  "sScrollXInner": "100%",
			...
			/* Click event handler */
			  ...
			  $('#example tbody').on( 'click', 'tr', function () {
					if ( $(this).hasClass('selected') ) {
					  ...
					  anSelectedValidasi = element[7];
			  });
			  
			  $("#reqBulan").change(function() {
				 oTable.fnReloadAjax("../json-absensi/absensi_ijin_cuti_json.php?reqTahun="+$("#reqTahun").val()+"&reqBulan="+$("#reqBulan").val());				 	 
			  });
			  ...
			  $('#btnEdit').on('click', function () {
				  if(anSelectedData == "")
					  return false;				
				  window.parent.OpenDHTML('absensi_ijin_add.php?reqMode=update&reqId='+anSelectedId, 'Office Management - Aplikasi Presensi', '600', '350');	
			  });
			  ...
			  return false;
			   ...
			   $('.vmenu .inner_li span').on('click',function() {												
					  if($(this).text() == 'Disetujui')
					  {
						  if(anSelectedValidasi == 'Disetujui')
						  {	
						 	alert('Data telah disetujui, tidak dapat dirubah.');
						 	return;
						  }
						  $.getJSON('../json-absensi/absensi_ijin_json_set.php?reqId='+anSelectedId+'&reqNilai=1', function (data) 
						  {
							  $.each(data, function (i, SingleElement) {
							  });
						  });						
						  oTable.fnUpdate('Disetujui', anSelectedPosition, 7, false);
						  oTable.fnUpdate('<?=$userLogin->nama?>', anSelectedPosition, 8, false);
						  anSelectedValidasi = 'Disetujui';
					  }
					  else if($(this).text() == 'Ditolak')
					  {
						  ...
						  $.getJSON('../json-absensi/absensi_ijin_json_set.php?reqId='+anSelectedId+'&reqNilai=2', function (data) 
						  {
							  $.each(data, function (i, SingleElement) {
...
<div id="wadah">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <div id="judul-halaman">
                <span><img src="../WEB-INF/images/panah-judul.png"> Data Ijin</span>
            </div>            
            </td>
        </tr>
    </table>
    ...
    <div id="bluemenu" class="bluetabs" style="background:url(../WEB-INF/css/media/bluetab.gif);">    
        <ul>
            <li>
            <a href="#" id="btnAdd" onClick="window.parent.OpenDHTML('absensi_ijin_add?reqMode=insert.php', 'Office Management - Aplikasi Presensi', '600', '350');" title="Tambah">Tambah</a>
    ...
    <div style="position: absolute; margin-top:6px; margin-left:180px; z-index:9999; font-size:12px;">
 		Periode : <select name="reqBulan" id="reqBulan">
        		  <?
            for($i=1; $i<=12; $i++)
            {
    ...
    <!--RIGHT CLICK EVENT -->
    <div class="vmenu">
        <div class="first_li"><span>Validasi</span>
            <div class="inner_li">
                <span>Disetujui</span> 
                <span>Ditolak</span>
			</div>
