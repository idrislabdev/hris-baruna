...
class setting_json extends CI_Controller {
	...
	function setting_aplikasi_json()
	{
		include_once("../WEB-INF/classes/utils/UserLogin.php");
		include_once("../WEB-INF/functions/string.func.php");
		include_once("../WEB-INF/functions/default.func.php");
		...
		$aColumns = array("KODE", "KETERANGAN", "NILAI");
		$aColumnsAlias = array("KODE", "KETERANGAN", "NILAI");
		...
		$allRecord = $setting_aplikasi->getCountByParams(array());
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $setting_aplikasi->getCountByParams(array(), " AND (UPPER(KETERANGAN) LIKE '%".strtoupper($_GET['sSearch'])."%')");

		$setting_aplikasi->selectByParams(array(), $dsplyRange, $dsplyStart, " AND (UPPER(KETERANGAN) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
		...
		while($setting_aplikasi->nextRow())
		{
			...
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				if($aColumns[$i] == "TANGGAL")
					$row[] = getFormattedDate($setting_aplikasi->getField($aColumns[$i]));
				else
					$row[] = $setting_aplikasi->getField($aColumns[$i]);
			}
