...
class informasi_json extends CI_Controller {
	...
	function json()
	{
		...
		$aColumns = array("INFORMASI_ID", "DEPARTEMEN", "NAMA", "TANGGAL", "KETERANGAN", "STATUS");
		$aColumnsAlias = array("INFORMASI_ID", "C.NAMA", "A.NAMA", "TANGGAL", "KETERANGAN", "STATUS");
		...
		$allRecord = $informasi->getCountByParams(array(), $statement);
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $informasi->getCountByParams(array(), $statement." AND (UPPER(A.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%') ");

		$informasi->selectByParams(array(), $dsplyRange, $dsplyStart, $statement." AND (UPPER(A.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
		...
		while($informasi->nextRow())
		{
			...
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				if($aColumns[$i] == "TANGGAL")
					$row[] = getFormattedDate($informasi->getField($aColumns[$i]));
