...
class penanda_json extends CI_Controller {
	...
	function penanda_tangan_json()
	{
		...
		$aColumns = array("PENANDA_TANGAN_ID", "NAMA", "JABATAN");
		$aColumnsAlias = array("PENANDA_TANGAN_ID", "NAMA", "JABATAN");
		...
		if ( isset( $_GET['iSortCol_0'] ) )
		{
			...
			if ( trim($sOrder) == "ORDER BY PENANDA_TANGAN_ID asc" )
			{
				/*
				...
				*/
				$sOrder = " ORDER BY PENANDA_TANGAN_ID DESC";
		...
		$allRecord = $penanda_tangan->getCountByParams(array());
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $penanda_tangan->getCountByParams(array(), " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')");

		$penanda_tangan->selectByParams(array(), $dsplyRange, $dsplyStart, " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
		...
		while($penanda_tangan->nextRow())
		{
			...
			{
				$row[] = $penanda_tangan->getField($aColumns[$i]);
			}
