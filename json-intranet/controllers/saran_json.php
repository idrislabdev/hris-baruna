...
class saran_json extends CI_Controller {
	...
	function saran_dan_kritik_json()
	{
		include_once("../WEB-INF/classes/utils/UserLogin.php");
		include_once("../WEB-INF/functions/string.func.php");
		include_once("../WEB-INF/functions/default.func.php");
		include_once("../WEB-INF/functions/date.func.php");
		include_once("../WEB-INF/classes/base/CustomerService.php");
		...
		$reqKapalId = httpFilterGet("reqKapalId");

		ini_set("memory_limit","500M");
		ini_set('max_execution_time', 520);

		$aColumns = array("TANGGAL", "NO_TELEPON", "PESAN");
		$aColumnsAlias = array("TANGGAL", "NO_TELEPON", "PESAN");
		...
		if ( isset( $_GET['iSortCol_0'] ) )
		{
			...
			if ( trim($sOrder) == "ORDER BY NO_TELEPON asc" )
			{
				/*
		...
		{}
			...
			$statement = " AND A.KAPAL_ID = '".$reqKapalId."'";*/

		$allRecord = $customer_service->getCountByParams(array(),$statement);
		//echo $customer_service->query;
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $customer_service->getCountByParams(array(), $statement." AND ((UPPER(NO_TELEPON) LIKE '%".strtoupper($_GET['sSearch'])."%')) ");

		//echo $customer_service->query;
		$customer_service->selectByParams(array(), $dsplyRange, $dsplyStart, $statement." AND ((UPPER(NO_TELEPON) LIKE '%".strtoupper($_GET['sSearch'])."%')) " , $sOrder);
		...
		while($customer_service->nextRow())
		{
			$row = array();
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				if($aColumns[$i] == "TANGGAL")
					$row[] = getFormattedDate($customer_service->getField($aColumns[$i]));
				else if($aColumns[$i] == "KETERANGAN")
					$row[] = truncate($customer_service->getField($aColumns[$i]), 5)."...";
				else
					$row[] = $customer_service->getField($aColumns[$i]);
			}
