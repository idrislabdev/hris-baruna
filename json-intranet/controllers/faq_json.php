...
class faq_json extends CI_Controller {
	...
	function json()
	{
		...
		$aColumns = array("FAQ_ID", "NO_URUT", "PERTANYAAN", "JAWABAN");
		$aColumnsAlias = array("FAQ_ID", "NO_URUT", "PERTANYAAN", "JAWABAN");
		...
		$allRecord = $faq->getCountByParams(array());
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $faq->getCountByParams(array(), " AND (UPPER(PERTANYAAN) LIKE '%".strtoupper($_GET['sSearch'])."%')");

		$faq->selectByParams(array(), $dsplyRange, $dsplyStart, " AND (UPPER(PERTANYAAN) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
		...
		while($faq->nextRow())
		{
			...
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				if($aColumns[$i] == "TANGGAL")
					$row[] = getFormattedDate($faq->getField($aColumns[$i]));
				else if(($aColumns[$i] == "PERTANYAAN") || ($aColumns[$i] == "JAWABAN"))
					$row[] = truncate($faq->getField($aColumns[$i]), 5)."...";
				else
