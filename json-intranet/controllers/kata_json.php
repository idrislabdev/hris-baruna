...
class kata_json extends CI_Controller {
	...
	function kata_mutiara_json()
	{
		...
		$aColumns = array("KATA_MUTIARA_ID", "NAMA", "KETERANGAN", "STATUS");
		$aColumnsAlias = array("KATA_MUTIARA_ID", "NAMA", "KETERANGAN", "STATUS");
		...
		$allRecord = $kata_mutiara->getCountByParams(array());
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $kata_mutiara->getCountByParams(array(), " AND (UPPER(A.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')");

		$kata_mutiara->selectByParams(array(), $dsplyRange, $dsplyStart, " AND (UPPER(A.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
		...
		while($kata_mutiara->nextRow())
		{
			...
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				if($aColumns[$i] == "TANGGAL")
					$row[] = getFormattedDate($kata_mutiara->getField($aColumns[$i]));
				else if($aColumns[$i] == "KETERANGAN")
					$row[] = truncate($kata_mutiara->getField($aColumns[$i]), 5)."...";
				else
					$row[] = $kata_mutiara->getField($aColumns[$i]);
			}
	...
	function kata_mutiara_set_status()
	{
		/* INCLUDE FILE */
		include_once("../WEB-INF/classes/utils/UserLogin.php");
		include_once("../WEB-INF/classes/base/KataMutiara.php");
