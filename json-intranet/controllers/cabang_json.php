...
class cabang_json extends CI_Controller {
	...
	function json()
	{
		...
		$aColumns = array("CABANG_ID", "KODE", "NAMA", "KETERANGAN");
		$aColumnsAlias = array("CABANG_ID", "KODE", "NAMA", "KETERANGAN");
		...
		if ( isset( $_GET['iSortCol_0'] ) )
		{
			...
			if ( trim($sOrder) == "ORDER BY CABANG_ID asc" )
			{
				...
				*/
				$sOrder = " ORDER BY CABANG_ID DESC";
		...
		$allRecord = $cabang->getCountByParams(array());
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $cabang->getCountByParams(array(), " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')");

		$cabang->selectByParams(array(), $dsplyRange, $dsplyStart, " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
		...
		while($cabang->nextRow())
		{
			...
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				if($aColumns[$i] == "TANGGAL")
					$row[] = getFormattedDate($cabang->getField($aColumns[$i]));
				else if($aColumns[$i] == "KETERANGAN")
					$row[] = truncate($cabang->getField($aColumns[$i]), 5)."...";
				else
