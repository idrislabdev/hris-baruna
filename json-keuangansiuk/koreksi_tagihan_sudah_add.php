...
$reqTotalBayar = 0;
for($i=0;$i<count($reqTagihan);$i++)
{
	$reqTotalBayar += dotToNo($reqTagihan[$i]);
}
...
$kptt_nota->setField('TIPE_TRANS', "JRR-KPT-44");
$kptt_nota->setField('NO_NOTA', $reqNoBukti);
...
$kptt_nota->setField('JML_VAL_TRANS', dotToNo((int)$reqTotalBayar));
$kptt_nota->setField('JML_RP_TRANS', floor(dotToNo((int)$reqTotalBayar) * dotToNo((int)$reqKursValuta)));
...
$kptt_nota->setField('PROGRAM_NAME', "KPTT_EKAS_PEMBATALAN_JRR");			
...
if($kptt_nota->insert())
{
	for($i=0; $i<count($reqNoPpkb); $i++)
	{
		...
		else
		{
			...
			$kptt_nota_d->setField('TIPE_TRANS', "JRR-KPT-44");
			$kptt_nota_d->setField('NO_NOTA', $reqNoBukti);
			...
			$kptt_nota_d->setField('JML_VAL_TRANS', dotToNo($reqTagihan[$i]));
			$kptt_nota_d->setField('STATUS_KENA_PAJAK', "");
			$kptt_nota_d->setField('JML_VAL_PAJAK', "0");
			$kptt_nota_d->setField('JML_RP_TRANS', floor(dotToNo($reqTagihan[$i]) * dotToNo($reqKursValuta)));
			...
			$kptt_nota_d->setField('KD_SUB_BANTU', $reqNIS[$i]);	
			$kptt_nota_d->setField('KD_BUKU_PUSAT', "000.00.00");
			$kptt_nota_d->setField('KD_D_K', "K");
			$kptt_nota_d->setField('PREV_NO_NOTA', "");
			$kptt_nota_d->setField('KET_TAMBAHAN', "KOREKSI NOTA NO : ".$reqNoRef3);
			$kptt_nota_d->setField('STATUS_PROSES', "1");	
			$kptt_nota_d->setField('FLAG_JURNAL', "");
			$kptt_nota_d->setField('NO_REF1', $reqNoUrut[$i]);
			$kptt_nota_d->setField('NO_REF2', $reqNoPpkb[$i]); 
			...
			$kptt_nota_d->setField('PROGRAM_NAME', "KPTT_EKAS_PEMBATALAN_JRR");
			$kptt_nota_d->setField('KD_TERMINAL', "");
			$kptt_nota_d->setField('SISA', dotToNo($reqSisaPiutang[$i]));
