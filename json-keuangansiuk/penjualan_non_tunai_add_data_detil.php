...
if($reqMode == "insert")
{
	for($i=0; $i<count($reqKlasTrans); $i++)
	{
		...
		$kptt_nota_d->setField('TIPE_TRANS', $reqTipeTrans);
		...
		$kptt_nota_d->setField('KD_BUKU_BESAR', "(SELECT KD_BUKU_BESAR1 FROM SIUK.KBBR_TIPE_TRANS_D X WHERE X.TIPE_TRANS = '".$reqTipeTrans."' AND KLAS_TRANS = '".$reqKlasTrans[$i]."')");
		...
		$kptt_nota_d->setField('KET_TAMBAHAN', $reqKeterangan[$i]);
