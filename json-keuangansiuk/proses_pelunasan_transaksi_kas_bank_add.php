...
$kptt_nota->setField('JNS_JUAL', "TX");
...
if($kptt_nota->insert())
{
	$kptt_nota->selectByParams(array("NO_NOTA" => $reqNoBukti));
	$kptt_nota->firstRow();
	...
	for($i=0; $i<count($reqNoPpkb); $i++)
	{
		...
		$kptt_nota_d->setField('KD_BUKU_BESAR', $kd_bb_kusto);
		...
		$kptt_nota_d->setField('NO_REF1', $reqNoPpkb[$i]);
		$kptt_nota_d->setField('NO_REF2', "");
		$kptt_nota_d->setField('NO_REF3', $reqNoPpkb[$i]);
	...
	$kbbt_jur_bb->setField("NO_NOTA", $reqNoBukti);
	$kbbt_jur_bb->callGenerateJurnalKnDnPiut();
