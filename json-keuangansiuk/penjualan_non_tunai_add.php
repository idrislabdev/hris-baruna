...
$kptt_nota->setField('KD_BB_KUSTO', "(SELECT DECODE('".$reqValutaNama."', 'IDR', COA1, COA1) FROM KBBR_COA_KUST_KLIEN WHERE KD_KUST_KLIEN = 1 AND BADAN_USAHA = '".$reqBadanUsaha."')");
...
if($kptt_nota->insert())
{
	...
	for($i=0; $i<count($reqKlasTrans); $i++)
	{
		...
		if($reqKdBukuBesar[$i] == "")
		{
			...
			$kbbr_tipe_trans_d->selectByParams(array("TIPE_TRANS"=>$reqSegmen, "KLAS_TRANS" => $reqKlasTrans[$i]), -1, -1, " AND (KD_AKTIF = 'A' OR KD_AKTIF IS NULL)");
