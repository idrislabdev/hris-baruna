...
$aColumns = array("PERIODE", "BULAN", "DEBET", "KREDIT", "SALDO_AKHIR");
$aColumnsAlias = array("PERIODE", "BULAN", "DEBET", "KREDIT", "SALDO_AKHIR");
...
$kbbt_neraca_saldo->selectByParamsDetil($reqTahun, $reqKodeValuta, $reqKodeBukuBesar, $reqKodeSubBantu);     		
...
while($kbbt_neraca_saldo->nextRow())
{
	...
	{
		if($aColumns[$i] == "DEBET" || $aColumns[$i] == "KREDIT")
			$row[] = numberToIna($kbbt_neraca_saldo->getField($aColumns[$i]), "");
		else if($aColumns[$i] == "SALDO_AKHIR")
		{
			$saldo_akhir = $saldo_akhir + round($kbbt_neraca_saldo->getField("DEBET") - $kbbt_neraca_saldo->getField("KREDIT"), 2);
			$row[] = numberToIna($saldo_akhir, "");	
		}
