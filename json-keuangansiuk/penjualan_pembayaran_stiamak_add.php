...
$reqHapusId = httpFilterPost("reqHapusId");
...
$kptt_nota->setField('NO_EKSPEDISI', $reqHapusId);	
...
$kptt_nota->setField('DEPARTEMEN_KELAS_ID', "70");
$kptt_nota->setField('DEPARTEMEN_ID', "16");
$kptt_nota->setField('DEPARTEMEN_KELAS', "STIAMAK BARUNAWATI SURABAYA - STIAMAK");		
...
if($kptt_nota->insert())
{
	...
	$hapusPelanggan = "'X'";	
	$arrHapus = explode(";", $reqHapusId);
	for($i=0;$i<count($arrHapus);$i++)
	{
		if($arrHapus[$i] == "")
		{}
		else
			$hapusPelanggan .= ",'".$arrHapus[$i]."'";	
	}
	
	$kptt_nota_d->setField('MPLG_KODE_HAPUS', $hapusPelanggan);
	
	
	
	
	$kptt_nota_d->insertSTIAMAK();
	//BUAT JURNAL
