...
if($kptt_nota->insert())
{
	...
	/* UPDATE NPWP*/
	$safm_pelanggan->setField("MPLG_KODE", $reqNoPelanggan);
	$safm_pelanggan->setField("MPLG_NPWP", $reqNpwp);
	$safm_pelanggan->updateNpwp();
	...
	for($i=0; $i<count($reqKlasTrans); $i++)
	{
		...
		$kptt_nota_d->setField('KET_TAMBAHAN', $reqKeteranganTambah[$i]);
