...
$reqJenisTransaksi = httpFilterPost("reqJenisTransaksi");
...
$kptt_nota->setField('NO_REF2', $reqJenisTransaksi);
$kptt_nota->setField('NO_REF3', $reqNoPPKB);
$kptt_nota->setField('KD_KUSTO', "");	
$kptt_nota->setField('BADAN_USAHA', $reqBadanUsaha);
$kptt_nota->setField('KD_BB_KUSTO', "(SELECT KD_BUKU_BESAR1 FROM KBBR_TIPE_TRANS_D WHERE TIPE_TRANS = '".$reqSegmen."' AND KLAS_TRANS = '".$reqJenisTransaksi."')");
...
if($kptt_nota->insert())
{
	...
	for($i=0; $i<count($reqNoUrut); $i++)
	{
		...
		$kptt_nota_d->setField('KLAS_TRANS', $reqJenisTransaksi);
		...
		$kptt_nota_d->setField('KD_SUB_BANTU', $reqNoUrut[$i]);	
		...
		$kptt_nota_d->setField('NO_REF1', $reqSiswa[$i]);
		$kptt_nota_d->setField('NO_REF2', $tempKdBukuBesar);
	...
	{
		$kbbt_jur_bb->setField("NO_NOTA", $reqNoBukti);
		$kbbt_jur_bb->callGenerateJurnalNonSPP();
