...
{			   
	if($reqDebet[$i] > 0)
	{
		$kodeAwal = substr($reqBukuBesar[$i], 0, 1);		
		if($kodeAwal == "1")
		{}
		else
		{
			echo "X-Posisi debet harus rekening 1";
			exit;
		}
	}
}
...
$kbbt_jur_bb->setField("PROGRAM_NAME", "KBB_ENTRY_JUR_JKM_IMAIS");
...
if($kbbt_jur_bb->insert())
{
	...
	{			   
		...
		$kbbt_jur_bb_d->setField('KD_BUKU_BESAR', trim($reqBukuBesar[$i]));
		...
		// if ($reqDebet[$i] >=  "") 
		// {
		// 	if (substr($reqBukuBesar[$i],0.3 <> "101"))
		// 	{
		// 		echo $reqNoBukti."-Kode buku besar salah.";exit();
		// 	}
		// }
			
		$kbbt_jur_bb_d->setField('KD_BUKU_PUSAT', trim($buku_pusat));
		...
		$kbbt_jur_bb_d->setField('PROGRAM_NAME', "KBB_ENTRY_JUR_JKM_IMAIS");
