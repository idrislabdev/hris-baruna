...
class merit_json extends CI_Controller {
	...
	function merit_p3_json()
	{
		...
		$aColumns = array("MERIT_P3_ID", "KELAS", "PERIODE", "JUMLAH");
		$aColumnsAlias = array("MERIT_P3_ID", "KELAS", "PERIODE", "JUMLAH");
		...
		$allRecord = $merit_p3->getCountByParams(array());
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $merit_p3->getCountByParams(array(), " AND (UPPER(KELAS) LIKE '%".strtoupper($_GET['sSearch'])."%')");

		$merit_p3->selectByParams(array(), $dsplyRange, $dsplyStart, " AND (UPPER(KELAS) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
		...
		while($merit_p3->nextRow())
		{
			$row = array();
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				if($aColumns[$i] == "TANGGAL")
					$row[] = getFormattedDate($merit_p3->getField($aColumns[$i]));
				else if($aColumns[$i] == "KETERANGAN")
					$row[] = truncate($merit_p3->getField($aColumns[$i]), 5)."...";
				else if($aColumns[$i] == "JUMLAH")
					$row[] = currencyToPage($merit_p3->getField($aColumns[$i]));			
				else
					$row[] = $merit_p3->getField($aColumns[$i]);
			}
	...
	function merit_pms_json()
	{
		...
		$aColumns = array("MERIT_PMS_ID", "KELAS", "PERIODE", "JUMLAH");
		$aColumnsAlias = array("MERIT_PMS_ID", "KELAS", "PERIODE", "JUMLAH");
		...
		$allRecord = $merit_pms->getCountByParams(array());
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $merit_pms->getCountByParams(array(), " AND (UPPER(KELAS) LIKE '%".strtoupper($_GET['sSearch'])."%')");

		$merit_pms->selectByParams(array(), $dsplyRange, $dsplyStart, " AND (UPPER(KELAS) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
		...
		while($merit_pms->nextRow())
		{
			$row = array();
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				if($aColumns[$i] == "TANGGAL")
					$row[] = getFormattedDate($merit_pms->getField($aColumns[$i]));
				else if($aColumns[$i] == "KETERANGAN")
					$row[] = truncate($merit_pms->getField($aColumns[$i]), 5)."...";
				else if($aColumns[$i] == "JUMLAH")
					$row[] = currencyToPage($merit_pms->getField($aColumns[$i]));			
				else
					$row[] = $merit_pms->getField($aColumns[$i]);
			}
