...
class daftar_json extends CI_Controller {
	...
	function daftar_potongan_lain_json()
	{
		...
		$reqLainKondisi = httpFilterGet("reqLainKondisi");
						   
		$aColumns = array("BANK", "PEGAWAI_ID", "NRP", "NAMA", "ANGSURAN", "ANGSURAN_TERBAYAR", "JUMLAH");
		$aColumnsAlias = array("BANK", "PEGAWAI_ID", "NRP", "NAMA", "ANGSURAN", "ANGSURAN_TERBAYAR", "JUMLAH");
		...
		if ( isset( $_GET['iSortCol_0'] ) )
		{
			...
			if ( trim($sOrder) == "ORDER BY BANK asc, BANK asc" )
			{
				/*
		...
		if($reqLainKondisi == "")
			...
			$statement = " AND LAIN_KONDISI_ID = '".$reqLainKondisi."'";


		$allRecord = $gaji_awal_bulan->getCountByParamsPotonganReport(array(), $statement, $reqPeriode);
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $gaji_awal_bulan->getCountByParamsPotonganReport(array(), " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')".$statement, $reqPeriode);

		$gaji_awal_bulan->selectByParamsPotonganReport(array(), $dsplyRange, $dsplyStart, " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%') ".$statement, $reqPeriode, $sOrder);  		
	...
	function daftar_rekening_bank_add()
	{
		/* INCLUDE FILE */
		include_once("../WEB-INF/functions/string.func.php");
		include_once("../WEB-INF/functions/default.func.php");
	...
	function daftar_rekening_bank_json()
	{
		include_once("../WEB-INF/classes/utils/UserLogin.php");
		include_once("../WEB-INF/functions/string.func.php");
		include_once("../WEB-INF/functions/default.func.php");
		...
		$aColumns = array("ID", "JENIS_REKENING", "REF_ID", "TIPE", "NAMA_REFERENSI", "NAMA_BANK", "NO_REKENING", "NAMA_REKENING");
		$aColumnsAlias = array("ID", "JENIS_REKENING", "REF_ID", "TIPE", "NAMA_REFERENSI", "NAMA_BANK", "NO_REKENING", "NAMA_REKENING");
		...
		if ( isset( $_GET['iSortCol_0'] ) )
		{
			...
			//Check if there is an order by clause
			if ( trim($sOrder) == "ORDER BY JENIS_REKENING, NAMA_REFERENSI " )
			{
				/*
				* If there is no order by clause - ORDER BY INDEX COLUMN!!! DON'T DELETE IT!
				* If there is no order by clause there might be bugs in table display.
				...
				* another row will not be displayed at all.
				*/
				$sOrder = " ORDER BY JENIS_REKENING, NAMA_REFERENSI ";
		...
		$allRecord = $premi->getCountByParams(array());
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $premi->getCountByParams(array(), " AND (UPPER(NAMA_REFERENSI) LIKE '%".strtoupper($_GET['sSearch'])."%')");

		$premi->selectByParams(array(), $dsplyRange, $dsplyStart, " AND (UPPER(NAMA_REFERENSI) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
		...
		while($premi->nextRow())
		{
			...
			{
				/*if($aColumns[$i] == "TANGGAL")
					$row[] = getFormattedDate($premi->getField($aColumns[$i]));
				else if($aColumns[$i] == "KETERANGAN")
					$row[] = truncate($premi->getField($aColumns[$i]), 5)."...";
				else if($aColumns[$i] == "TARIF_NORMAL" || $aColumns[$i] == "TARIF_MAKSIMAL")
					$row[] = currencyToPage($premi->getField($aColumns[$i]));			
				else*/
					$row[] = $premi->getField($aColumns[$i]);
			}
	...
	function daftar_unit_link_json()
	{
		include_once("../WEB-INF/classes/utils/UserLogin.php");
		include_once("../WEB-INF/functions/string.func.php");
		include_once("../WEB-INF/functions/default.func.php");
		...
		$reqPeriode = httpFilterGet("reqPeriode");
		$reqAsuransi = httpFilterGet("reqAsuransi");
						   
		$aColumns = array("POTONGAN", "PEGAWAI_ID", "NRP", "NAMA", "JUMLAH");
		$aColumnsAlias = array("POTONGAN", "PEGAWAI_ID", "NRP", "NAMA", "JUMLAH");
		...
		if ( isset( $_GET['iSortCol_0'] ) )
		{
			...
			//Check if there is an order by clause
			if ( trim($sOrder) == "ORDER BY POTONGAN asc, POTONGAN asc" )
			{
				/*
				* If there is no order by clause - ORDER BY INDEX COLUMN!!! DON'T DELETE IT!
				* If there is no order by clause there might be bugs in table display.
				...
				* another row will not be displayed at all.
				*/
				$sOrder = "ORDER BY POTONGAN, NAMA ASC";
		...
		if($reqAsuransi == "")
			...
			$statement = " AND KODE = '".$reqAsuransi."'";


		$allRecord = $gaji_awal_bulan->getCountByParamsAsuransiReport(array(), $statement, $reqPeriode);
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $gaji_awal_bulan->getCountByParamsAsuransiReport(array(), " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')".$statement, $reqPeriode);

		$gaji_awal_bulan->selectByParamsAsuransiReport(array(), $dsplyRange, $dsplyStart, " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%') ".$statement, $reqPeriode, $sOrder);  		
