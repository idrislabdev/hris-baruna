...
class unit_json extends CI_Controller {
	...
	function unit_link_json()
	{
		...
		$aColumns = array("UNIT_LINK_ID", "KELAS", "JUMLAH");
		$aColumnsAlias = array("UNIT_LINK_ID", "KELAS", "JUMLAH");
		...
		$allRecord = $unit_link->getCountByParams(array());
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $unit_link->getCountByParams(array(), " AND (UPPER(KELAS) LIKE '%".strtoupper($_GET['sSearch'])."%')");

		$unit_link->selectByParams(array(), $dsplyRange, $dsplyStart, " AND (UPPER(KELAS) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
		...
		while($unit_link->nextRow())
		{
			$row = array();
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				if($aColumns[$i] == "TANGGAL")
					$row[] = getFormattedDate($unit_link->getField($aColumns[$i]));
				else if($aColumns[$i] == "KETERANGAN")
					$row[] = truncate($unit_link->getField($aColumns[$i]), 5)."...";
				else if($aColumns[$i] == "JUMLAH")
					$row[] = currencyToPage($unit_link->getField($aColumns[$i]));			
				else
					$row[] = $unit_link->getField($aColumns[$i]);
			}
