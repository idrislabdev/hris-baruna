...
class asuransi_json extends CI_Controller {
	...
	function json()
	{
		...
		$aColumns = array("ASURANSI_ID", "NAMA", "KETERANGAN");
		$aColumnsAlias = array("ASURANSI_ID", "NAMA", "KETERANGAN");
		...
		if ( isset( $_GET['iSortCol_0'] ) )
		{
			...
			if ( trim($sOrder) == "ORDER BY ASURANSI_ID asc" )
			{
				/*
		...
		$allRecord = $asuransi->getCountByParams(array());
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $asuransi->getCountByParams(array(), " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')");

		$asuransi->selectByParams(array(), $dsplyRange, $dsplyStart, " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
		...
		while($asuransi->nextRow())
		{
			...
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				if($aColumns[$i] == "TANGGAL")
					$row[] = getFormattedDate($asuransi->getField($aColumns[$i]));
				else if($aColumns[$i] == "KETERANGAN1")
					$row[] = truncate($asuransi->getField($aColumns[$i]), 5)."...";
				else if($aColumns[$i] == "KETERANGAN1")
					$row[] = currencyToPage($asuransi->getField($aColumns[$i]));			
				else
					$row[] = $asuransi->getField($aColumns[$i]);
			}
