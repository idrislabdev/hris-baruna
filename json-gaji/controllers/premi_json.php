...
class premi_json extends CI_Controller {
	...
	function json()
	{
		...
		$aColumns = array("PREMI_ID", "LOKASI", "KAPAL_JENIS", "JABATAN", "PRODUKSI_NORMAL", "PRODUKSI_MAKSIMAL", "INTERVAL_PRODUKSI", "TARIF_NORMAL", "TARIF_MAKSIMAL");
		$aColumnsAlias = array("PREMI_ID", "B.NAMA", "D.NAMA", "C.NAMA", "PRODUKSI_NORMAL", "PRODUKSI_MAKSIMAL", "INTERVAL_PRODUKSI", "TARIF_NORMAL", "TARIF_MAKSIMAL");
		...
		if ( isset( $_GET['iSortCol_0'] ) )
		{
			...
			if ( trim($sOrder) == "ORDER BY PREMI_ID ASC" )
			{
				/*
		...
		if($_GET['sSearch'] == "")
			...
			$allRecordFilter = $premi->getCountByParams(array(), " AND (UPPER(B.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')");

		$premi->selectByParams(array(), $dsplyRange, $dsplyStart, " AND (UPPER(B.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
		//echo $premi->query; exit;
