...
class parameter_json extends CI_Controller {
	...
	function parameter_potongan_wajib_add()
	{
		/* INCLUDE FILE */
	...
	function parameter_potongan_wajib_json()
	{
		include_once("../WEB-INF/classes/utils/UserLogin.php");
		include_once("../WEB-INF/functions/string.func.php");
		include_once("../WEB-INF/functions/default.func.php");
		...
		$aColumns = array("JENIS_POTONGAN", "KELAS", "JUMLAH");
		$aColumnsAlias = array("JENIS_POTONGAN", "KELAS", "JUMLAH");
		...
		if ( isset( $_GET['iSortCol_0'] ) )
		{
			...
			if ( trim($sOrder) == "ORDER BY JENIS_POTONGAN asc, JENIS_POTONGAN asc" )
			{
				/*
				...
				*/
				$sOrder = " ORDER BY JENIS_POTONGAN, KELAS ASC";
		...
		$allRecord = $parameter_potongan_wajib->getCountByParams(array());
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $parameter_potongan_wajib->getCountByParams(array(), " AND (UPPER(JENIS_POTONGAN) LIKE '%".strtoupper($_GET['sSearch'])."%' OR UPPER(KELAS) LIKE '%".strtoupper($_GET['sSearch'])."%')");

		$parameter_potongan_wajib->selectByParams(array(), $dsplyRange, $dsplyStart, " AND (UPPER(JENIS_POTONGAN) LIKE '%".strtoupper($_GET['sSearch'])."%' OR UPPER(KELAS) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
		...
		while($parameter_potongan_wajib->nextRow())
		{
			$row = array();
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				if($aColumns[$i] == "TANGGAL")
					$row[] = getFormattedDate($parameter_potongan_wajib->getField($aColumns[$i]));
				else if($aColumns[$i] == "KETERANGAN")
					$row[] = truncate($parameter_potongan_wajib->getField($aColumns[$i]), 5)."...";
				else if($aColumns[$i] == "JUMLAH")
					$row[] = currencyToPage($parameter_potongan_wajib->getField($aColumns[$i]));			
				else
					$row[] = $parameter_potongan_wajib->getField($aColumns[$i]);
			}
