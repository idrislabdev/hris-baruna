...
$aColumns = array("JABATAN_ID", "NAMA", "KELAS", "KELOMPOK");
$aColumnsAlias = array("JABATAN_ID", "NAMA", "KELAS", "KELOMPOK");
...
if ( isset( $_GET['iSortCol_0'] ) )
{
	...
	if ( trim($sOrder) == "ORDER BY JABATAN_ID asc" )
	{
		...
		*/
		$sOrder = " ORDER BY JABATAN_ID DESC";
...
if(substr($reqDepartemen, 0, 3) == "CAB"){
	...
	/*if($reqJenisPegawai == 4 )
		...
		$statement .= " AND EXISTS(SELECT 1 FROM PPI_SIMPEG.DEPARTEMEN X WHERE (B.DEPARTEMEN_ID = X.DEPARTEMEN_ID AND X.CABANG_ID = '".str_replace("CAB","",$reqDepartemen)."') OR B.DEPARTEMEN_ID IS NULL ) ";
}
else
	$statement .= " AND A.DEPARTEMEN_ID LIKE '".$reqDepartemen."%'";


$allRecord = $jabatan->getCountByParams(array(),$statement);
if($_GET['sSearch'] == "")
	$allRecordFilter = $allRecord;
else	
	$allRecordFilter = $jabatan->getCountByParams(array(), $statement." AND (UPPER(A.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')");

$jabatan->selectByParams(array(), $dsplyRange, $dsplyStart, $statement." AND (UPPER(A.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
