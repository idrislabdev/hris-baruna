...
class pengawakan_json extends CI_Controller {
	...
	function json()
	{
		...
		$aColumns = array("SURAT_PERINTAH_PEGAWAI_ID", "NRP", "NAMA", "KELAS", "JABATAN_AWAL", "KAPAL_AWAL", "JABATAN_AKHIR", "KAPAL_AKHIR");
		$aColumnsAlias = array("SURAT_PERINTAH_PEGAWAI_ID", "NRP", "NAMA", "KELAS", "JABATAN_AWAL", "KAPAL_AWAL", "JABATAN_AKHIR", "KAPAL_AKHIR");
		...
		if ( isset( $_GET['iSortCol_0'] ) )
		{
			...
			if ( trim($sOrder) == "ORDER BY GALANGAN_ID asc" )
			{
				/*
				...
				*/
				$sOrder = " ORDER BY GALANGAN_ID DESC";
		...
		$allRecord = $surat_perintah_pegawai->getCountByParams(array("STATUS_MASUK" => "M", "STATUS_VALIDASI" => $reqStatus));
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $surat_perintah_pegawai->getCountByParams(array("STATUS_MASUK" => "M", "STATUS_VALIDASI" => $reqStatus), " AND (UPPER(A.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')");

		$surat_perintah_pegawai->selectByParams(array("STATUS_MASUK" => "M", "STATUS_VALIDASI" => $reqStatus), $dsplyRange, $dsplyStart, " AND (UPPER(A.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
		...
		while($surat_perintah_pegawai->nextRow())
		{
			...
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				if($aColumns[$i] == "TANGGAL")
					$row[] = getFormattedDate($surat_perintah_pegawai->getField($aColumns[$i]));
				else if($aColumns[$i] == "KETERANGAN")
					$row[] = truncate($surat_perintah_pegawai->getField($aColumns[$i]), 5)."...";
				else
					$row[] = $surat_perintah_pegawai->getField($aColumns[$i]);
			}
