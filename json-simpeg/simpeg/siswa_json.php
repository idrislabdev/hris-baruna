...
class siswa_json extends CI_Controller {
	...
	function siswa_magang_json()
	{
		...
		$aColumns = array("PEGAWAI_ID", "NIS", "NAMA", "AGAMA_NAMA", "JENIS_KELAMIN", "TEMPAT_LAHIR", "TANGGAL_LAHIR", "ALAMAT", "MAGANG_TIPE", "DEPARTEMEN" );
		$aColumnsAlias = array("PEGAWAI_ID", "NIS", "NAMA", "AGAMA_NAMA", "JENIS_KELAMIN", "TEMPAT_LAHIR", "TANGGAL_LAHIR", "ALAMAT", "MAGANG_TIPE", "DEPARTEMEN");
		...
		*/
		...
		if ($reqTanggalAwal<>"" || $reqTanggalAkhir <> "") {
			$statement .= "AND TRUNC(TANGGAL_KONTRAK_AWAL) BETWEEN TO_DATE('". $reqTanggalAwal ."','MM/DD/YYYY') AND TO_DATE('". $reqTanggalAkhir ."','MM/DD/YYYY') ";
		}
			
		$allRecord = $pegawai->getCountByParams(array("JENIS_PEGAWAI_ID" => 8),$statement);
		//echo $pegawai->query;
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $pegawai->getCountByParams(array("JENIS_PEGAWAI_ID" => 8), $statement." AND ((UPPER(A.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%') OR (UPPER(A.NIPP) LIKE '%".strtoupper($_GET['sSearch'])."%')) ");

		//echo $pegawai->query;
		$pegawai->selectByParams(array("JENIS_PEGAWAI_ID" => 8), $dsplyRange, $dsplyStart, $statement." AND ((UPPER(A.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%') OR (UPPER(A.NIPP) LIKE '%".strtoupper($_GET['sSearch'])."%')) " , $sOrder);
