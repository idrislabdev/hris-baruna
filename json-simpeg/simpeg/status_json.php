...
class status_json extends CI_Controller {
	...
	function status_pegawai_json()
	{
		...
		$aColumns = array("STATUS_PEGAWAI_ID", "NAMA", "KETERANGAN");
		$aColumnsAlias = array("STATUS_PEGAWAI_ID", "NAMA", "KETERANGAN");
		...
		if ( isset( $_GET['iSortCol_0'] ) )
		{
			...
			if ( trim($sOrder) == "ORDER BY STATUS_PEGAWAI_ID asc" )
			{
				/*
				...
				*/
				$sOrder = " ORDER BY STATUS_PEGAWAI_ID ASC";
		...
		$allRecord = $status_pegawai->getCountByParams(array());
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $status_pegawai->getCountByParams(array(), " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')");

		$status_pegawai->selectByParams(array(), $dsplyRange, $dsplyStart, " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
		...
		while($status_pegawai->nextRow())
		{
			...
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				if($aColumns[$i] == "TANGGAL")
					$row[] = getFormattedDate($status_pegawai->getField($aColumns[$i]));
				else if($aColumns[$i] == "KETERANGAN")
					$row[] = truncate($status_pegawai->getField($aColumns[$i]), 5)."...";
				else
					$row[] = $status_pegawai->getField($aColumns[$i]);
			}
	...
	function status_pegawai_lookup_json()
	{
		include_once("../WEB-INF/classes/utils/UserLogin.php");
		include_once("../WEB-INF/classes/base-simpeg/StatusPegawai.php");
