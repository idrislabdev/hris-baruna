...
class pejabat_json extends CI_Controller {
	...
	function pejabat_penetap_json()
	{
		...
		$aColumns = array("PEJABAT_PENETAP_ID", "NAMA", "KETERANGAN");
		$aColumnsAlias = array("PEJABAT_PENETAP_ID", "NAMA", "KETERANGAN");
		...
		if ( isset( $_GET['iSortCol_0'] ) )
		{
			...
			if ( trim($sOrder) == "ORDER BY PEJABAT_PENETAP_ID asc" )
			{
				/*
		...
		$allRecord = $pejabat_penetap->getCountByParams(array());
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $pejabat_penetap->getCountByParams(array(), " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')");

		$pejabat_penetap->selectByParams(array(), $dsplyRange, $dsplyStart, " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
		...
		while($pejabat_penetap->nextRow())
		{
			...
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				if($aColumns[$i] == "TANGGAL")
					$row[] = getFormattedDate($pejabat_penetap->getField($aColumns[$i]));
				else if($aColumns[$i] == "KETERANGAN")
					$row[] = truncate($pejabat_penetap->getField($aColumns[$i]), 5)."...";
				else
					$row[] = $pejabat_penetap->getField($aColumns[$i]);
			}
