...
class agama_json extends CI_Controller {
	...
	function json()
	{
		...
		$aColumns = array("AGAMA_ID", "NAMA", "KETERANGAN");
		$aColumnsAlias = array("AGAMA_ID", "NAMA", "KETERANGAN");
		...
		if ( isset( $_GET['iSortCol_0'] ) )
		{
			...
			if ( trim($sOrder) == "ORDER BY AGAMA_ID asc" )
			{
				/*
		...
		$allRecord = $agama->getCountByParams(array());
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $agama->getCountByParams(array(), " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')");

		$agama->selectByParams(array(), $dsplyRange, $dsplyStart, " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
		...
		while($agama->nextRow())
		{
			...
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				if($aColumns[$i] == "TANGGAL")
					$row[] = getFormattedDate($agama->getField($aColumns[$i]));
				else if($aColumns[$i] == "KETERANGAN")
					$row[] = truncate($agama->getField($aColumns[$i]), 5)."...";
				else
					$row[] = $agama->getField($aColumns[$i]);
			}
