...
class pendidikan_json extends CI_Controller {
	...
	function pendidikan_biaya_json()
	{
		include_once("../WEB-INF/classes/utils/UserLogin.php");
		include_once("../WEB-INF/functions/string.func.php");
		include_once("../WEB-INF/functions/default.func.php");
		...
		$aColumns = array("PENDIDIKAN_BIAYA_ID", "NAMA", "KETERANGAN");
		$aColumnsAlias = array("PENDIDIKAN_BIAYA_ID", "NAMA", "KETERANGAN");
		...
		if ( isset( $_GET['iSortCol_0'] ) )
		{
			...
			if ( trim($sOrder) == "ORDER BY PENDIDIKAN_BIAYA_ID asc" )
			{
				/*
		...
		$allRecord = $pendidikan_biaya->getCountByParams(array());
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $pendidikan_biaya->getCountByParams(array(), " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')");

		$pendidikan_biaya->selectByParams(array(), $dsplyRange, $dsplyStart, " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
		...
		while($pendidikan_biaya->nextRow())
		{
			...
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				if($aColumns[$i] == "TANGGAL")
					$row[] = getFormattedDate($pendidikan_biaya->getField($aColumns[$i]));
				else if($aColumns[$i] == "KETERANGAN")
					$row[] = truncate($pendidikan_biaya->getField($aColumns[$i]), 5)."...";
				else
					$row[] = $pendidikan_biaya->getField($aColumns[$i]);
			}
	...
	function json()
	{
		...
		$aColumns = array("PENDIDIKAN_ID", "NAMA", "KETERANGAN");
		$aColumnsAlias = array("PENDIDIKAN_ID", "NAMA", "KETERANGAN");
		...
		if ( isset( $_GET['iSortCol_0'] ) )
		{
			...
			//Check if there is an order by clause
			if ( trim($sOrder) == "ORDER BY PENDIDIKAN_ID asc" )
			{
				/*
				* If there is no order by clause - ORDER BY INDEX COLUMN!!! DON'T DELETE IT!
				* If there is no order by clause there might be bugs in table display.
		...
		$allRecord = $pendidikan->getCountByParams(array());
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $pendidikan->getCountByParams(array(), " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')");

		$pendidikan->selectByParams(array(), $dsplyRange, $dsplyStart, " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
		...
		while($pendidikan->nextRow())
		{
			...
			{
				if($aColumns[$i] == "TANGGAL")
					$row[] = getFormattedDate($pendidikan->getField($aColumns[$i]));
				else if($aColumns[$i] == "KETERANGAN")
					$row[] = truncate($pendidikan->getField($aColumns[$i]), 5)."...";
				else
					$row[] = $pendidikan->getField($aColumns[$i]);
			}
