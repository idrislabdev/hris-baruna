...
class hubungan_json extends CI_Controller {
	...
	function agama_add()
	{
		...
		$aColumns = array("HUBUNGAN_KELUARGA_ID", "NAMA", "KETERANGAN");
		$aColumnsAlias = array("HUBUNGAN_KELUARGA_ID", "NAMA", "KETERANGAN");
		...
		if ( isset( $_GET['iSortCol_0'] ) )
		{
			...
			if ( trim($sOrder) == "ORDER BY HUBUNGAN_KELUARGA_ID asc" )
			{
				/*
				...
				*/
				$sOrder = " ORDER BY HUBUNGAN_KELUARGA_ID ASC";
		...
		$allRecord = $hubungan_keluarga->getCountByParams(array());
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $hubungan_keluarga->getCountByParams(array(), " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')");

		$hubungan_keluarga->selectByParams(array(), $dsplyRange, $dsplyStart, " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
		...
		while($hubungan_keluarga->nextRow())
		{
			...
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				if($aColumns[$i] == "TANGGAL")
					$row[] = getFormattedDate($hubungan_keluarga->getField($aColumns[$i]));
				else if($aColumns[$i] == "KETERANGAN")
					$row[] = truncate($hubungan_keluarga->getField($aColumns[$i]), 5)."...";
				else
					$row[] = $hubungan_keluarga->getField($aColumns[$i]);
			}
