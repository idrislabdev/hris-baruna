...
class universitas_json extends CI_Controller {
	...
	function json()
	{
		...
		$aColumns = array("UNIVERSITAS_ID", "KODE", "NAMA", "KETERANGAN");
		$aColumnsAlias = array("UNIVERSITAS_ID", "KODE", "NAMA", "KETERANGAN");
		...
		if ( isset( $_GET['iSortCol_0'] ) )
		{
			...
			if ( trim($sOrder) == "ORDER BY UNIVERSITAS_ID asc" )
			{
				/*
		...
		$allRecord = $universitas->getCountByParams(array());
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $universitas->getCountByParams(array(), " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')");

		$universitas->selectByParams(array(), $dsplyRange, $dsplyStart, " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
		...
		while($universitas->nextRow())
		{
			...
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				if($aColumns[$i] == "TANGGAL")
					$row[] = getFormattedDate($universitas->getField($aColumns[$i]));
				else if($aColumns[$i] == "KETERANGAN")
					$row[] = truncate($universitas->getField($aColumns[$i]), 5)."...";
				else
					$row[] = $universitas->getField($aColumns[$i]);
			}
