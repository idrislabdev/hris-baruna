...
class pkwt_json extends CI_Controller {
	...
	function work_order_kontrak_pkwt_json()
	{
		...
		$aColumns = array("PEGAWAI_ID", "STATUS_KONTRAK", "NRP","NAMA", "JABATAN_NAMA", "MKP", "DEPARTEMEN", "AGAMA_NAMA", "JENIS_KELAMIN", "TANGGAL_LAHIR", "TANGGAL_KONTRAK_AWAL", "TANGGAL_KONTRAK_AKHIR");
		$aColumnsAlias = array("A.PEGAWAI_ID", "STATUS_KONTRAK", "NRP","A.NAMA", "D.NAMA", "MKP", "DEPARTEMEN", "C.NAMA", "JENIS_KELAMIN", "TANGGAL_LAHIR", "TANGGAL_KONTRAK_AWAL", "TANGGAL_KONTRAK_AKHIR");
		...
		$batas_notifikasi = $notifikasi->getNotifikasiBatas("DATA_KONTRAK_PKWT");
		...
		$statement .= " AND G.JENIS_PEGAWAI_ID = 3 AND (((G.TANGGAL_KONTRAK_AKHIR - INTERVAL '".$batas_notifikasi."' DAY) <= SYSDATE AND G.TANGGAL_KONTRAK_AKHIR > SYSDATE) OR (G.TANGGAL_KONTRAK_AKHIR <= SYSDATE)) ";
		...
		//echo $pegawai->query;
		$pegawai->selectByParamsKontrakPKWT(array(), $dsplyRange, $dsplyStart, $statement." AND ((UPPER(A.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%') OR (UPPER(A.NIPP) LIKE '%".strtoupper($_GET['sSearch'])."%')) " , $sOrder);
