...
class kandidat_json extends CI_Controller {
	...
	function kandidat_jabatan_kosong_pilih_json()
	{
		...
		$aColumns = array("PEGAWAI_ID", "NRP", "NIPP", "NAMA", "JABATAN_NAMA", "MKP", "UMUR", "PENDIDIKAN", "PENGALAMAN", "TANGGAL_LAHIR", "ZODIAC", "STATUS_KAWIN", "STATUS_PEGAWAI_NAMA", "GOLONGAN_DARAH", "ALAMAT", "TELEPON", "EMAIL", "HOBBY", "NIS", "STATUS_PEGAWAI_ID");
		$aColumnsAlias = array("A.PEGAWAI_ID", "NRP", "NIPP", "A.NAMA", "D.NAMA", "MASA_KERJA_TAHUN", "UMUR", "J.PENDIDIKAN", "PENGALAMAN", "TANGGAL_LAHIR", "TANGGAL_LAHIR", "STATUS_KAWIN", "E.NAMA", "GOLONGAN_DARAH", "ALAMAT", "TELEPON", "EMAIL", "HOBBY", "A.NIS", "STATUS_PEGAWAI_ID");
		...
		if ( isset( $_GET['iSortCol_0'] ) )
		{
			...
			if ( trim($sOrder) == "ORDER BY A.PEGAWAI_ID asc" )
			{
				...
				*/
				...
				$sOrder = " ORDER BY PENGALAMAN DESC, UMUR DESC, PENDIDIKAN_ID DESC";
			}
		}
		...
		$statement .= "
		AND PPI_SIMPEG.AMBIL_UMUR(A.TANGGAL_LAHIR) >= '".$tempKandidatUsia."'
		AND PPI_SIMPEG.AMBIL_UMUR(I.TMT_JABATAN) >= '".$tempKandidatPengalaman."'
		AND EXISTS(SELECT 1 FROM PPI_SIMPEG.PEGAWAI_PENDIDIKAN X INNER JOIN PPI_SIMPEG.PENDIDIKAN Y ON Y.PENDIDIKAN_ID = X.PENDIDIKAN_ID WHERE X.PEGAWAI_ID = A.PEGAWAI_ID AND Y.KODE LIKE '".substr($tempKandidatPendidikanKode,0,2)."%')
		";

		$allRecord = $pegawai->getCountByParamsKandidatJabatanKosongPilih(array(),$statement);
		//echo $pegawai->query;
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $pegawai->getCountByParamsKandidatJabatanKosongPilih(array(), $statement." AND ((UPPER(A.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%') OR (UPPER(A.NIPP) LIKE '%".strtoupper($_GET['sSearch'])."%')) ");

		//echo $pegawai->query;
		$pegawai->selectByParamsKandidatJabatanKosongPilih(array(), $dsplyRange, $dsplyStart, $statement." AND ((UPPER(A.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%') OR (UPPER(A.NIPP) LIKE '%".strtoupper($_GET['sSearch'])."%')) " , $sOrder);
	...
	function kandidat_jabatan_kosong_json()
	{
		include_once("../WEB-INF/classes/utils/UserLogin.php");
		include_once("../WEB-INF/functions/string.func.php");
		include_once("../WEB-INF/functions/default.func.php");
		...
		$aColumns = array("JABATAN_ID", "PEGAWAI_ID", "JABATAN", "KELAS", "NRP", "NAMA");
		$aColumnsAlias = array("A.JABATAN_ID", "B.PEGAWAI_ID", "A.NAMA", "A.KELAS", "NRP", "C.NAMA");
		...
		if ( isset( $_GET['iSortCol_0'] ) )
		{
			...
			if ( trim($sOrder) == "ORDER BY JABATAN_ID asc" )
			{
				...
				* another row will not be displayed at all.
				*/
				$sOrder = " ORDER BY ORDER BY TO_NUMBER(A.KELAS) ASC, A.JABATAN_ID ASC";
		...
		$allRecord = $jabatan->getCountByParamsKandidatJabatan(array("A.STATUS" => 1));
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $jabatan->getCountByParamsKandidatJabatan(array("A.STATUS" => 1), " AND (UPPER(A.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')");

		$jabatan->selectByParamsKandidatJabatan(array("A.STATUS" => 1), $dsplyRange, $dsplyStart, " AND (UPPER(A.NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
		...
		while($jabatan->nextRow())
		{
			...
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				if($aColumns[$i] == "TANGGAL")
					...
					$row[] = getNameValueAktif($jabatan->getField($aColumns[$i]));			
				else
