...
class pangkat_json extends CI_Controller {
	...
	function json()
	{
		...
		$aColumns = array("PANGKAT_ID", "NAMA", "KETERANGAN");
		$aColumnsAlias = array("PANGKAT_ID", "NAMA", "KETERANGAN");
		...
		if ( isset( $_GET['iSortCol_0'] ) )
		{
			...
			if ( trim($sOrder) == "ORDER BY PANGKAT_ID asc" )
			{
				/*
				...
				*/
				$sOrder = " ORDER BY PANGKAT_ID ASC";
		...
		$allRecord = $pangkat->getCountByParams(array());
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $pangkat->getCountByParams(array(), " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')");

		$pangkat->selectByParams(array(), $dsplyRange, $dsplyStart, " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
		...
		while($pangkat->nextRow())
		{
			...
			for ( $i=0 ; $i<count($aColumns) ; $i++ )
			{
				if($aColumns[$i] == "TANGGAL")
					$row[] = getFormattedDate($pangkat->getField($aColumns[$i]));
				else if($aColumns[$i] == "KETERANGAN")
					$row[] = truncate($pangkat->getField($aColumns[$i]), 5)."...";
				else
					$row[] = $pangkat->getField($aColumns[$i]);
			}
	...
	function pangkat_kode_json()
	{
		...
		$aColumns = array("PANGKAT_KODE_ID", "NAMA", "KETERANGAN");
		$aColumnsAlias = array("PANGKAT_KODE_ID", "NAMA", "KETERANGAN");
		...
		if ( isset( $_GET['iSortCol_0'] ) )
		{
			...
			//Check if there is an order by clause
			if ( trim($sOrder) == "ORDER BY PANGKAT_KODE_ID asc" )
			{
				/*
				* If there is no order by clause - ORDER BY INDEX COLUMN!!! DON'T DELETE IT!
				* If there is no order by clause there might be bugs in table display.
		...
		$allRecord = $pangkat_kode->getCountByParams(array());
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $pangkat_kode->getCountByParams(array(), " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')");

		$pangkat_kode->selectByParams(array(), $dsplyRange, $dsplyStart, " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
		...
		while($pangkat_kode->nextRow())
		{
			...
			{
				if($aColumns[$i] == "TANGGAL")
					$row[] = getFormattedDate($pangkat_kode->getField($aColumns[$i]));
				else if($aColumns[$i] == "KETERANGAN")
					$row[] = truncate($pangkat_kode->getField($aColumns[$i]), 5)."...";
				else
					$row[] = $pangkat_kode->getField($aColumns[$i]);
			}
	...
	function apangkat_perubahan_kode_add()
	{
		/* INCLUDE FILE */
		include_once("../WEB-INF/functions/string.func.php");
		include_once("../WEB-INF/functions/default.func.php");
	...
	function pangkat_perubahan_kode_json()
	{
		include_once("../WEB-INF/classes/utils/UserLogin.php");
		include_once("../WEB-INF/functions/string.func.php");
		include_once("../WEB-INF/functions/default.func.php");
		...
		$aColumns = array("PANGKAT_PERUBAHAN_KODE_ID", "KODE", "NAMA", "KETERANGAN");
		$aColumnsAlias = array("PANGKAT_PERUBAHAN_KODE_ID", "KODE", "NAMA", "KETERANGAN");
		...
		if ( isset( $_GET['iSortCol_0'] ) )
		{
			...
			//Check if there is an order by clause
			if ( trim($sOrder) == "ORDER BY PANGKAT_PERUBAHAN_KODE_ID asc" )
			{
				/*
				* If there is no order by clause - ORDER BY INDEX COLUMN!!! DON'T DELETE IT!
				* If there is no order by clause there might be bugs in table display.
		...
		$allRecord = $pangkat_perubahan_kode->getCountByParams(array());
		if($_GET['sSearch'] == "")
			$allRecordFilter = $allRecord;
		else	
			$allRecordFilter = $pangkat_perubahan_kode->getCountByParams(array(), " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')");

		$pangkat_perubahan_kode->selectByParams(array(), $dsplyRange, $dsplyStart, " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%')", $sOrder);     		
		...
		while($pangkat_perubahan_kode->nextRow())
		{
			...
			{
				if($aColumns[$i] == "TANGGAL")
					$row[] = getFormattedDate($pangkat_perubahan_kode->getField($aColumns[$i]));
				else if($aColumns[$i] == "KETERANGAN")
					$row[] = truncate($pangkat_perubahan_kode->getField($aColumns[$i]), 5)."...";
				else
					$row[] = $pangkat_perubahan_kode->getField($aColumns[$i]);
			}
