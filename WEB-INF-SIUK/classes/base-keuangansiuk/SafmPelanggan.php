  ...
  class SafmPelanggan extends EntitySIUK{ 
    ...
    **/
	...
	function insert()
	{
		...
		$str = "
				...
				VALUES ('".$this->getField("KD_CABANG")."', '".$this->getField("JENIS_TABLE")."', '".$this->getField("ID_TABLE")."',
						'".$this->getField("MPLG_KODE")."', '".$this->getField("MPLG_NAMA")."', '".$this->getField("MPLG_ALAMAT")."',
	...
	function importKadet()
	{
		...
		//'".$this->getField("FOTO")."',  FOTO,
		$str = "
				INSERT INTO PPI_SIUK.SAFM_PELANGGAN (
					   KD_CABANG, JENIS_TABLE, ID_TABLE, 
					   MPLG_KODE, MPLG_NAMA, MPLG_ALAMAT, 
					   MPLG_KOTA, MPLG_JENIS_USAHA, MPLG_BADAN_USAHA) 
				VALUES ('".$this->getField("KD_CABANG")."', '".$this->getField("JENIS_TABLE")."', '".$this->getField("ID_TABLE")."',
    ...
    **/ 
    ...
    function selectByParamsDepartemenKelas($paramsArray=array(),$limit=-1,$from=-1, $statement="", $order="")
	{
		$str = "
				SELECT 
				DEPARTEMEN_ID, DEPARTEMEN_KELAS_ID, NAMA_KELAS, 
				   JUMLAH_SPP, NVL(JUMLAH_SISWA, 0) JUMLAH_SISWA, JUMLAH_SISWA_SPP
				FROM DEPARTEMEN_KELAS A
				WHERE 1 = 1
				"; 
		//, FOTO
		while(list($key,$val) = each($paramsArray))
    ...
    function selectByParamsBB($paramsArray=array(),$limit=-1,$from=-1, $statement="", $order="")
	{
		$str = "
				SELECT 
					JENIS, DEPARTEMEN_ID, DEPARTEMEN_KELAS_ID, 
					   NIS, NAMA, SEKOLAH, 
					   KELAS, KODE_BB, KODE_BP
    ...
    **/ 
    ...
    function getCountByParamsDepartemenKelas($paramsArray=array(), $statement="")
	{
		$str = "SELECT COUNT(1) AS ROWCOUNT FROM DEPARTEMEN_KELAS
		        WHERE 1 = 1 ".$statement; 
	...
	function getKodePelanggan($paramsArray=array(), $statement="")
	{
		$str = "SELECT LPAD(MAX(TO_NUMBER(MPLG_KODE)) + 1, 5, '0') AS ROWCOUNT FROM SAFM_PELANGGAN
		        WHERE 1 = 1 ".$statement; 
	...
	function getPelanggan($paramsArray=array(), $statement="")
	{
		$str = "SELECT MPLG_NAMA AS ROWCOUNT FROM SAFM_PELANGGAN
		        WHERE 1 = 1 ".$statement; 
