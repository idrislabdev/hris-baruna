...
***************************************************************************************************** */
  ...
  /***
  * EntitySIUK-base class untuk mengimplementasikan tabel KPTT_NOTA_SPP_D.
  * 
  ***/
  include_once("../WEB-INF-SIUK/classes/db/Entity.php");

  class KpttNotaSppD extends EntitySIUK{ 

	var $query;
    /**
    * Class constructor.
    **/
    function KpttNotaSppD()
	{
      $this->EntitySIUK(); 
    }
	...
	function insert()
	{
		...
		//$this->setField("KPTT_NOTA_SPP_D_ID", $this->getNextId("KPTT_NOTA_SPP_D_ID","KPTT_NOTA_SPP_D")); 		
		
		$str = "
				INSERT INTO KPTT_NOTA_SPP_D (
		...
		$str = "
				INSERT INTO KPTT_NOTA_SPP_D (
					   ...
					   PROGRAM_NAME, KD_TERMINAL, NL_TARIF, TAGIHAN, SISA, PERIODE_PEMBAYARAN) 
						...
						AND NOT EXISTS(SELECT 1 FROM KPTT_NOTA_SPP_D X WHERE X.KD_SUB_BANTU = A.MPLG_KODE AND X.PERIODE_PEMBAYARAN = '".$this->getField("PERIODE_PEMBAYARAN")."' AND NOT X.NO_NOTA = '".$this->getField("NO_NOTA")."' )
    ...
    function update()
	{
		$str = "
				...
				WHERE  KPTT_NOTA_SPP_D_ID = '".$this->getField("KPTT_NOTA_SPP_D_ID")."'
	...
	{
        $str = "UPDATE KPTT_NOTA_SPP_D SET
				STATUS_PROSES = 2
                WHERE 
                NO_NOTA = '".$this->getField("NO_NOTA")."' "; 
	...
	{
        $str = "UPDATE KPTT_NOTA_SPP_D A 
				SET SISA = TAGIHAN,
					NO_REF_PELUNASAN = NULL
				WHERE A.NO_REF_PELUNASAN = '".$this->getField("NO_NOTA")."' AND 
				NOT EXISTS(SELECT 1 FROM KPTT_NOTA_SPP_D X WHERE X.NO_NOTA = A.NO_REF_PELUNASAN AND X.KD_SUB_BANTU = A.KD_SUB_BANTU) "; 
	...
	function updatePelunasan()
	{
        $str = "UPDATE KPTT_NOTA_D A 
				SET SISA = TAGIHAN - (SELECT SUM(JML_VAL_TRANS) FROM KPTT_NOTA_SPP_D X WHERE X.PREV_NO_NOTA = A.NO_NOTA AND X.KD_SUB_BANTU = A.			KD_SUB_BANTU),
					NO_REF_PELUNASAN = '".$this->getField("NO_NOTA")."'
				WHERE A.NO_NOTA = '".$this->getField("PREV_NO_NOTA")."'  AND KD_SUB_BANTU = '".$this->getField("KD_SUB_BANTU")."' 
				 "; 
	...
	function updateBatalPelunasanKoreksi()
	{
        $str = "UPDATE KPTT_NOTA_SPP_D A 
				SET SISA = TAGIHAN,
					NO_REF_PELUNASAN = NULL
				WHERE A.NO_REF_PELUNASAN = '".$this->getField("NO_NOTA")."' AND 
				NOT EXISTS(SELECT 1 FROM KPTT_NOTA_SPP_D X WHERE X.NO_NOTA = A.NO_REF_PELUNASAN AND X.NO_REF2 = A.KD_SUB_BANTU) "; 
    ...
    **/ 
    ...
    function selectByParamsJurnal($paramsArray=array(),$limit=-1,$from=-1, $statement="", $sOrder="ORDER BY A.KD_SUB_BANTU ASC")
	{
		$str = "
				SELECT A.KD_CABANG, A.KD_SUBSIS, A.JEN_JURNAL, 
                A.TIPE_TRANS, A.NO_NOTA, A.LINE_SEQ, 
                KLAS_TRANS, KWANTITAS, SATUAN, 
                ...
                CASE WHEN A.STATUS_KENA_PAJAK = '0' THEN 'TIDAK' ELSE 'YA' END STATUS_KENA_PAJAK_DESC,
                A.JML_VAL_PAJAK, A.JML_RP_TRANS, A.JML_RP_PAJAK, A.JML_VAL_TRANS + A.JML_VAL_PAJAK JUMLAH_TOTAL,
        		JML_RP_SLSH_KURS, A.TANDA_TRANS, A.KD_BUKU_BESAR, 
				A.KD_SUB_BANTU, A.KD_BUKU_PUSAT, A.KD_D_K, 
				PREV_NO_NOTA, A.KET_TAMBAHAN, A.STATUS_PROSES, 
				FLAG_JURNAL, A.NO_REF1, A.NO_REF2, 
				A.NO_REF3, A.LAST_UPDATE_DATE, A.LAST_UPDATED_BY, 
				A.PROGRAM_NAME, A.KD_TERMINAL, NL_TARIF
				FROM KPTT_NOTA_SPP_D A
                INNER JOIN KPTT_NOTA_SPP B ON A.NO_NOTA = B.NO_NOTA
				WHERE 1 = 1
				"; 
		//, FOTO
		while(list($key,$val) = each($paramsArray))
    ...
    function selectDaftarTagihanJKMPembatalanPelunasan($paramsArray=array(),$limit=-1,$from=-1, $statement="", $sOrder= " ORDER BY C.NM_SUB_BANTU ASC ")
	{
		$str = "
			SELECT A.NO_NOTA, B.BLN_BUKU, B.THN_BUKU, B.BLN_BUKU || B.THN_BUKU PERIODE, 
                ...
                INNER JOIN KPTT_NOTA_SPP B ON A.NO_NOTA = B.NO_NOTA 
                INNER JOIN KBBR_KARTU_TAMBAH C ON A.KD_SUB_BANTU = C.KD_SUB_BANTU
        		INNER JOIN KPTT_NOTA_SPP_D D ON A.PREV_NO_NOTA = D.NO_NOTA AND A.KD_SUB_BANTU = D.KD_SUB_BANTU
                WHERE 1 = 1
				"; 
		//, FOTO
		while(list($key,$val) = each($paramsArray))
    ...
    function selectDaftarTagihanJPJPembatalanTagihan($paramsArray=array(),$limit=-1,$from=-1, $statement="", $sOrder= " ORDER BY C.NM_SUB_BANTU ASC ")
	{
		$str = "
			SELECT A.NO_NOTA, A.KLAS_TRANS, B.BLN_BUKU, B.THN_BUKU, B.BLN_BUKU || B.THN_BUKU PERIODE, 
                   A.KD_SUB_BANTU, C.NM_SUB_BANTU, B.TGL_TRANS, A.KD_BUKU_BESAR, 
                   A.TAGIHAN, A.SISA, (SELECT MAX(X.NO_NOTA) FROM KPTT_NOTA_SPP_D X WHERE X.PREV_NO_NOTA = A.NO_NOTA AND X.KD_SUB_BANTU = A.KD_SUB_BANTU) PREV_NO_NOTA
                FROM KPTT_NOTA_SPP_D A 
                INNER JOIN KPTT_NOTA_SPP B ON A.NO_NOTA = B.NO_NOTA 
                INNER JOIN KBBR_KARTU_TAMBAH C ON A.KD_SUB_BANTU = C.KD_SUB_BANTU
                WHERE 1 = 1
    ...
    function selectByParamsPelunasanKasBank($paramsArray=array(),$limit=-1,$from=-1, $statement="", $sOrder="ORDER BY A.KD_SUBSIS ASC")
	{
		$str = "
				...
				FROM KPTT_NOTA_SPP_D A INNER JOIN KPTT_NOTA_SPP B ON A.NO_NOTA = B.NO_NOTA
		...
		$str = "
				SELECT   DISTINCT A.KD_CABANG, A.KD_SUBSIS, A.JEN_JURNAL, A.TIPE_TRANS, A.NO_NOTA,
				    ...
				    FROM KPTT_NOTA_SPP_D A INNER JOIN KPTT_NOTA_SPP B ON A.NO_NOTA = B.NO_NOTA
				         ...
				         LEFT JOIN KPTT_NOTA_SPP_D D ON B.PREV_NOTA_UPDATE = D.NO_NOTA AND A.KD_SUB_BANTU = D.KD_SUB_BANTU
				         LEFT JOIN KPTT_NOTA_SPP E ON D.NO_NOTA = E.NO_NOTA
				WHERE 1 = 1
				"; 
		//, FOTO
		while(list($key,$val) = each($paramsArray))
    ...
    function selectByParamsPelunasanPembayaran($paramsArray=array(),$limit=-1,$from=-1, $statement="", $sOrder="ORDER BY A.KD_SUBSIS ASC")
	{
		$str = "
				...
				FROM KPTT_NOTA_SPP_D A INNER JOIN KPTT_NOTA_SPP B ON A.NO_REF3 = B.NO_REF3
    ...
    function selectByParamsPelunasanPembayaranV2($paramsArray=array(),$limit=-1,$from=-1, $statement="", $sOrder="ORDER BY A.KD_SUBSIS ASC")
	{
		$str = "
				...
				FROM KPTT_NOTA_SPP_D A INNER JOIN KPTT_NOTA_SPP_D B ON A.PREV_NO_NOTA = B.NO_NOTA AND A.KD_SUB_BANTU = B.KD_SUB_BANTU
				 INNER JOIN SAFM_PELANGGAN C ON C.MPLG_KODE = A.KD_SUB_BANTU
				WHERE 1 = 1
				"; 
		//, FOTO
    ...
    function selectByParamsKompensasiSisaUperPencarian($paramsArray=array(),$limit=-1,$from=-1, $kusto="", $valuta="", $sOrder=" ORDER BY SISA_TAGIHAN ASC ", $statement="")
	{
		$str = "
				...
				FROM KPTT_NOTA_SPP A, KBBT_JUR_BB_D B
				WHERE JEN_JURNAL='JPJ' AND 
    ...
    **/ 
	...
	{
		$str = "SELECT COUNT(KPTT_NOTA_SPP_D_ID) AS ROWCOUNT FROM KPTT_NOTA_SPP_D
		        WHERE KPTT_NOTA_SPP_D_ID IS NOT NULL ".$statement; 
