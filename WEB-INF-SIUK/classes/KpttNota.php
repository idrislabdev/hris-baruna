...
***************************************************************************************************** */
  ...
  ***/
  ...
  class KpttNota extends EntitySIUK{ 
    ...
    **/ 
	...
	{
		$str = "			    
                SELECT B.MPLG_NPWP, B.MPLG_ALAMAT, B.MPLG_JENIS_USAHA, A.KD_CABANG, KD_SUBSIS, JEN_JURNAL, 
    ...
    function selectByParamsPembatalanNotaPencarian($pNoRef3, $pKodeKusto, $pKdValuta, $pTglTrans)
	{
		$str = "			    
				 ...
				 PREV_NOTA_UPDATE, NO_NOTA, METERAI, JML_TAGIHAN, GET_KURS_VALAS(TO_CHAR(TO_DATE('".$pTglTrans."', 'DD-MM-YYYY'),'DDMMRR'),'".$pKdValuta."') KURS
				 FROM KPTT_NOTA A
    ...
    function selectByParamsPembatalanPelunasan($paramsArray=array(),$limit=-1,$from=-1, $statement="", $order="")
	{
		$str = "			    
                 SELECT KD_BAYAR,KD_BANK,REK_BANK,KD_BB_BANK, KD_KUSTO,
					   ...
					   NO_CHEQUE,KD_VALUTA,
					   JML_VAL_TRANS,BADAN_USAHA,KD_BB_KUSTO,NO_POSTING, MPLG_NAMA
				FROM   KPTT_NOTA A INNER JOIN SAFM_PELANGGAN B ON A.KD_KUSTO = B.MPLG_KODE 
				WHERE  1 = 1
				"; 
		//, FOTO
		while(list($key,$val) = each($paramsArray))
	...
	{
		$str = "			    
					SELECT ALL KPTT_NOTA.NO_NOTA,     
					SAFR_GENERAL_REF_DETAIL.KET_REF_DATA,
					KPTT_NOTA.TGL_TRANS, 
					KPTT_NOTA.THN_BUKU||' / '||KPTT_NOTA.BLN_BUKU BULAN_BUKU,
					KPTT_NOTA.KD_VALUTA, 
					KPTT_NOTA.JML_VAL_TRANS JML_TRANS
					FROM KPTT_NOTA, SAFR_GENERAL_REF_DETAIL 
					WHERE 
					JEN_JURNAL = 'JKM' AND
					STATUS_PROSES = '1' AND
					SAFR_GENERAL_REF_DETAIL.ID_REF_FILE = 'BADAN USAHA' AND
					SAFR_GENERAL_REF_DETAIL.ID_REF_DATA(+) = KPTT_NOTA.BADAN_USAHA
				"; 
		//, FOTO
		while(list($key,$val) = each($paramsArray))
		{
	...
	{
		$str = "			    
				SELECT A.NO_FAKT_PAJAK NO_FAKTUR, A.NO_REF3 NO_PPKB, B.MPLG_NAMA PELANGGAN,
					   DECODE(A.TGL_CETAK, NULL, A.TGL_TRANS, A.TGL_CETAK) TGL_NOTA,
					   NVL(A.JML_TAGIHAN, 0) TOT_TAGIHAN, NVL(A.JML_VAL_BAYAR, 0) BAYAR, 
					   NVL(A.SISA_TAGIHAN, 0) SISA_TAGIHAN, A.NO_NOTA, KD_BB_KUSTO
				FROM KPTT_NOTA A, SAFM_PELANGGAN B
	...
	{
		$str = "SELECT A.LINE_SEQ, A.NO_REF3, A.PREV_NO_NOTA, A.KLAS_TRANS, A.JML_VAL_TRANS, B.TGL_TRANS, B.TGL_JT_TEMPO,
					   'KOREKSI ' || A.KET_TAMBAHAN KET_TAMBAHAN, A.KD_BUKU_BESAR, A.KD_SUB_BANTU, B.JML_TAGIHAN 
				FROM KPTT_NOTA_D A INNER JOIN KPTT_NOTA B ON A.PREV_NO_NOTA = B.NO_NOTA
			   WHERE 1 = 1
				"; 
		//, FOTO
		while(list($key,$val) = each($paramsArray))
    ...
    **/ 
    ...
    function getCountByParamsPembatalanPelunasanPencarian($paramsArray=array(), $statement="")
	{
		$str = "SELECT COUNT(1) ROWCOUNT
				...
				STATUS_PROSES = '1' AND
				SAFR_GENERAL_REF_DETAIL.ID_REF_FILE = 'BADAN USAHA' AND
				SAFR_GENERAL_REF_DETAIL.ID_REF_DATA(+) = KPTT_NOTA.BADAN_USAHA ".$statement; 
    ...
    function getCountByParams($paramsArray=array(), $statement="")
	{
		$str = "SELECT COUNT(KD_KUSTO) AS ROWCOUNT 
				...
				LEFT JOIN SAFM_BANK C ON A.KD_BANK = C.MBANK_KODE
				LEFT JOIN KBBR_TIPE_TRANS D ON D.TIPE_TRANS = A.TIPE_TRANS
				LEFT JOIN (SELECT NO_NOTA, SUM(HARGA_SATUAN) JUMLAH FROM KPTT_NOTA_D X GROUP BY NO_NOTA) E ON A.NO_NOTA = E.NO_NOTA
				LEFT JOIN KBBR_BUKU_BESAR F ON F.KD_BUKU_BESAR=A.KD_BB_BANK
                WHERE 1 = 1 ".$statement; 
