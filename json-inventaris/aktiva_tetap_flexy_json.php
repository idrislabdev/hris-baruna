...
if ( isset( $_GET['iSortCol_0'] ) )
{
	...
	if ( trim($sOrder) == "ORDER BY JENIS_INVENTARIS asc, JENIS_INVENTARIS asc" )
	{
		/*
		...
		*/
		$sOrder = " ORDER BY JENIS_INVENTARIS_ID asc, INVENTARIS_ID asc, INVENTARIS_RUANGAN_ID asc ";
...
$allRecord = $kalkulas_penyusutan->getCountByParamsRekapKelompokFlexy($reqPeriode, $reqPeriodeLalu, array(), $statement_privacy);	
if($_GET['sSearch'] == "")
	$allRecordFilter = $allRecord;
else	
	$allRecordFilter = $kalkulas_penyusutan->getCountByParamsRekapKelompokFlexy($reqPeriode, $reqPeriodeLalu, array(), " AND (UPPER(NAMA_HEADER) LIKE '%".strtoupper($_GET['sSearch'])."%')".$statement_privacy);

$kalkulas_penyusutan->selectByParamsRekapKelompokFlexy($reqPeriode, $reqPeriodeLalu, array(), $dsplyRange, $dsplyStart, " AND (UPPER(NAMA_HEADER) LIKE '%".strtoupper($_GET['sSearch'])."%')".$statement_privacy, $sOrder);     		
