...
if ( isset( $_GET['iSortCol_0'] ) )
{
	...
	if ( trim($sOrder) == "ORDER BY HEADER asc, HEADER asc" )
	{
		/*
		...
		*/
		$sOrder = " ORDER BY KODE_HEADER, KODE, LOKASI_ID ASC";
...
if($_GET['sSearch'] == "")
	...
	$allRecordFilter = $kalkulas_penyusutan->getCountByParamsRekapAktiva(array(), " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%' OR UPPER(KODE) LIKE '%".strtoupper($_GET['sSearch'])."%')".$statement_privacy);

$kalkulas_penyusutan->selectByParamsRekapAktivaV2($reqPeriode, $reqPeriodeLalu, array(), $dsplyRange, $dsplyStart, " AND (UPPER(NAMA) LIKE '%".strtoupper($_GET['sSearch'])."%' OR UPPER(KODE) LIKE '%".strtoupper($_GET['sSearch'])."%')".$statement_privacy, $sOrder);     		
...
while($kalkulas_penyusutan->nextRow())
{
	...
	else
	{
		...
		else
		{
			...
			{
				if($aColumns[$i] == "NILAI_PEROLEHAN" || $aColumns[$i] == "NILAI_PENYUSUTAN"  || $aColumns[$i] == "AKM_PENYUSUTAN_LALU" || $aColumns[$i] == "AKM_PENYUSUTAN" || $aColumns[$i] == "NILAI_AKHIR")
					$row[] = "<div align=\"right\"><b>".numberToIna(array_sum($arr[$aColumns[$i]]))."</b></div>";			
				else if($aColumns[$i] == "SISA_UMUR")
					$row[] = "<div align=\"right\"><b>JUMLAH</b></div>";
				else if($aColumns[$i] == "HEADER")
					$row[] = $HEADER;
				else
					$row[] = "";
			}	
	...
	for ( $i=0 ; $i<count($aColumns) ; $i++ )
	{
		...
		{
			$arr[$aColumns[$i]][] = $kalkulas_penyusutan->getField($aColumns[$i]);
			$row[] = "<div align=\"right\">".numberToIna($kalkulas_penyusutan->getField($aColumns[$i]))."</div>";			
		}
		else
